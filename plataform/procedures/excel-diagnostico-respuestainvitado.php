<?php
	...
	$sql_reporte = "SELECT NumeroDocumento,TipoUsuario,Nombre,PD.IDPreguntaDiagnostico,DR.FechaTrCr
									FROM PreguntaDiagnostico PD,DiagnosticoRespuesta DR
									Where PD.IDPreguntaDiagnostico=DR.IDPreguntaDiagnostico and PD.IDDiagnostico = '".$_POST["IDDiagnostico"]."' and TipoUsuario = 'Externo' ".$condicion_fecha."
									Group by NumeroDocumento,DR.FechaTrCr";
    ...
    endwhile;
		...
		while( $Datos = $dbo->fetchArray( $result_reporte ) )
		{
			...
			$Fecha="";
			$sql_inv="SELECT IDInvitado,Nombre, Apellido From Invitado WHERE NumeroDocumento = '".$Datos["NumeroDocumento"]."' Limit 1";
			$r_inv=$dbo->query($sql_inv);
			$row_inv=$dbo->fetchArray($r_inv);


			$Nombre=$row_inv["Nombre"] . " " . $row_inv["Apellido"];
			...
			$html .= "<td>Externo</td>";
			$html .= "<td></td>";
			$sql_repuesta_socio="Select * From DiagnosticoRespuesta DR Where NumeroDocumento = '".$Datos["NumeroDocumento"]."' and FechaTrCr = '".$Datos["FechaTrCr"]."'  Group by IDPreguntaDiagnostico";

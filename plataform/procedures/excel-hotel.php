<?php
    ...
    $html .= "<th>NUMERO RESERVA</th>";
	  ...
	  $html .= "<th>HABITACION</th>";
		...
		$r_campos =& $dbo->all( "CampoHotel" , "IDClub = '" . $_POST["IDClub"] ."'");
		while( $r = $dbo->object( $r_campos ) ):
		   $array_campos[] = $r->IDCampoHotel;
		   $html .= "<th>".$r->Nombre ."</th>";
		 endwhile;
		$html .= "</tr>";
		...
		while( $Datos = $dbo->fetchArray( $result_reporte ) )
		{
			...
			//Consulto los campos dinamicos
			$r_campos =& $dbo->all( "HotelCampoHotel");
			while( $rdatos = $dbo->object( $r_campos ) ):
				 $array_otros_datos[$rdatos->IDReserva][$rdatos->IDCampoHotel] =  $rdatos->Valor;					
			 endwhile;				

		   if(count($array_campos)>0):
			 foreach($array_campos as $id_campo):
				 $html .= "<td>" . $array_otros_datos[$Datos["IDReserva"]][$id_campo] . "</td>";
			 endforeach;				
		   endif;

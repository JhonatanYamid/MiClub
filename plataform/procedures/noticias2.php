		...
		*/
			...
			if (!SIMNotify::capture(SIMUtil::valida($_POST, $array_valida), "error")) {
				...
				if ($frm["NotificarPush"] == "S") {
					...
					$frm["IDModulo"] = 66;
					$frm["TipoNotificacion"] = 'noticias2'; //socio
					...
					if ($frm["DirigidoA"] == "S") :
						...
						$sql_socios = "SELECT Socio.IDSocio, Socio.IDClub, Socio.Token, Socio.Dispositivo FROM Socio, SocioSeccion2 WHERE Socio.IDClub = '" . $frm["IDClub"] . "' AND Socio.IDSocio = SocioSeccion2.IDSocio AND SocioSeccion2.IDSeccion = '" . $frm["IDSeccion"] . "' and Token <> '' and Token <> '2byte'  " . $condicion_tipo;
			...
			if (!SIMNotify::capture(SIMUtil::valida($_POST, $array_valida), "error")) {
				...
				if ($frm["NotificarPush"] == "S") {
					...
					if ($frm["DirigidoA"] == "S" || $frm["DirigidoA"] == "T") :
						...
						//$sql_socios = "SELECT Socio.IDSocio, Socio.IDClub, Socio.Token, Socio.Dispositivo FROM Socio, SocioSeccion2 WHERE Socio.IDClub = '" . $frm["IDClub"] . "' AND Socio.IDSocio = SocioSeccion2.IDSocio AND SocioSeccion2.IDSeccion = '" . $frm["IDSeccion"] . "' and Token <> '' and Token <> '2byte' and Socio.IDSocio = '118543'";
						$frm["TipoUsuario"] = 'S'; //socio
						$qry_socios = $dbo->query($sql_socios);
						while ($r_socios = $dbo->fetchArray($qry_socios)) {
							SIMUtil::envia_cola_notificacion($r_socios, $frm);
		...
		case "DelImgNot":
			$campo = $_GET['cam'];
			if ($campo == "SWF") {
				$doceliminar = SWFNOTICIA_DIR . $dbo->getFields("Noticia2", "$campo", "IDNoticia = '" . $_GET[id] . "'");
				unlink($doceliminar);
				$dbo->query("UPDATE Noticia2 SET $campo = '' WHERE IDNoticia = $_GET[id] LIMIT 1 ;");
				SIMHTML::jsAlert(SIMUtil::get_traduccion('', '', 'SWFeliminadoCorrectamente', LANGSESSION));
			} else {
				$doceliminar = IMGNOTICIA_DIR . $dbo->getFields("Noticia2", "$campo", "IDNoticia = '" . $_GET[id] . "'");
				unlink($doceliminar);
				$dbo->query("UPDATE Noticia2 SET $campo = '' WHERE IDNoticia = $_GET[id] LIMIT 1 ;");

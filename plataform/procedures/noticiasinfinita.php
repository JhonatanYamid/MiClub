	...
	switch (SIMNet::req("action")) {
		...
		*/
		...
		case "DelImgNot":
			...
			if ($campo == "SWF") {
				$doceliminar = SWFNOTICIA_DIR . $dbo->getFields("$table", "$campo", "$key = '" . $_GET[id] . "'");
				unlink($doceliminar);
				...
				$doceliminar = IMGNOTICIA_DIR . $dbo->getFields("Noticia", "$campo", "$key = '" . $_GET[id] . "'");
				unlink($doceliminar);
				$dbo->query("UPDATE $table SET $campo = '' WHERE $key = $_GET[id] LIMIT 1 ;");
				SIMHTML::jsAlert(SIMUtil::get_traduccion('', '', 'ImagenEliminadaCorrectamente', LANGSESSION));
			}
			SIMHTML::jsRedirect("?mod=" . $mod . "&action=edit&id=" . $_GET[id] . "&IDModulo=" . $IDModulo);
			exit;
			break;
		...
		case "DelDocNot":
			$campo = $_GET['cam'];
			$IDModulo = $_GET["IDModulo"];
			$doceliminar = IMGNOTICIA_DIR . $dbo->getFields("$table", "$campo", "$key = '" . $_GET[id] . "'");
			unlink($doceliminar);
			$dbo->query("UPDATE $table SET $campo = '' WHERE $key = $_GET[id] LIMIT 1 ;");
			SIMHTML::jsAlert(SIMUtil::get_traduccion('', '', 'ImagenEliminadaCorrectamente', LANGSESSION));
			SIMHTML::jsRedirect("?mod=" . $mod . "&action=edit&id=" . $_GET[id] . "&IDModulo=" . $IDModulo);

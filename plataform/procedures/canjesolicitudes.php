	...
	switch (SIMNet::req("action")) {
		...
		case "update":
			...
			if (!SIMNotify::capture(SIMUtil::valida($_POST, $array_valida), "error")) {
				...
				$Mensaje = "Estado de la solicitud del canje es aprobado";
				$IDModulo = 34;

				//consulto el id del estado aprobado del canje
				$sql_estadocanj_club = "Select * From EstadoCanjeSolicitud Where IDClub = '" . SIMUser::get("club") . "' AND Descripcion='Aprobado'";
				$qry_estadocanj_club = $dbo->query($sql_estadocanj_club);
				$estadoCanje = $dbo->fetchArray($qry_estadocanj_club);
				$frm1 = $estadoCanje;

				if ($frm1["IDEstadoCanjeSolicitud"] == $_POST["IDEstadoCanjeSolicitud"]) {
					//envio push al socio 
					SIMUtil::enviar_notificacion_push_general($IDClub, $IDSocio, $Mensaje, $IDModulo, "");

					//envio push a los beneficiarios
					$datosBeneficiarios = explode("|", $_POST["IDSocioBeneficiario"]);
					for ($i = 0; $i < count($datosBeneficiarios); $i++) {

						SIMUtil::enviar_notificacion_push_general($IDClub, $datosBeneficiarios[$i], $Mensaje, $IDModulo, "");
					}
				}
				...
				if ($_POST[EnviarCorreo] == 1) :
					SIMUtil::notificar_solicitud_canje(SIMNet::reqInt("id"), "destino");
				endif;

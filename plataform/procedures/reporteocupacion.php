 <?
	...
	$sql_ocupacion_entrada = "Select  *
					  From LogAcceso
					  Where Tipo <> '' and IDClub = '" . SIMUser::get("club") . "' and Entrada = 'S' " .  $condicion_fecha_ingreso . " " . $condicion_busqueda;
	...
	while ($r_ocupacion_entrada = $dbo->fetchArray($result_ocupacion_entrada)) :
		...
		if ($iDInvitacion !== $r_ocupacion_entrada["IDInvitacion"]) {
			$iDInvitacion = $r_ocupacion_entrada["IDInvitacion"];
			$array_ocupacion_entrada_personas[$tipo_entrada]++;
		}
	endwhile;
	...
	$sql_ocupacion_salida = "Select *
					  From LogAcceso
					  Where Tipo <> '' and IDClub = '" . SIMUser::get("club") . "' and Salida = 'S' "  . $condicion_fecha_salida . " " . $condicion_busqueda;
	...
	while ($r_ocupacion_salida = $dbo->fetchArray($result_ocupacion_salida)) :
		...
		if ($iDInvitacion !== $r_ocupacion_salida["IDInvitacion"]) {
			$iDInvitacion = $r_ocupacion_salida["IDInvitacion"];
			$array_ocupacion_salida_personas[$tipo_salida]++;
		}
	endwhile;

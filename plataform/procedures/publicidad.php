	...
	switch (SIMNet::req("action")) {
		...
		case "insert":
			...
			if (!SIMNotify::capture(SIMUtil::valida($_POST, $array_valida), "error")) {
				...
				//Actualizo Servicios reserva
				if (count($frm["IDServicio"]) > 0) {
					foreach ($frm["IDServicio"] as $IDServicio) {
						$sql_pub_servicio = "INSERT INTO PublicidadServicio (IDPublicidad,IDServicio) VALUES ('" . SIMNet::reqInt("id") . "','" . $IDServicio . "')";
						$dbo->query($sql_pub_servicio);
					}
				}
		...
		case "update":
			...
			if (!SIMNotify::capture(SIMUtil::valida($_POST, $array_valida), "error")) {
				...
				//Actualizo Servicios reserva
				$sql_publi = "DELETE FROM PublicidadServicio WHERE IDPublicidad = '" . SIMNet::reqInt("id") . "'";
				$dbo->query($sql_publi);
				if (count($frm["IDServicio"]) > 0) {
					foreach ($frm["IDServicio"] as $IDServicio) {
						$sql_pub_servicio = "INSERT INTO PublicidadServicio (IDPublicidad,IDServicio) VALUES ('" . SIMNet::reqInt("id") . "','" . $IDServicio . "')";

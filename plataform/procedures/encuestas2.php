...
switch (SIMNet::req("action")) {
	...
	case "InsertarPregunta":
		$frm = SIMUtil::varsLOG($_POST);
		$id = $dbo->insert($frm, "PreguntaEncuesta2", "IDPreguntaEncuesta2");
		...
		for ($i = 1; $i <= $frm["CantidadOpciones"]; $i++) {
			if (!empty($frm["Respuesta" . $i])) {
				...
				$sql_insert = "INSERT INTO Encuesta2OpcionesRespuesta (IDEncuesta2Pregunta,IDEncuesta2PreguntaSiguiente,Opcion,RespuestaCorrecta,Puntos,Orden)
														 VALUES('" . $id . "','" . $frm["IDEncuesta2PreguntaSiguiente" . $i] . "','" . $frm["Respuesta" . $i] . "','" . $frm["RespuestaCorrecta" . $i] . "','" . $frm["Puntos" . $i] . "','" . $frm["Orden" . $i] . "')";
				$dbo->query($sql_insert);
			}
		}

		SIMHTML::jsAlert(SIMUtil::get_traduccion('', '', 'RegistroExitoso', LANGSESSION));
		SIMHTML::jsRedirect($script . ".php?action=edit&tabencuesta=formulario&id=" . $frm[IDEncuesta2]);
		exit;
		break;

	case "ModificaPregunta":
		$frm = SIMUtil::varsLOG($_POST);
		$dbo->update($frm, "PreguntaEncuesta2", "IDPreguntaEncuesta2", $frm["IDPreguntaEncuesta2"]);

		$borro_opciones = "DELETE FROM Encuesta2OpcionesRespuesta WHERE IDEncuesta2Pregunta = '" . $frm["IDPreguntaEncuesta2"] . "'";
		$dbo->query($borro_opciones);
		for ($i = 1; $i <= $frm["CantidadOpciones"]; $i++) {
			if (!empty($frm["Respuesta" . $i])) {
				$frm["Respuesta" . $i] = trim(preg_replace('/\s+/', ' ', $frm["Respuesta" . $i]));
				$sql_insert = "INSERT INTO Encuesta2OpcionesRespuesta (IDEncuesta2Pregunta,IDEncuesta2PreguntaSiguiente,Opcion,RespuestaCorrecta,Puntos,Orden)
																 VALUES('" . $frm["IDPreguntaEncuesta2"] . "','" . $frm["IDEncuesta2PreguntaSiguiente" . $i] . "','" . $frm["Respuesta" . $i] . "','" . $frm["RespuestaCorrecta" . $i] . "','" . $frm["Puntos" . $i] . "','" . $frm["Orden" . $i] . "')";
				$dbo->query($sql_insert);
			}
		}
	...
	case "EliminaPregunta":
		$id = $dbo->query("DELETE FROM PreguntaEncuesta2 WHERE IDPreguntaEncuesta2   = '" . $_GET["IDPreguntaEncuesta2"] . "' LIMIT 1");
	...
	case "DelDocNot":
		$campo = $_GET['cam'];
		$doceliminar = BANNERAPP_DIR . $dbo->getFields("Encuesta2", "$campo", "IDEncuesta2 = '" . $_GET[id] . "'");
		unlink($doceliminar);
		$dbo->query("UPDATE Encuesta2 SET $campo = '' WHERE IDEncuesta2 = $_GET[id] LIMIT 1 ;");

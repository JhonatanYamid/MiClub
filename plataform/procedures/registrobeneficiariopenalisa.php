...
switch ($_POST["action"]) {
	...
	case "insert":
		...
		if ($response != null && $response->success) {
			...
			SIMHTML::jsRedirect("registrobeneficiariopenalisa.php");
			exit;
		}
		...
		if (!empty($_POST["NumeroDocumento"]) && !empty($_POST["Nombre"]) && !empty($_POST["Apellido"]) && !empty($_POST["Email"]) && !empty($_POST["Telefono"]) && !empty($_POST["UnidadResidencial"]) && !empty($_POST["Parentesco"]) && $robot_verificacion == "S") :
			//seguridad para cada campo del formulario
			foreach ($_POST as $clave => $valor) {
				$_POST[$clave] = SIMUtil::antiinjection($valor);
			} //end for

			$AccionNuevo = $_POST["Accion"] . $_POST["NumeroDocumento"];
			$post = [
				'key' => 'CEr0CLUB',
				'IDClub' => '35',
				'action'   => 'setsocio',
				'Accion'   => $AccionNuevo,
				'AccionPadre'   => $_POST["Accion"],
			...
			$dest = "comunicacionesydeportes@clubpuertopenalisa.com";
			...
			if (mail($dest, "Solicitud registro beneficiario Puerto Peñalisa Mi Club App", $msg, $head))
				SIMHTML::jsRedirect("registrobeneficiariopenalisa.php?msg=1");
			else
				SIMHTML::jsRedirect("registrobeneficiariopenalisa.php?msg=2");
		else :

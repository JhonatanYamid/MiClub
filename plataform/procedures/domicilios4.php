	...
	switch (SIMNet::req("action")) {
			...
			if (!SIMNotify::capture(SIMUtil::valida($_POST, $array_valida), "error")) {
				...
				if ($frm["NotificarPush"] == "S") :
					...
					while ($r_socios = $dbo->fetchArray($qry_socios)) {
						...
						$NombrarSocios = $dbo->getFields("ConfiguracionClub", "TextoSocios", "IDClub = '" . SIMUser::get("club") . "'");
						$EstadoSocio = $dbo->getFields("EstadoDomicilio", "Nombre", "IDEstadoDomicilio = '" . $frm["IDEstadoDomicilio"] . "' And Publicar='S'");

						if (!empty($NombrarSocios)) {
							$message = "Estimado " . $NombrarSocios . ",le informamos que su pedido esta en estado: " . $EstadoSocio;
						} else {
						...
						//Guardo el log
						$sql_log = $dbo->query("Insert Into LogNotificacion (IDNotificacionesGenerales	, IDSocio, Mensaje, App,IDClub, Token, Dispositivo, Fecha) Values ('" . SIMNet::reqInt("id") . "', '" . $r_socios["IDSocio"] . "','" . $message . "','" . $App . "','" . $r_socios["IDClub"] . "','" . $r_socios["Token"] . "','" . $r_socios["Dispositivo"] . "',NOW())");
					} //end while
				endif;
		...
		case "CrearOrdenCompleta":
			...
			foreach ($_POST["Producto"] as $dato => $valor) :
				...
				$precio = $dbo->getFields("Producto4", "Precio", "IDProducto = '" . $valor . "'");
			...
			$respuesta = SIMWebServiceDomicilios::set_domicilio($IDClub, $IDSocio, $HoraEntrega, $ComentariosSocio, $DetallePedido, $Celular, $Direccion, $ValorDomicilio, $FormaPago, $Version, $IDRestaurante, $NumeroMesa, $CamposFormulario, $Propina);

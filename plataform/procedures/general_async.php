...
require("../../../admin/lib/SIMWebServiceReservas.inc.php");
...
} elseif ($datos->club == 141) { //Venezuela
...
while ($r_servicio = $dbo->fetchArray($qry_servicios)) {
    ...
    if (SIMUser::get("IDPerfil") < 2)  //es un coordinador se deben traer todos los elementos
    {
        ...
        $elementos = SIMWebServiceReservas::get_elementos(SIMUser::get("club"), "", $r_servicio["IDServicio"]);
    } //end if
    else {
        $coordinador = 0;
        $elementos = SIMWebServiceReservas::get_elementos(SIMUser::get("club"), "", $r_servicio["IDServicio"], SIMUser::get("IDUsuario"));
    } //else

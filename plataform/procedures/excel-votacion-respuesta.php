    ...
    $html .= "<th>VOTACION</th>";
		...
		while( $Datos = $dbo->fetchArray( $result_reporte ) )
		{
			...
			if($datos_encuesta["DirigidoA"]=="E"){
				$sql_usu="SELECT NumeroDocumento,Nombre FROM Usuario WHERE  IDUsuario = '".$Datos[IDSocio]."' Limit 1";
				$r_usu=$dbo->query($sql_usu);
				$row_usu=$dbo->fetch($r_usu);
				$NombreResponde= utf8_encode($row_usu["Nombre"]);
				$DocumentoResponde= $row_usu["NumeroDocumento"];
				//$NombreResponde= utf8_encode($dbo->getFields( "Usuario" , "Nombre" , "IDUsuario = '".$Datos[IDSocio]."'" ));
			}
			else{
				$sql_usu="SELECT Nombre,Apellido,NumeroDocumento FROM Socio WHERE  IDSocio = '".$Datos[IDSocio]."' Limit 1";
				$r_usu=$dbo->query($sql_usu);
				$row_usu=$dbo->fetch($r_usu);
				$NombreResponde= utf8_encode($row_usu["Nombre"] . " " . $row_usu["Apellido"]);
				$DocumentoResponde= $row_usu["NumeroDocumento"];
			...
			endwhile;
			  if(count($array_preguntas)>0):
				foreach($array_preguntas as $id_pregunta):
					if($datos_encuesta["IDClub"]!=23 && $datos_encuesta["IDClub"]!=7){
						$html .= "<td>privado</td>";
					}
					else {

...
if ($_POST["IDPerfil"] > 1) :
	...
	$condicion_area = " and PqrFuncionario.IDArea in (" . $id_areas . ")";
endif;
//condicion reporte por area
if (!empty($_POST["FechaInicio"]) && !empty($_POST["FechaFin"]) && !empty($_POST["IDArea"])) {
	$condicion_fecha = " and Fecha >= '" . $_POST["FechaInicio"] . "'  and Fecha <= '" . $_POST["FechaFin"] . "'" . "and IDArea='" . $_POST["IDArea"] . "'";
} else
...
if ($NumSocios > 0) {
	...
	while ($Datos = $dbo->fetchArray($result_reporte)) {
		...
		$html .= "</tr>";
		/* creo una nueva fila para contar las pqr abiertas,anuladas, cerradas, pendiente por asignar,
			en proceso o pendiente por aprobacion  */

		if ($Datos["IDPqrEstado"] == 1) {

			$ContadorEstadoPqrAbierto++;
		} else
		if ($Datos["IDPqrEstado"] == 2) {
			$ContadorEstadoPqrAnulado++;
		} else
		if ($Datos["IDPqrEstado"] == 3) {
			$ContadorEstadoPqrCerrado++;
		} else
		if ($Datos["IDPqrEstado"] == 4) {
			$ContadorEstadoPqrPendienteAsignar++;
		} else
		if ($Datos["IDPqrEstado"] == 5) {
			$ContadorEstadoPqrEnProceso++;
		} else
		if ($Datos["IDPqrEstado"] == 6) {
			$ContadorEstadoPqrPendienteAprobacion++;
		}
	}
	$html .= "<tr>";
	$html .= "<th>PQR ABIERTO</th>";
	$html .= "<th>PQR ANULADO</th>";
	$html .= "<th>PQR CERRADO</th>";
	$html .= "<th>PQR PENDIENTE POR ASIGNAR</th>";
	$html .= "<th>PQR EN PROCESO</th>";
	$html .= "<th>PQR PENDIENTE APROBACION</th>";
	$html .= "</tr>";



	$html .= "<td>" . $ContadorEstadoPqrAnulado . "</td>";
	$html .= "<td>" . $ContadorEstadoPqrCerrado . "</td>";
	$html .= "<td>" . $ContadorEstadoPqrPendienteAsignar . "</td>";
	$html .= "<td>" . $ContadorEstadoPqrEnProceso . "</td>";
	$html .= "<td>" . $ContadorEstadoPqrPendienteAprobacion . "</td>";
	$html .= "<td>" . $ContadorEstadoPqrAbierto . "</td>";
	$html .= "</table>";

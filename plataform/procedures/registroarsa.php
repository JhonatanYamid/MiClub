...
switch ($_POST["action"]) {
	...
	case "insert":
		...
		if ($response != null && $response->success) {
			...
			SIMHTML::jsRedirect("registronucleoarsa.php");
			exit;
		}
		...
		if (!empty($_POST["Nombre"]) && !empty($_POST["Apellido"]) && !empty($_POST["NumeroDocumento"])  && $robot_verificacion == "S") :
			...
			$id = $dbo->insert($_POST, "ActualizacionArsa", "IDActualizacionArsa");


			for ($i = 1; $i <= $_POST["TotalBenef"]; $i++) {
				if ($_POST["NombreBeneficiario" . $i]) {
					$sql_benef = "INSERT INTO ActualizacionArsaBeneficiarios (IDActualizacionArsa,NombreBeneficiario,ApellidoBeneficiario,NumeroIdentificacionBeneficiario,CorreoBeneficiario,TelefonoBeneficiario,FechaTrCr)
								VALUES('" . $id . "','" . $_POST["NombreBeneficiario" . $i] . "','" . $_POST["ApellidoBeneficiario" . $i] . "','" . $_POST["NumeroIdentificacionBeneficiario" . $i] . "','" . $_POST["CorreoBeneficiario" . $i] . "','" . $_POST["TelefonoBeneficiario" . $i] . "',NOW())";
					$dbo->query($sql_benef);
				}
			}
			...
			if ($confirm) {
				...
				SIMHTML::jsRedirect("registronucleoarsa.php?msg=1");
				//SIMHTML::jsRedirect( "miclubrancho://open" );
				exit;
			} else
				SIMHTML::jsRedirect("registronucleoarsa.php?msg=2");
		else :

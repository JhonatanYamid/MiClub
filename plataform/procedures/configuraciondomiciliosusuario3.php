 <?
	...
	SIMReg::setFromStructure(array(
		"title" => "ConfiguracionDomiciliosFuncionario",
		"table" => "ConfiguracionDomicilios3",
		"key" => "IDConfiguracionDomicilios",
		"mod" => "ConfiguracionDomicilios"
	));


	$script = "configuraciondomiciliosusuario3";
	...
	switch (SIMNet::req("action")) {
		...
		case "insert":
			...
			if (!SIMNotify::capture(SIMUtil::valida($_POST, $array_valida), "error")) {
				...
				foreach ($frm["IDTipoPago"] as $Pago_seleccionado) :
					$sql_servicio_forma_pago = $dbo->query("Insert into DomicilioTipoPago2 (IDConfiguracionDomicilio, IDTipoPago) Values ('" . $id . "', '" . $Pago_seleccionado . "')");
				endforeach;
		...
		case "update":
			...
			if (!SIMNotify::capture(SIMUtil::valida($_POST, $array_valida), "error")) {
				...
				$delete_tipo_pago = $dbo->query("Delete From DomicilioTipoPago2 Where IDConfiguracionDomicilio = '" . SIMNet::reqInt("id") . "'");
				foreach ($frm["IDTipoPago"] as $Pago_seleccionado) :
					$sql_servicio_forma_pago = $dbo->query("Insert into DomicilioTipoPago2 (IDConfiguracionDomicilio, IDTipoPago) Values ('" . SIMNet::reqInt("id") . "', '" . $Pago_seleccionado . "')");
				endforeach;

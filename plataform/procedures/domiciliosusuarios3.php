		...
		case "update":
			...
			if (!SIMNotify::capture(SIMUtil::valida($_POST, $array_valida), "error")) {
				...
				if ($frm["NotificarPush"] == "S") :
					...
					$sql_socios = "SELECT Usuario.* FROM  Usuario WHERE Usuario.IDClub = '" . SIMUser::get("club") . "' AND  IDUsuario = '" . $frm["IDUsuario"] . "' AND Token <> '' and Token <> '2byte' Limit 1";
					...
					while ($r_socios = $dbo->fetchArray($qry_socios)) {
						...
						$message = "Estimado Usuario, le informamos que su pedido esta en estado: " . $EstadoSocio;
						$App = "Empleado";
						$custom = array(
						...
						//Guardo el log
						$sql_log = $dbo->query("Insert Into LogNotificacion (IDNotificacionesGenerales, IDUsuario,Mensaje,App,IDClub, Token, Dispositivo, Fecha) Values ('" . SIMNet::reqInt("id") . "', '" . $r_socios["IDUsuario"] . "','" . $message . "','" . $App . "','" . $r_socios["IDClub"] . "','" . $r_socios["Token"] . "','" . $r_socios["Dispositivo"] . "',NOW())");
					} //end while
				endif;

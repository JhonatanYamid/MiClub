...
switch ($_POST["action"]) {
	...
	case "insert":
		...
		if ($response != null && $response->success) {
			...
			SIMHTML::jsRedirect("actualizasocioarrayanes.php");
			exit;
		}
		...
		if (!empty($_POST["NumeroDerecho"]) && !empty($_POST["NombreApellidoTitular"]) && !empty($_POST["DocumentoIdentidad"])  && $robot_verificacion == "S") :
			...
			echo $_POST["TotalBenef"];


			for ($i = 1; $i <= $_POST["TotalBenef"]; $i++) {
				if ($_POST["Nombre" . $i]) {
					$sql_benef = "INSERT INTO ActualizacionArrayanesBeneficiarios (IDActualizacionArrayanes,DocumentoBeneficiario,Nombre,Parentesco,CorreoElectronico,Telefono,FechaNacimiento,Ocupacion,EmpresaUniversidadColegio,Deporte,FechaTrCr)
								VALUES('" . $id . "','" . $_POST["DocumentoBeneficiario" . $i] . "','" . $_POST["Nombre" . $i] . "','" . $_POST["Parentesco" . $i] . "','" . $_POST["CorreoBeneficiario" . $i] . "','" . $_POST["Telefono" . $i] . "','" . $_POST["FechaNacimientoBeneficiario" . $i] . "','" . $_POST["OcupacionBeneficiario" . $i] . "','" . $_POST["EmpresaUniversidadColegio" . $i] . "','" . $_POST["DeporteBeneficiario" . $i] . "',NOW())";
					$dbo->query($sql_benef);
				}
			}
			...
			if ($confirm) {
				...
				SIMHTML::jsRedirect("actualizasocioarrayanes.php?msg=1");
				//SIMHTML::jsRedirect( "miclubrancho://open" );
				exit;
			} else
				SIMHTML::jsRedirect("actualizasocioarrayanes.php?msg=2");
		else :

...
switch (SIMNet::req("action")) {
		...
		*/
		...
		if (!SIMNotify::capture(SIMUtil::valida($_POST, $array_valida), "error")) {
			...
			for ($cont_invitado = 1; $cont_invitado <= ($frm["NumeroInvitados"] - 1); $cont_invitado++) :
				...
				$NumeroCarnet = "NumeroCarnet" . $cont_invitado;
				$Predio = "Predio" . $cont_invitado;
				if (!empty($frm[$campo_nombre]) && !empty($frm[$campo_documento])) :
					...
					$array_datos_invitado["NumeroCarnet"] = $frm[$NumeroCarnet];
					$array_datos_invitado["Predio"] = $frm[$Predio];
			...
			if ($frm["NumeroInvitados"] > 0) :
				...
				$respuesta = SIMWebServiceAccesos::set_autorizacion_invitado($frm["IDClub"], $frm["IDSocio"], $frm["FechaInicio"], $frm["FechaFin"], $DatosInvitado, SIMUser::get("IDUsuario"), $val, $val);
	...
	case "updateinfo":
		...
		if (!SIMNotify::capture(SIMUtil::valida($_POST, $array_valida), "error")) {
			...
			if ($frm["NumeroInvitados"] > 0) :
				//Servicio de invitados
				$respuesta = SIMWebServiceAccesos::set_autorizacion_invitado_update($frm["IDClub"], $frm["IDSocio"], $frm["ID"], $frm["FechaInicio"], $frm["FechaFin"], $DatosInvitado, "");
				...
				SIMHTML::jsAlert(SIMUtil::get_traduccion('', '', 'Faltanparametros', LANGSESSION));
				SIMNotify::capture("Faltan parametros", "error alert-danger");
			endif;
		} else
			exit;

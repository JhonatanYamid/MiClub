 <?
	...
	function get_data_accion($nombrearchivo, $file, $IGNORE_FIRTS_ROW, $FIELD_TEMINATED = '', $field = '', $IDClub, $IDGrupoSocio)
	{
		...
		if ($fp = fopen($file, "r")) {
			...
			if (count($array_id_socio) > 0) :
				...
				$sql_grupo = "Update GrupoSocio Set IDSocio = '" . $id_socios . "' Where IDGrupoSocio = '" . $IDGrupoSocio . "'";
				$dbo->query($sql_grupo);
			endif;
		...
		case "insert":
			...
			if (!SIMNotify::capture(SIMUtil::valida($_POST, $array_valida), "error")) {
				...
				if (count($datos_invitado) > 0) :
					$frm["IDSocio"] = implode("|||", $datos_invitado);
				endif;
	    ...
	    if(!empty($file)):
            ...
            for ($row = 1; $row <= $highestRow; $row++) {
                $Cliente = $sheet->getCell("A" . $row)->getValue();
                   ...
                   $query = $dbo->query("SELECT IDSocio FROM Socio WHERE Accion='$Cliente' AND IDClub='".$_POST['IDClub']."'");

                   $row_socios = $dbo->fetchArray($query);

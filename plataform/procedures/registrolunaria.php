...
switch ($_POST["action"]) {
	...
	case "insert":
		...
		if ($response != null && $response->success) {
			...
			SIMHTML::jsRedirect("registrolunaria.php");
			exit;
		}
		...
		if (!empty($_POST["Nombre"]) && !empty($_POST["Apellido"]) && !empty($_POST["NumeroDocumento"])  && $robot_verificacion == "S") :
			...
			$id = $dbo->insert($_POST, "ActualizacionLunaria", "IDActualizacionLunaria");

			for ($i = 1; $i <= $_POST["TotalBenef"]; $i++) {
				if ($_POST["NombreBeneficiario" . $i]) {
					$sql_benef = "INSERT INTO ActualizacionLunariaBeneficiarios 
													(IDActualizacionLunaria,
													NombreBeneficiario,
			...
			for ($i = 1; $i <= $_POST["TotalMascotas"]; $i++) {
				if ($_POST["NombreMascota" . $i]) {
					...
					$sql_mascota = "INSERT INTO ActualizacionMascotasLunaria
													(IDActualizacionLunaria,
													NombreMascota,
													EspecieMascota,													
													Raza,													
			...
			for ($i = 1; $i <= $_POST["TotalVehiculos"]; $i++) {
				if ($_POST["PlacaVehiculo" . $i]) {
					...
					$sql_vehiculo = "INSERT INTO ActualizacionVehiculosLunaria
													(IDActualizacionLunaria,
													TipoVehiculo,
													Marca,
													Color,													
			...
			for ($i = 1; $i <= $_POST["TotalContactos"]; $i++) {
				if ($_POST["NombreContacto" . $i]) {
					...
					$sql_contacto = "INSERT INTO ActulizacionContactosLunaria
													(IDActualizacionLunaria,
													Nombre,
													Telefono1,													
													Telefono2,													
			...
			SIMHTML::jsRedirect("registrolunaria.php?mensaje=Registro de datos exitoso");
		else :

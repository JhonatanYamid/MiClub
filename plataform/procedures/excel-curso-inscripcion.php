...
if (!empty($_GET["IDCursoSede"])) {
	...
	$sql_busq = "SELECT DISTINCT(IDCursoHorario) From CursoSede CS, CursoHorario CH
								 ...
								 AND CS.IDCursoSede = '" . $_GET["IDCursoSede"] . "' and CH.IDClub = '" . $_GET["IDClub"] . "'";
	echo $sql_busq;
	$result_busq = $dbo->query($sql_busq);
	while ($row_busq = $dbo->fetchArray($result_busq)) :
		$array_id_horario[] = $row_busq["IDCursoHorario"];
...
$sql_reporte = "Select * From CursoInscripcion Where IDClub = '" . $_GET["IDClub"] . "'  " . $where . " " . " Order By IDCursoInscripcion DESC";
...
if ($NumSocios > 0) {
	...
	while ($Datos = $dbo->fetchArray($result_reporte)) {
		...
		$html .= "<td>" . $datos_socio["CorreoElectronico"] . "</td>";

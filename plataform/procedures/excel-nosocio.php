...
$sql = "SELECT IDEvento, IDNoSocios, UsuarioTrCr, FechaTrCr
        FROM EventoRegistro 
...
$nombre = "Participantes_Externos_Reporte:" . date("Y_m_d");
...
if ($registros > 0) {
    ...
    while ($Datos = $dbo->fetchArray($result)) {
        ...
        //Evento
        $evento = $dbo->getFields("Evento", array("IDSeccionEvento", "Titular", "Introduccion", "Lugar", "Hora", "FechaEvento","FechaFinEvento", "Valor", "MaximoParticipantes"), "IDEvento = " . $Datos["IDEvento"]);

        //Seccion
        $seccion = $dbo->getFields("SeccionEvento", "Nombre", "IDSeccionEvento = " . $evento["IDSeccionEvento"]);

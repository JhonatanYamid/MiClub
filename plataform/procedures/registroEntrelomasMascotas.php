		...
		if (!empty($_POST["Nombre"]) && !empty($_POST["Apellido"]) && !empty($_POST["NumeroDocumento"])  && $robot_verificacion == "S") :
			...
			for ($i = 1; $i <= $_POST["TotalMascotas"]; $i++) {
				if ($_POST["NombreMascota" . $i]) {
					...
					$id = $dbo->insert($frm1, 'ActualizacionMascotasEntrelomas', 'IDActualizacionMascotasEntrelomas');

					$documento = $dbo->getFields("Socio", "IDSocio", " NumeroDocumento = '" . $_POST["NumeroDocumento"] . "' AND IDClub = 75");

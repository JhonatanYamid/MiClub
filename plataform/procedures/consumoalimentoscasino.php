...
function get_data($nombrearchivo, $file, $IGNORE_FIRTS_ROW, $FIELD_TEMINATED = '', $field = '', $IDClub)
{
    ...
    for ($row = 2; $row <= $highestRow; $row++) {
        ...
        $Cena =  utf8_decode($sheet->getCell("E" . $row)->getValue());
        $FechaInicio =  utf8_decode($sheet->getCell("F" . $row)->getValue());
        $FechaFin =  utf8_decode($sheet->getCell("G" . $row)->getValue());
        ...
        if (!empty($Cedula)) {
            ...
            //Consulto Socio
            $sql_socio = "SELECT Cedula
                      From ConsumoAlimentosCasino
                      Where IDClub = '" . $IDClub . "' and Cedula = '" . $Cedula . "' Limit 1";
            ...
            if ($dbo->rows($result_socio) > 0) :
                ...
                $sql_edit_socio = "UPDATE ConsumoAlimentosCasino Set Nombre = '" . $Nombre . "', Desayuno = '" . $Desayuno . "', Almuerzo = '" . $Almuerzo . "', Cena = '" . $Cena . "', FechaInicio = '" . $FechaInicio . "', FechaFin = '" . $FechaFin . "'"  .   "Where Cedula = '" . $Cedula . "' and IDClub = '" . $IDClub . "'";
                ...
                if (!empty($Cedula) && !empty($Nombre)) {
                    //Crear Socio
                    $sql_inserta_socio = "INSERT INTO ConsumoAlimentosCasino(IDClub,Cedula, Nombre, Desayuno,Almuerzo,Cena,FechaInicio,FechaFin)
                                Values ('" . $IDClub . "','" . $Cedula . "','" . $Nombre . "','" . $Desayuno . "','" . $Almuerzo . "','"  . $Cena . "','" . $FechaInicio . "','
                                " . $FechaFin . "')";
                    //echo "<br>Crear ";

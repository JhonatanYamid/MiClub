...
switch ($_POST["action"]) {
    ...
    case "insert":
        ...
        if ($response != null && $response->success) {
            ...
            SIMHTML::jsRedirect("registro127livingMascotas.php");
            exit;
        }
        ...
        if (!empty($_POST["Nombre"]) && !empty($_POST["Apellido"]) && !empty($_POST["NumeroDocumento"]) && $robot_verificacion == "S") :
            ...
            for ($i = 1; $i <= $_POST["TotalMascotas"]; $i++) {
                if ($_POST["NombreMascota" . $i]) {
                    ...
                    $id = $dbo->insert($frm1, 'ActualizacionMascotas127Living', 'IDActualizacionMascotas127Living');

                    $IDClub = 102;
                    $documento = $dbo->getFields("Socio", "IDSocio", " NumeroDocumento = '" . $_POST["NumeroDocumento"] . "' AND IDClub = $IDClub");

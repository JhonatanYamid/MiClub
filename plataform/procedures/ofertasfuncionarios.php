 <?
	...
	switch (SIMNet::req("action")) {
		...
		case "insert":
			...
			if (!SIMNotify::capture(SIMUtil::valida($_POST, $array_valida), "error")) {
				...
				if ($frm["NotificarPush"] == "S") {
					...
					$frm["IDModulo"] = 80;
					$frm["Titulo"]="Oferta Laboral";
					$frm["TipoNotificacion"] = 'noticias'; //socio
					$frm["Mensaje"] = "Se ha creado una nueva oferta de trabajo, quizas te interese";
					...
					//notiifcar push
						...
						//traer funcionarios
		$sql_socios = "SELECT * FROM Usuario WHERE IDClub = '" . $frm["IDClub"] . "' and Token <> '' and Token <> '2byte'   ";
						...
						while ($r_socios = $dbo->fetchArray($qry_socios)) {
							...
							//Guardo el log
							$sql_log = $dbo->query("INSERT INTO LogNotificacion (IDNotificacionesGenerales	, IDUsuario, IDClub, Token, Dispositivo, Fecha, App, Tipo, Titulo, Mensaje, Modulo, IDSeccion, IDDetalle)
              Values ('" . $id . "', '" . $r_socios["IDUsuario"] . "','" . $r_socios["IDClub"] . "','" . $r_socios["Token"] . "','" . $r_socios["Dispositivo"] . "',NOW(),'Empleado','" . $frm["TipoNotificacion"] . "', '" . $frm["Titulo"] . "', '" . $frm["Mensaje"] . "', '" . $frm["IDModulo"] . "','" . $frm["IDSeccion"] . "', '" . $id . "')");
						} 
		...
		case "actualizarcandidato":
			...
			$idofertacandidato = $_POST['IDOfertaCandidato'];
			$estado = $_POST['Estado'];
			$IDUsuario = $_POST['IDUsuario'];
			//$estado = $_POST['Estado'];
			//$IDSocio = $_POST['IDsocio'];
			$query = "UPDATE OfertaCandidato SET Estado= '$estado' WHERE IDOfertaCandidato='$idofertacandidato'";
			$dbo->query($query);
			$Mensaje = "Estado actual de la oferta " . $estado;
			SIMUtil::enviar_notificacion_push_general_funcionario($_POST["IDClub"], $IDUsuario, $Mensaje);
			SIMHTML::jsAlert(SIMUtil::get_traduccion('', '', 'RegistroGuardadoCorrectamente', LANGSESSION));
			SIMHTML::jsRedirect($script . ".php?action=edit&tabclasificados=candidatos&id=" . $_GET["id"]);
			break;

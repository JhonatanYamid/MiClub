...
//Actualizar socios
// $actualiza = SIMWebService::actualizar_plan_socios($idSocio);
// echo $actualiza;
...
$sql = "SELECT 
			...
			pf.Nombre as Producto, pf.IDServicioMaestro, pf.IDServicio, pf.IDTipoFacturacion, pf.TipoCongelacion,
			...
			f.IDClub, f.FechaCreacion, tf.IDCategoriaFacturacion, pp.Precio, pp.IDTalonera, tf.PermitirReservar
		FROM 
			RegistroSocioProducto as r, ProductoFacturacion as pf, Facturacion as f, Socio as s, Club as c, 
			TipoFacturacion as tf, ProductoPrecio as pp
		WHERE 
			...
			r.IDSocio = s.IDSocio AND f.IDClub = c.IDClub AND pf.IDTipoFacturacion = tf.IDTipoFacturacion AND 
			pp.IDProductoFacturacion = pf.IDProductoFacturacion) ";

$result = $dbo->query($sql);

while($row = $dbo->fetchArray($result)) {

	if($row['IDCategoriaFacturacion'] == 1){
		...
		else if($fechaInicio <= $hoy && $fechaFin > $hoy && $row['FechaInicio'] != 0 && $row['Estado'] != 1 && $row['Estado'] != 4){
		...
		if($fechaFin < $hoy && $row['FechaInicio'] != 0 && $row['FechaFin'] != 0 && $row['Estado'] < 3 ){

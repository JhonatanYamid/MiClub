...
switch ($_POST["action"]) {
	...
	case "insert":
		...
		if ($response != null && $response->success) {
			...
			SIMHTML::jsRedirect("registrocaminoarrayanes.php");
			exit;
		}
		...
		if (!empty($_POST["Nombre"]) && !empty($_POST["Apellido"]) && !empty($_POST["NumeroDocumento"])  && $robot_verificacion == "S") :
			...
			$id = $dbo->insert($_POST, "ActualizacionCaminoArrayanes", "IDAActualizacionCaminoArrayanes");

			for ($i = 1; $i <= $_POST["TotalBenef"]; $i++) {
				if ($_POST["NombreBeneficiario" . $i]) {
					echo $sql_benef = "INSERT INTO ActualizacionCaminoArrayanesBeneficiarios 
													(IDActualizacionCaminoArrayanes,
													NombreBeneficiario,
			...
			for ($i = 1; $i <= $_POST["TotalEmpleados"]; $i++) {
				if ($_POST["Empleado" . $i]) {
					$sql_benef = "INSERT INTO ActualizacionCaminoArrayanesBeneficiarios 
													(IDActualizacionCaminoArrayanes,
													NombreBeneficiario,
													ApellidoBeneficiario,
													ManzanaBeneficiario,
											...
											VALUES	('" . $id . "',
													...
													'" . $_POST["ManzanaEmpleado" . $i] . "',
													'" . $_POST["CasaEmpleado" . $i] . "',
			...
			if ($confirm) {
				...
				SIMHTML::jsRedirect("registrocaminoarrayanes.php?msg=1");
				//SIMHTML::jsRedirect( "miclubrancho://open" );
				exit;
			} else
				SIMHTML::jsRedirect("registrocaminoarrayanes.php?msg=2");
		else :

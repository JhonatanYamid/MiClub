	...
	switch (SIMNet::req("action")) {
		...
		case "insert":
			...
			if (!SIMNotify::capture(SIMUtil::valida($_POST, $array_valida), "error")) {
				...
				SIMUtil::noticar_nuevo_pqr($id);
				SIMUtil::noticar_respuesta_aut_pqr($id);
		...
		case "update":
			...
			if (!SIMNotify::capture(SIMUtil::valida($_POST, $array_valida), "error")) {
				...
				// si el estado de la pqr es 3 que es cerrado tomo la fecha del servidor y inserto la fecha en el campo FechaFinalizacion
				$IDPqrEstadoActual = $_POST['IDPqrEstado'];
				if ($IDPqrEstadoActual == 3) {
					$dia = date("d");
					$mes = date("m");
					$annio = date("Y");


					$fechaServidor = $annio . "-" . $mes . "-" . $dia;
					$frm["FechaFinalizacion"] = $fechaServidor;
				} else {
					$frm["FechaFinalizacion"] = "";
				}
				...
				if (!empty($_POST[Cuerpo])) {
					...
					$correoSocio = utf8_encode($dbo->getFields("Socio", "CorreoElectronico", "IDSocio = '" . $frm["IDSocio"] . "'"));
					$sql_inserta_respuesta = "INSERT INTO Detalle_Pqr (IDPqr, IDUsuario, Fecha, Respuesta, Archivo, UsuarioTrCr, FechaTrCr,CorreoRespuestaSocio)
												VALUES ('" . $frm[IDPqr] . "','" . SIMUser::get("IDUsuario") . "','" . date("Y-m-d") . "','" . $_POST[Cuerpo] . "','" . $frm["Archivo"] . "','Admin',NOW(),'" . $correoSocio . "')";
					...
					if ($notificar_cliente == "S") :
						...
						if ($frm["IDClub"] == 51) {
							$Mensaje = "Cordial Saludo, se ha dado respuesta a su " . $nombre_modulo . ",Numero " . $frm["Numero"] . " con el asunto:" . $frm["Asunto"]  . ", por favor ingrese al correo electrónico para conocer mas detalles.";
						} else {
							$Mensaje = "Cordial Saludo, se ha dado respuesta a su " . $nombre_modulo . ",Numero " . $frm["Numero"] . " con el asunto:" . $frm["Asunto"] . ", por favor ingrese al app para conocer mas detalles.";
						}

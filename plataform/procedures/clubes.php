 ...
 */
        ...
        case "InsertarConfiguracionDirectorioSocio":
            $frm = SIMUtil::varsLOG($_POST);
            $id = $dbo->insert($frm, "ConfiguracionDirectorioSocio", "IDConfiguracionDirectorioSocio");
            SIMHTML::jsAlert(SIMUtil::get_traduccion('', '', 'RegistroExitoso', LANGSESSION));
            SIMHTML::jsRedirect($script . ".php?action=edit&tabclub=parametros&tabparametro=configuraciondirectoriosocio&id=" . $frm[IDClub]);
            exit;
            break;

        case "ModificarConfiguracionDirectorioSocio":
            $frm = SIMUtil::varsLOG($_POST);
            $dbo->update($frm, "ConfiguracionDirectorioSocio", "IDConfiguracionDirectorioSocio", $frm[IDConfiguracionDirectorioSocio]);
            SIMHTML::jsAlert(SIMUtil::get_traduccion('', '', 'ModificacionExitoso', LANGSESSION));
            SIMHTML::jsRedirect($script . ".php?action=edit&tabclub=parametros&tabparametro=configuraciondirectoriosocio&id=" . $frm[IDClub]);
            exit;
            break;
        ...
        case "InsertarConfiguracionDirectorio":
            $frm = SIMUtil::varsLOG($_POST);
            $id = $dbo->insert($frm, "ConfiguracionDirectorio", "IDConfiguracionDirectorio");
            SIMHTML::jsAlert(SIMUtil::get_traduccion('', '', 'RegistroExitoso', LANGSESSION));
            SIMHTML::jsRedirect($script . ".php?action=edit&tabclub=parametros&tabparametro=configuraciondirectorio&id=" . $frm[IDClub]);
            exit;
            break;

        case "ModificarConfiguracionDirectorio":
            $frm = SIMUtil::varsLOG($_POST);
            $dbo->update($frm, "ConfiguracionDirectorio", "IDConfiguracionDirectorio", $frm[IDConfiguracionDirectorio]);
            SIMHTML::jsAlert(SIMUtil::get_traduccion('', '', 'ModificacionExitoso', LANGSESSION));
            SIMHTML::jsRedirect($script . ".php?action=edit&tabclub=parametros&tabparametro=configuraciondirectorio&id=" . $frm[IDClub]);
            exit;
            break;

        case "EliminaConfiguracionDirectorio":
            $id = $dbo->query("DELETE FROM ConfiguracionDirectorio WHERE IDConfiguracionDirectorio   = '" . $_GET[IDConfiguracionDirectorio] . "' LIMIT 1");
            SIMHTML::jsAlert(SIMUtil::get_traduccion('', '', 'EliminacionExitosa', LANGSESSION));
            SIMHTML::jsRedirect($script . ".php?action=edit&tabclub=parametros&tabparametro=configuraciondirectorio&id=" . $_GET["id"]);
            exit;
            break;



        case "InsertarConfiguracionGaleria":
            $frm = SIMUtil::varsLOG($_POST);

            $files = SIMFile::upload($_FILES["ImagenMeGustaLleno"], GALERIA_DIR, "IMAGE");
            if (empty($files) && !empty($_FILES["ImagenMeGustaLleno"]["name"])) {
                SIMNotify::capture("Ha ocurrido un error durante la carga de la imagen. Verifique que la imagen no contenga errores y que el tipo de archivo sea permitido.", "error");
            }
            $frm["ImagenMeGustaLleno"] = $files[0]["innername"];

            $files = SIMFile::upload($_FILES["ImagenMeGustaVacio"], GALERIA_DIR, "IMAGE");
            if (empty($files) && !empty($_FILES["ImagenMeGustaVacio"]["name"])) {
                SIMNotify::capture("Ha ocurrido un error durante la carga de la imagen. Verifique que la imagen no contenga errores y que el tipo de archivo sea permitido.", "error");
            }
            $frm["ImagenMeGustaVacio"] = $files[0]["innername"];

            $id = $dbo->insert($frm, "ConfiguracionGaleria", "IDConfiguracionGaleria");
            SIMHTML::jsAlert(SIMUtil::get_traduccion('', '', 'RegistroExitoso', LANGSESSION));
            SIMHTML::jsRedirect($script . ".php?action=edit&tabclub=parametros&tabparametro=configuraciongaleria&id=" . $frm[IDClub]);
            exit;
            break;

        case "ModificarConfiguracionGaleria":
            $frm = SIMUtil::varsLOG($_POST);
            $files = SIMFile::upload($_FILES["ImagenMeGustaLleno"], GALERIA_DIR, "IMAGE");
            if (empty($files) && !empty($_FILES["ImagenMeGustaLleno"]["name"])) {
                SIMNotify::capture("Ha ocurrido un error durante la carga de la imagen. Verifique que la imagen no contenga errores y que el tipo de archivo sea permitido.", "error");
            ...
            $frm["ImagenMeGustaVacio"] = $files[0]["innername"];
            $dbo->update($frm, "ConfiguracionGaleria", "IDConfiguracionGaleria", $frm[IDConfiguracionGaleria]);
            SIMHTML::jsAlert(SIMUtil::get_traduccion('', '', 'ModificacionExitoso', LANGSESSION));
            SIMHTML::jsRedirect($script . ".php?action=edit&tabclub=parametros&tabparametro=configuraciongaleria&id=" . $frm[IDClub]);
            exit;
            break;

        case "EliminaConfiguracionGaleria":
            $id = $dbo->query("DELETE FROM ConfiguracionGaleria WHERE IDConfiguracionGaleria   = '" . $_GET[IDConfiguracionGaleria] . "' LIMIT 1");
            SIMHTML::jsAlert(SIMUtil::get_traduccion('', '', 'EliminacionExitosa', LANGSESSION));
            SIMHTML::jsRedirect($script . ".php?action=edit&tabclub=parametros&tabparametro=configuraciongaleria&id=" . $_GET["id"]);
            exit;
            break;

        case "InsertarConfiguracionGaleriaEmpleados":
            $frm = SIMUtil::varsLOG($_POST);
            $files = SIMFile::upload($_FILES["ImagenMeGustaLleno"], GALERIA_DIR, "IMAGE");
            if (empty($files) && !empty($_FILES["ImagenMeGustaLleno"]["name"])) {
                SIMNotify::capture("Ha ocurrido un error durante la carga de la imagen. Verifique que la imagen no contenga errores y que el tipo de archivo sea permitido.", "error");
            ...
            $frm["ImagenMeGustaVacio"] = $files[0]["innername"];
            $id = $dbo->insert($frm, "ConfiguracionGaleriaEmpleados", "IDConfiguracionGaleriaEmpleados");
            SIMHTML::jsAlert(SIMUtil::get_traduccion('', '', 'RegistroExitoso', LANGSESSION));
            SIMHTML::jsRedirect($script . ".php?action=edit&tabclub=parametros&tabparametro=configuraciongaleriaempleados&id=" . $frm[IDClub]);
            exit;
            break;

        case "ModificarConfiguracionGaleriaEmpleados":
            $frm = SIMUtil::varsLOG($_POST);
            ...
            $dbo->update($frm, "ConfiguracionGaleriaEmpleados", "IDConfiguracionGaleriaEmpleados", $frm[IDConfiguracionGaleriaEmpleados]);
            SIMHTML::jsAlert(SIMUtil::get_traduccion('', '', 'ModificacionExitoso', LANGSESSION));
            SIMHTML::jsRedirect($script . ".php?action=edit&tabclub=parametros&tabparametro=configuraciongaleriaempleados&id=" . $frm[IDClub]);
            exit;
            break;

        case "EliminaConfiguracionGaleriaEmpleados":
            $id = $dbo->query("DELETE FROM ConfiguracionGaleriaEmpleados WHERE IDConfiguracionGaleriaEmpleados   = '" . $_GET[IDConfiguracionGaleriaEmpleados] . "' LIMIT 1");
            SIMHTML::jsAlert(SIMUtil::get_traduccion('', '', 'EliminacionExitosa', LANGSESSION));
            SIMHTML::jsRedirect($script . ".php?action=edit&tabclub=parametros&tabparametro=configuraciongaleriaempleados&id=" . $_GET["id"]);
            exit;
            break;
        ...
        case "EliminaConfiguracionDirectorioSocio":
            $id = $dbo->query("DELETE FROM ConfiguracionDirectorioSocio WHERE IDConfiguracionDirectorioSocio   = '" . $_GET[IDConfiguracionDirectorioSocio] . "' LIMIT 1");
            SIMHTML::jsAlert(SIMUtil::get_traduccion('', '', 'EliminacionExitosa', LANGSESSION));
            SIMHTML::jsRedirect($script . ".php?action=edit&tabclub=parametros&tabparametro=configuraciondirectoriosocio&id=" . $_GET["id"]);
            exit;
            break;
        ...
        case "InsertarModuloTipoSocio":
            ...
            if (count($frm["IDCategoriaServicio"]) > 0) :
                $servicios = implode("|", $frm["IDCategoriaServicio"]);
                $frm["IDCategoriaServicio"] = $servicios;
            else :
                $frm["IDCategoriaServicio"] = "";
            endif;
        ...
        case "updatesubmodulos":
            ...
            while ($r = $dbo->object($r_modulo)) {
                ...
                $nombre_campo_valores_Not2 = "SeccionNoticia2" . $r->IDModulo;
                $nombre_campo_valores_Not3 = "SeccionNoticia3" . $r->IDModulo;
                $nombre_campo_valores_NotI = "SeccionNoticiaInfi" . $r->IDModulo;
                ...
                unset($valores_secc_noticia2);
                unset($valores_secc_noticia3);
                unset($valores_secc_noticiaI);
                ...
                if (!empty($frm[$nombre_campo_valores_Not2]) && count($frm[$nombre_campo_valores_Not2]) > 0) :
                    $valores_secc_noticia2 = implode("|", $frm[$nombre_campo_valores_Not2]);
                    $modulo_configurado = "S";
                endif;
                if (!empty($frm[$nombre_campo_valores_Not3]) && count($frm[$nombre_campo_valores_Not3]) > 0) :
                    $valores_secc_noticia3 = implode("|", $frm[$nombre_campo_valores_Not3]);
                    $modulo_configurado = "S";
                endif;
                if (!empty($frm[$nombre_campo_valores_NotI]) && count($frm[$nombre_campo_valores_NotI]) > 0) :
                    $valores_secc_noticiaI = implode("|", $frm[$nombre_campo_valores_NotI]);
                    $modulo_configurado = "S";
                endif;
            ...
            SIMHTML::jsAlert(SIMUtil::get_traduccion('', '', 'RegistroGuardadoCorrectamente', LANGSESSION));
            SIMHTML::jsRedirect($script . ".php?action=edit&vista=submodulos&tabclub=submodulos&id=" . SIMNet::reqInt("id"));
            break;
        ...
        case "InsertCorreoAlertaAcceso":
            ...
            SIMHTML::jsAlert(SIMUtil::get_traduccion('', '', 'Correoagregadoconexitoso', LANGSESSION));
            SIMHTML::jsRedirect($script . ".php?action=edit&tabclub=parametros&tabparametro=camposacceso&id=" . $frm["IDClub"]);
            exit;
            break;

            //preguntas modulo invitados
        case "InsertarPreguntaInvitados":
            $frm = SIMUtil::varsLOG($_POST);
            $id = $dbo->insert($frm, "CampoFormularioInvitado", "IDCampoFormularioInvitado");
            SIMHTML::jsAlert(SIMUtil::get_traduccion('', '', 'RegistroExitoso', LANGSESSION));
            SIMHTML::jsRedirect($script . ".php?action=edit&tabclub=parametros&tabparametro=preguntasinvitados&id=" . $frm[IDClub]);
            exit;
            break;

        case "ModificaPreguntaInvitados":
            $frm = SIMUtil::varsLOG($_POST);
            $dbo->update($frm, "CampoFormularioInvitado", "IDCampoFormularioInvitado", $frm["IDCampoFormularioInvitado"]);
        ...
        case "EliminarPreguntaInvitados":
            $id = $dbo->query("DELETE FROM CampoFormularioInvitado WHERE IDCampoFormularioInvitado   = '" . $_GET["IDCampoFormularioInvitado"] . "' LIMIT 1");
            SIMHTML::jsAlert(SIMUtil::get_traduccion('', '', 'EliminacionExitosa', LANGSESSION));
            SIMHTML::jsRedirect($script . ".php?action=edit&tabclub=parametros&tabparametro=preguntasinvitados&id=" . $_GET["id"]);
            exit;
            break;
        ...
        case "enviarcontratos":
            ...
            SIMHTML::jsAlert(SIMUtil::get_traduccion('', '', 'CorreoEnviado', LANGSESSION));
            SIMHTML::jsRedirect($script . ".php?action=edit&id=" . SIMNet::reqInt("id"));
        ...
        case "copiar":
            ...
            SIMHTML::jsAlert(SIMUtil::get_traduccion('', '', 'Clubcopiadoconexito!', LANGSESSION));
            SIMHTML::jsRedirect($script . ".php?id=$IDClubPadre&Tipo=Padre");

            break;


        case "InsertarConfiguracionReservas":
            $frm = SIMUtil::varsLOG($_POST);
            $id = $dbo->insert($frm, "ConfiguracionReservas", "IDConfiguracionReservas");
            SIMHTML::jsAlert(SIMUtil::get_traduccion('', '', 'RegistroExitoso', LANGSESSION));
            SIMHTML::jsRedirect($script . ".php?action=edit&tabclub=parametros&tabparametro=configuracionreservas&id=" . $frm[IDClub]);
            exit;
            break;

        case "ModificarConfiguracionReservas":
            $frm = SIMUtil::varsLOG($_POST);

            $dbo->update($frm, "ConfiguracionReservas", "IDConfiguracionReservas", $frm[IDConfiguracionReservas]);
            SIMHTML::jsAlert(SIMUtil::get_traduccion('', '', 'ModificacionExitoso', LANGSESSION));
            SIMHTML::jsRedirect($script . ".php?action=edit&tabclub=parametros&tabparametro=configuracionreservas&id=" . $frm[IDClub]);
            exit;
            break;
        case "InsertarConfiguracionFacturacion":

            $frm = SIMUtil::varsLOG($_POST);
            if (isset($_FILES)) {

                $files = SIMFile::upload($_FILES["ImagenLateral"], CLUB_DIR, "IMAGE");
                if (empty($files) && !empty($_FILES["ImagenLateral"]["name"])) {
                    SIMNotify::capture("Ha ocurrido un error durante la carga de la imagen. Verifique que la imagen no contenga errores y que el tipo de archivo sea permitido.", "error");
                }

                $frm["ImagenLateral"] = $files[0]["innername"];
            }
            $id = $dbo->insert($frm, "ConfiguracionFacturacion", "IDConfiguracionFacturacion");
            SIMHTML::jsAlert(SIMUtil::get_traduccion('', '', 'RegistroExitoso', LANGSESSION));
            SIMHTML::jsRedirect($script . ".php?action=edit&tabclub=parametros&tabparametro=configuracionfacturacion&id=" . $frm[IDClub]);
            exit;
            break;

        case "ModificarConfiguracionFacturacion":
            $frm = SIMUtil::varsLOG($_POST);
            $frm = SIMUtil::varsLOG($_POST);
            if (isset($_FILES)) {
            ...
            $dbo->update($frm, "ConfiguracionFacturacion", "IDClub", $frm[IDClub]);
            SIMHTML::jsAlert(SIMUtil::get_traduccion('', '', 'ModificacionExitoso', LANGSESSION));
            SIMHTML::jsRedirect($script . ".php?action=edit&tabclub=parametros&tabparametro=configuracionfacturacion&id=" . $frm[IDClub]);
            exit;
            break;

        case "EliminaConfiguracionReservas":
            $id = $dbo->query("DELETE FROM ConfiguracionReservas WHERE IDConfiguracionReservas   = '" . $_GET[IDConfiguracionReservas] . "' LIMIT 1");
            SIMHTML::jsAlert(SIMUtil::get_traduccion('', '', 'EliminacionExitosa', LANGSESSION));
            SIMHTML::jsRedirect($script . ".php?action=edit&tabclub=parametros&tabparametro=configuracionreservas&id=" . $_GET["id"]);
            exit;
            break;

        case "InsertarCantidadReservasTipoSocio":
            $frm = SIMUtil::varsLOG($_POST);
            $id = $dbo->insert($frm, "CantidadReservasTipoSocio", "IDCantidadReservasTipoSocio");
            SIMHTML::jsAlert(SIMUtil::get_traduccion('', '', 'RegistroExitoso', LANGSESSION));
            SIMHTML::jsRedirect($script . ".php?action=edit&tabclub=parametros&tabparametro=cantidadreservastiposocio&id=" . $frm[IDClub]);
            exit;
            break;

        case "ModificarCantidadReservasTipoSocio":
            $frm = SIMUtil::varsLOG($_POST);
            $dbo->update($frm, "CantidadReservasTipoSocio", "IDCantidadReservasTipoSocio", $frm[IDCantidadReservasTipoSocio]);
            SIMHTML::jsAlert(SIMUtil::get_traduccion('', '', 'ModificacionExitoso', LANGSESSION));
            SIMHTML::jsRedirect($script . ".php?action=edit&tabclub=parametros&tabparametro=cantidadreservastiposocio&id=" . $frm[IDClub]);
            exit;
            break;

        case "EliminaCantidadReservasTipoSocio":
            $id = $dbo->query("DELETE FROM CantidadReservasTipoSocio WHERE IDCantidadReservasTipoSocio   = '" . $_GET[IDCantidadReservasTipoSocio] . "' LIMIT 1");
            SIMHTML::jsAlert(SIMUtil::get_traduccion('', '', 'EliminacionExitosa', LANGSESSION));
            SIMHTML::jsRedirect($script . ".php?action=edit&tabclub=parametros&tabparametro=cantidadreservastiposocio&id=" . $_GET["id"]);
            exit;
            break;

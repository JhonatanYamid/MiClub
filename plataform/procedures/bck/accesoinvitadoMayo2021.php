    ...
    $seImprimeCarnet = "N";
     ...
     //BUSQUEDA INVITADOS ACCESOS
       ...
       if (ctype_digit($qryString)) {
         ...
         $sql_invitacion = "Select SIE.* From SocioInvitadoEspecial SIE, Invitado I Where SIE.IDInvitado = I.IDInvitado  and ( I.NumeroDocumento = '".(int)$qryString."' or I.NumeroDocumento = '".$qryString."' ) and FechaInicio <= CURDATE() AND FechaFin >= CURDATE() and I.IDClub = '".SIMUser::get("club")."' Order By IDSocioInvitadoEspecial";
           ...
           if($total_resultados>0){
            ...
            //Verifico si la cedula es de un funcionario
             ...
             elseif($row_resp_diag["Resultado"]>=$peso_permitido){
               $alerta_diagnostico="<font color='#F14823' size='4px' ><b> Atención la persona se debe comunicar con salud ocupacional. ".$row_resp_diag["Resultado"].">".$peso_permitido." </b></font><br>";
             }
             else{
     ...
     if($total_resultados<=0 && SIMUser::get("club") != 70000 &&  SIMUser::get("club") != 9 ):
           $sql_invitacion = "Select * From Usuario Where (NumeroDocumento = '".$qryString."' or NumeroDocumento = '".(int)$qryString."' ) and IDClub = '".SIMUser::get("club")."' and Activo='S'
                              UNION
                              Select U.* From Usuario U,VehiculoUsuario VU Where U.IDUsuario=VU.IDUsuario and VU.Placa = '".$qryString."' and IDClub = '".SIMUser::get("club")."'  and Activo='S' ";
           $modo_busqueda = "DOCUMENTO";
     ...
     if($total_resultados<=0 && SIMUser::get("club") != 70000 &&  SIMUser::get("club") != 9 ):
      $sql_invitacion = "Select * From Usuario Where (CodigoUsuario = '".$qryString."' or CodigoUsuario = '".(int)$qryString."' ) and IDClub = '".SIMUser::get("club")."' and Activo='S'
                         UNION
                         Select U.* From Usuario U,VehiculoUsuario VU Where U.IDUsuario=VU.IDUsuario and VU.Placa = '".$qryString."' and IDClub = '".SIMUser::get("club")."'  and Activo='S' ";
      $modo_busqueda = "CODIGO EMPLEADO";
...
//FIN BUSQUEDA USUARIO FUNCIONARIO
     ...
     if($total_resultados<=0 || $EsSocio=="S"):
         ...
         if (ctype_digit($qryString)) {
             // si es solo numeros en un numero de documento
             $sql_invitacion = "Select * From Socio Where (NumeroDocumento = '".$qryString."' or NumeroDocumento = '".(int)$qryString."' or Accion = '".$qryString."' or NumeroDerecho = '".$qryString."'  ) and IDClub = '".SIMUser::get("club")."' and IDEstadoSocio = 1 Order by AccionPadre ASC";
             $modo_busqueda = "DOCUMENTO";
         } else {
         ...
         */
         ...
         $sql_invitacion = "SELECT * FROM Socio WHERE (Accion = '".$qryString."' or NumeroDerecho = '".$qryString."' or Predio like '".$qryString."'or Email = '".$qryString."') and IDClub = '".SIMUser::get("club")."' and IDEstadoSocio = 1
                            UNION
                            SELECT S.* FROM Socio S , Predio P  Where S.IDSocio=P.IDSocio and  P.Predio = '".$qryString."' and IDClub = '".SIMUser::get("club")."'  and AccionPadre = '' and IDEstadoSocio = 1
                            UNION
                            SELECT S.* FROM Socio S , Vehiculo V  Where S.IDSocio=V.IDSocio and  V.Placa = '".$qryString."' and IDClub = '".SIMUser::get("club")."' and IDEstadoSocio = 1
           ...
           if($total_resultados>0){
             ...
             //PARA MY se debe mostrar si lleno la exoneracion (encuestas2)
             if(SIMUser::get("club")==9){
              $MostrarExo="S";
               ...
               if($row_exo["Valor"]=="Acepto"){
                 $acepta_exo="<font color='#059C1C'><b>Exoneracion: </b> Aceptada</font><br>";
               }
               else{
                 $acepta_exo="<font color='#F14823'><b>Exoneracion: </b> No ha diligenciado</font><br>";
               }
             }
           ...
           if (empty($datos_socio["AccionPadre"]) || $datos_socio["AccionPadre"] == $datos_socio["Accion"]): // Es Cabeza
             ...
             $sql_grupo = "SELECT IDClub, IDSocio, Foto, Nombre, Apellido, Accion, AccionPadre, CodigoBarras, NumeroDocumento, ObservacionGeneral,TipoSocio,Categoria,Parentesco FROM Socio WHERE IDClub = '".SIMUser::get("club")."' and IDSocio <> '".$datos_socio["IDSocio"]."' and IDEstadoSocio = 1 " . $condicion_nucleo;
             $result_grupo = $dbo->query($sql_grupo);
           endif;
     ...
     if($seImprimeCarnet == "S") {
	       //Obtener datos del usuario logueado
	       $idUser = SIMUser::get("IDUsuario");
	  }

...
function get_data($nombrearchivo,$file,$IGNORE_FIRTS_ROW,$FIELD_TEMINATED='',$field='',$IDClub){
  ...
  for ($row = 2; $row <= $highestRow; $row++){
        ...
        if(is_numeric($CedulaAutoriza) && !empty($DocumentoInvitado) && !empty($CedulaAutoriza) && !empty($FechaIngreso) && !empty($FechaSalida) && !empty($Nombre) && !empty($Apellido) ){
                ...
                $respuesta = SIMWebService::set_autorizacion_contratista($IDClub,$row_datos_socio["IDSocio"],$TipoAutorizacion,$FechaIngreso,$FechaSalida,"1",$DocumentoInvitado,$Nombre,$Apellido,$Email,$Placa,"S",$HoraInicio,$HoraSalida,$Observaciones,$IDUsuario,$Telefono,$FechaNacimiento,$TipoSangre,$Predio,$ObservacionSocio);

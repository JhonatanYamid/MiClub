				...
				if ((int)$frm["NumeroCampos"] > 0) :
					for ($i = 1; $i <= $frm["NumeroCampos"]; $i++) :
						...
						if (!empty($frm[$nombre_campo])) :
							...
							$id_directorio_valor = $dbo->getFields("CampoDirectorioClubValor", "IDCampoDirectorioClubValor", "IDDirectorio = '" . $id . "' and IDCampoDirectorioClub = '" . $frm[$id_campo] . "'");
							if ((int)$id_directorio_valor > 0) :
								$sql_edita = "Update CampoDirectorioClubValor Set Valor = '" . $frm[$nombre_campo] . "' Where IDCampoDirectorioClubValor = '" . $id_directorio_valor . "'";
								$dbo->query($sql_edita);
							else :
								$sql_inserta = "Insert Into CampoDirectorioClubValor (IDCampoDirectorioClub,IDDirectorio, Valor)
			...
			if (!SIMNotify::capture(SIMUtil::valida($_POST, $array_valida), "error")) {
				...
				if ((int)$frm["NumeroCampos"] > 0) :
					for ($i = 1; $i <= $frm["NumeroCampos"]; $i++) :
						...
						$id_campo = "IdentificadorCampo" . $i;
						if (!empty($frm[$nombre_campo])) :
							$frm[$nombre_campo] = utf8_decode($frm[$nombre_campo]);
							//Si existe la edito si no la creo
							$id_directorio_valor = $dbo->getFields("CampoDirectorioClubValor", "IDCampoDirectorioClubValor", "IDDirectorio = '" . SIMNet::reqInt("id") . "' and IDCampoDirectorioClub = '" . $frm[$id_campo] . "'");
							if ((int)$id_directorio_valor > 0) :
								$sql_edita = "Update CampoDirectorioClubValor Set Valor = '" . $frm[$nombre_campo] . "' Where IDCampoDirectorioClubValor = '" . $id_directorio_valor . "'";
								$dbo->query($sql_edita);
							else :

...
switch ($_POST["action"]) {
	...
	case "insert":
		...
		if ($response != null && $response->success) {
			...
			SIMHTML::jsRedirect("registrocarra.php");
			exit;
		}
		...
		if (!empty($_POST["Nombre"]) && !empty($_POST["Apellido"]) && !empty($_POST["NumeroDocumento"])  && $robot_verificacion == "S") :
			...
			$id = $dbo->insert($_POST, "ActualizacionCarra", "IDActualizacionCarra");

			for ($i = 1; $i <= $_POST["TotalBenef"]; $i++) {
				if ($_POST["NombreBeneficiario" . $i]) {
					$sql_benef = "INSERT INTO ActualizacionCarraBeneficiarios 
													(IDActualizacionCarra,
													NombreBeneficiario,
			...
			for ($i = 1; $i <= $_POST["TotalMascotas"]; $i++) {
				if ($_POST["NombreMascota" . $i]) {
					...
					$sql_mascota = "INSERT INTO ActualizacionMascotasCarra
													(IDActualizacionCarra,
													NombreMascota,
													EspecieMascota,													
													Raza,													
			...
			for ($i = 1; $i <= $_POST["TotalVehiculos"]; $i++) {
				if ($_POST["PlacaVehiculo" . $i]) {
					...
					$sql_vehiculo = "INSERT INTO ActualizacionVehiculosCarra
													(IDActualizacionCarra,
													Marca,
													Color,													
													Placa,													
			...
			for ($i = 1; $i <= $_POST["TotalContactos"]; $i++) {
				if ($_POST["NombreContacto" . $i]) {
					...
					$sql_contacto = "INSERT INTO ActulizacionContactosCarra
													(IDActualizacionCarra,
													Nombre,
													Telefono1,													
													Telefono2,													

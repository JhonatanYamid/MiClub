...
jQuery(document).ready(function () {
  $(".LimitarInvitadosPorAccion").change(function () {
    if ($("input[name=LimitarInvitadosPorAccion]:checked").val() == "S") {
      $(".NumeroInvitadosPorAccion").show("slow");
    } else {
      $(".NumeroInvitadosPorAccion").hide("slow");
    }
  });
  ...
  $("#EditTicketDescuentosocios").submit(function () {
    var detalle;
    $("#SeleccionServicios").val("");
    $("#SocioServicio option").map(function (i, el) {
      detalle = $("#SeleccionServicios").val() + $(el).val() + "|||";
  ...
  $("#frmconfiguraciongeneral").submit(function () {
    var detalle;
    $("#UsuarioEmpleado").val("");
    $("#SocioInvitadoUsuario1 option").map(function (i, el) {
      detalle = $("#UsuarioEmpleado").val() + $(el).val() + "|||";

      $("#UsuarioEmpleado").val(detalle);
      //$("#InvitadoSeleccion").val($(el).val());
      //$(el).attr('selected', 'selected');
    });
    return true;
          ...
          switch (modulo) {
            ...
            case "InvitadoEvento":
              jQuery.ajax({
                type: "POST",
                data: {
                  IDInvitadoEvento: rowid,
                  Mecanismo: mecanismo,
                  OtrosCampos: lista_campo,
                  PredioIngresoSocio: PredioIngresoSocio,
                },
                dataType: "json",
                url: "includes/async/ingreso_invitadoevento.async.php",
  ...
  $("#FechaEntrega").change(function () {
    var fecha = $(this).val();
    var idRestaurante = $("#IDRestauranteDomicilio").val();
    var idClub = $("#IDClub").val();
    var version = $("#Version").val();

    $("#HoraEntrega").load(
      "includes/async/domicilioshora.async.php?fecha=" +
        fecha +
        "&idRestaurante=" +
        idRestaurante +
        "&idClub=" +
...
function preparaform() {
  ...
  $(".update_registro").click(function () {
    ...
    jQuery.ajax({
      type: "POST",
      data: { ID: ID, estado: estado, comentario: comentario, tabla: tabla },
      dataType: "json",
  ...
  $(".update_registro_horas_extras_despues_de_turno").click(function () {
    var ID = $(this).attr("id");
    var estado = $("input[name=Estado" + ID + "]:checked").val();
    var comentario = $("#ComentarioRevision" + ID).val();
    var tabla = "CheckinLaboralHorasExtras";
  ...
  $(".eliminar_registro").click(function () {
    ...
    if (tabla == "CheckinLaboralHorasExtras") {
      var idCheckinLaboralHorasExtras = $(this).attr(
        "idCheckinLaboralHorasExtras"
      );
      var type = $(this).attr("type");
      linkcheckindespuesdelturno =
        "?action=extras&id=" + idCheckinLaboralHorasExtras + "&type=" + type;
    } else {
      linkcheckindespuesdelturno = "";
    }
    if (confirm("Esta seguro que desea borrar el registro?")) {
      jQuery.ajax({
        ...
        success: function (data) {
          ...
          window.location.href =
            redireccionar + ".php " + linkcheckindespuesdelturno;
        },
      });
    }
  ...
  $(".entregar_objeto_prestado").click(function () {
    var tabla = $(this).attr("rel");
    var id = $(this).attr("id");
    var redireccionar = $(this).attr("lang");
    var idcategoria = $(this).attr("idcategoria");
    var cantidadprestada = $(this).attr("cantidadprestada");

    if (confirm("Esta seguro que desea entregar objeto prestado?")) {
      jQuery.ajax({
        type: "POST",
        data: {
          Tabla: tabla,
        ...
        dataType: "json",
        url: "includes/async/entregarobjetosprestados.async.php",

        success: function (data) {
          if (data == "ok") {
            alert("Objeto entregado");
          } else {
            alert(data);
          }
  ...
  $(".eliminar_objeto_prestado").click(function () {
    var tabla = $(this).attr("rel");
    var id = $(this).attr("id");
    var redireccionar = $(this).attr("lang");
    var idcategoria = $(this).attr("idcategoria");
    ...
    if (confirm("Esta seguro que desea borrar el registro?")) {
      jQuery.ajax({
        ...
        dataType: "json",
        url: "includes/async/eliminarobjetosprestados.async.php",
  ...
  $("#frmdisponibilidad input[name='PermiteReservaComoInvitado']").click(
    function () {
      if ($(this).val() === "S") {
        $("#div_cantidad_invitado").show();
      } else if ($(this).val() === "N") {
        $("#div_cantidad_invitado").hide();
      }
    }
  );
...
;
  ...
  */
  ...
  $(".reabrirfecha").click(function () {
    var Fecha = $(this).attr("FechaAbrir");
    var IDClub = $("#IDClub").val();

    jQuery.ajax({
      type: "POST",
      data: { Fecha: Fecha, IDClub: IDClub },
      dataType: "json",
      url: "includes/async/reabrirfecha.async.php",
      success: function (data) {
        //alert("Registro exitoso");
        n = noty({
          text: "<br><br>Fecha abierta con exito<br><br>",
          type: "success",
          dismissQueue: true,
          layout: "topCenter",
          theme: "defaultTheme",
  ...
  $("#agregar_empleado").click(function () {
    ...
    var id_elemento = "externo-" + $("#AccionInvitadoUsuario").val();
  ...
  $("#agregar_empleado1").click(function () {
    var elemento = $("#AccionInvitadoUsuario1").val();
    var id_elemento = "externo-" + $("#").val();

    if (elemento != "") {
      $("#SocioInvitadoUsuario1").append(
        '<option value="' + id_elemento + '">' + elemento + "</option>"
      );
      //$('#SocioInvitado').val($('#SocioInvitado').val()+elemento+"-externo"+"\r");
      $("#AccionInvitadoUsuario1").val("");
      alert("Empleado agregado a la lista");
    } else {
      alert("Por favor digite o seleccione un invitado");
    }
    return false;
  ...
  $("#borrar_empleado1").click(function () {
    var index_del = "";
    $("#SocioInvitadoUsuario1 option:selected").map(function (i, el) {
      index_del = $(el).val();
      $("#SocioInvitadoUsuario1 option[value='" + index_del + "']").remove();
    });
    if (index_del == "") {
      alert("Seleccione un empleado de lista");
    }
    return false;
  });
  $("#frmconfiguraciongeneral").submit(function () {

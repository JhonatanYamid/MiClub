...
switch ($oper) {
	...
	case "search":
		...
		foreach ($array_buqueda->rules as $key_busqueda => $search_object) {
			switch ($search_object->field) {
				case 'qryString':
					$where .= " AND (  Predio LIKE '" . $search_object->data . "%'  OR Torre LIKE '" . $qryString . "%' )  ";
					break;
	...
	case "searchurl":
		...
		// buscador socios
		if (count($datos) == 1) {
			$where .= " AND (   Predio LIKE '" . $qryString . "%'  OR Torre LIKE '" . $qryString . "%'  )";
		} else 
		if (count($datos) == 2) {
			$where .= " AND (   Predio LIKE '" . $qryString . "%'  OR Torre LIKE '" . $qryString . "%'   )";
		}
...
//$limit = 58;
$sql = "SELECT Socio.IDSocio, IDEstadoSocio, Nombre, Apellido, Accion, TipoSocio, Celular, Predio, Torre, NumeroDocumento, Email,PermiteReservar, CONCAT( Socio.Nombre, ' ', Socio.Apellido ) AS Socio FROM " . $table . $where . " ORDER BY $sidx $sord LIMIT " . $start . "," . $limit;
//var_dump($sql);
/* echo $sql;
exit; */

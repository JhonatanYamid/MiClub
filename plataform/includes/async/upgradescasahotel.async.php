...
$table = "UpgradesCasaHotel";
$key = "IDUpgradesCasaHotel";
$where = " WHERE " . $table . ".IDClub = '" . SIMUser::get("club") . "' ";
$script ="upgradescasahotel";
...
switch( $oper )
{
		...
		foreach( $array_buqueda->rules as $key_busqueda => $search_object )
		{
			switch ($search_object->field) {
				case 'qryString':
					...
					$where .= " AND ( NombreMejora LIKE '%" . $search_object->data . "%' )";
				break;
	...
	case "searchurl":
		...
		if( !empty( $qryString ) )
		{
			...
			$where .= " AND ( NombreMejora LIKE '%" . $qryString . "%'  )  ";
		}//end if
	break;
}

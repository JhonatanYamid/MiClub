...
switch( $oper )
{
		...
		foreach( $array_buqueda->rules as $key_busqueda => $search_object )
		{
			switch ($search_object->field) {
				case 'qryString':
					...
					$where .= " AND ( tf.Nombre LIKE '%" . $search_object->data . "%' )";
				break;
	...
	case "searchurl":
		...
		if(!empty($qryString)){
			$where .= " AND ( tf.Nombre LIKE '%" . $qryString . "%'  )  ";
		}//end if
	break;
}
...
if(!$sidx) $sidx = "tf.Nombre";
// connect to the database

$result = $dbo->query("SELECT COUNT(*) AS count FROM " . $table ." as tf ". $where . "    ");
...
$sql = "SELECT IDTipoFacturacion, tf.IDCategoriaFacturacion, cf.Nombre as categoria, tf.Nombre, tf.Activo, tf.Eliminar, tf.Editar 
		FROM TipoFacturacion as tf, CategoriaFacturacion as cf ". $where . " AND tf.IDCategoriaFacturacion = cf.IDCategoriaFacturacion ORDER BY $sidx $sord " . $str_limit;
//echo $sql;
//exit;
//var_dump($sql);

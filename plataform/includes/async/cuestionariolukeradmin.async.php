...
$table = "CuestionarioLuker";
$key = "IDCuestionarioLuker";
$where = " WHERE " . $table . ".IDClub = '" . SIMUser::get("club") . "' ";
$script = "cuestionariolukeradmin";
...
switch ($oper) {
    ...
    case "del":
        ...
        $sql_delete = "DELETE FROM CuestionarioLuker WHERE IDCuestionarioLuker = '" . $_POST["id"] . "' LIMIT 1";
        //echo "<br>";
...
while ($row = $dbo->fetchArray($result)) {
    ...
    if ($origen <> "mobile")
        $responce->rows[$i]['cell'] = array(
            ...
            "NivelDeRolAlCualPerteneceDentroDeLaOrganizacion" => $row["NivelDeRolAlCualPerteneceDentroDeLaOrganizacion"],
            "NombreDelProcesoOAreaDondeSeEncuentraSuRol" => $row["NombreDelProcesoOAreaDondeSeEncuentraSuRol"],
            "NivelDeEstratoSocieconomico" => $row["NivelDeEstratoSocieconomico"],
            "NumeroDePersonasQueComponenElNucleoFamiliarPrincipal" => $row["NumeroDePersonasQueComponenElNucleoFamiliarPrincipal"],
            "NumeroDePersonasDeLaFamiliaQueTienenUnTrabajo" => $row["NumeroDePersonasDeLaFamiliaQueTienenUnTrabajo"],

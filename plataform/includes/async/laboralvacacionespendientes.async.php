...
while ($row = $dbo->fetchArray($result)) {
    ...
    if ($row["IDSocio"] > 0) {
        $nombreSocioUsuario = $dbo->getFields("Socio", "Nombre", "IDSocio = '" . $row["IDSocio"] . "'") . " " . $dbo->getFields("Socio", "Apellido", "IDSocio = '" . $row["IDSocio"] . "'");
    } else
    if ($row["IDUsuario"] > 0) {
        $nombreSocioUsuario = $dbo->getFields("Usuario", "Nombre", "IDUsuario = '" . $row["IDUsuario"] . "'");
    }
    ...
    if ($origen <> "mobile")
        $responce->rows[$i]['cell'] = array(
            ...
            "Socio/Usuario" => $nombreSocioUsuario,
            "Periodo" => $row["Periodo"],
            "Dias" => $row["Dias"],
            "DiasCompensatorio" => $row["DiasCompensatorio"],

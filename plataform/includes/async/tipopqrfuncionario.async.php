...
$table = "TipoPqrFuncionario";
$key = "IDTipoPqr";
$where = " WHERE " . $table . ".IDClub = '" . SIMUser::get("club") . "' ";
$script ="tipopqrfuncionario";
...
switch( $oper )
{
		...
		foreach( $array_buqueda->rules as $key_busqueda => $search_object )
		{
			switch ($search_object->field) {
				case 'qryString':
					...
					$where .= " AND ( Nombre LIKE '%" . $search_object->data . "%' OR Publicar LIKE '% )";
				break;
	...
	case "searchurl":
		...
		if( !empty( $qryString ) )
		{
			...
			$where .= " AND ( Nombre LIKE '%" . $qryString . "%' OR Publicar LIKE '%" . $qryString . "%' )  ";
		}//end if
	break;
}

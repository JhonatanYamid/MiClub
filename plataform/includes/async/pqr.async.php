...
$script = "pqr";
// si viene un parametro del buscador cambio el where
if (!empty($_GET["Numero"]) || !empty($_GET["Fecha"]) || !empty($_GET["Tipo"]) || !empty($_GET["Area"]) || !empty($_GET["TipoSocio"]) || !empty($_GET["Socio"]) || !empty($_GET["Predio"]) || !empty($_GET["Descripcion"]) || !empty($_GET["IDPqrEstado"]) || !empty($_GET["Asunto"]) || !empty($_GET["Responsable"])) {

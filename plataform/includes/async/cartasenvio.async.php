...
while($row = $dbo->fetchArray($result)) {
	...
	$botones='<a class="red generar_pdf"  id='.$row[$key].' lang = '.$script.' href="views/cartasenvio/pdf.php?Modo=pdf&IDCartasBase='.$row[$key].'" alt="PDF"><i class="ace-icon fa fa-file-pdf-o bigger-130" /></a>';
	$botones.=' <a class="red enviar_mail"  id='.$row[$key].' lang = '.$script.' href="views/cartasenvio/pdf.php?Modo=email&IDCartasBase='.$row[$key].'" alt="Enviar Mail"><i class="ace-icon fa fa-envelope-o bigger-130"  /></a>';

	$btn_seleccion='<input type="checkbox" name="CartaSeleccion" class="clasecheckcarta" value="'.$row[$key].'"> ';
	...
	if( $origen <> "mobile" )
		$responce->rows[$i]['cell'] = array(
										...
										"Carta" => utf8_encode($row["Carta"]),
										"Tipo" => utf8_encode($row["Tipo"]),
										"FechaTrCr" => $row["FechaTrCr"],
										"Operacion" => $botones,
										"Seleccion" => $btn_seleccion
									);

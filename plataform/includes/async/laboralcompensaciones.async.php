...
switch ($oper) {
	...
	case "del":
		...
		$sql_delete = "DELETE FROM $table WHERE IDLaboralCompensatorio = '" . $_POST["id"] . "' LIMIT 1";
		//echo "<br>";
...
if ($FiltrarPorJefe != 'S' || $datos_usuario['IDPerfil'] == 1 || $datos_usuario['IDPerfil'] == 0) {
	$sql = "SELECT $table.*, U.Nombre AS NombreUsuario, CONCAT(S.Nombre, ' ', S.Apellido) AS NombreSocio
		FROM $table
		LEFT JOIN Usuario U ON $table.IdUsuario = U.IDUsuario
		LEFT JOIN Socio S ON $table.IDSocio = S.IDSocio

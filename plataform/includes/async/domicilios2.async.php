...
if (SIMUser::get("IDPerfil") != 0) :
    //Consulto las areas
    $sql_restaurante = "Select * From UsuarioRestaurante2 Where IDUsuario = '" . SIMUser::get("IDUsuario") . "'";
...
$key = "IDDomicilio";
$hoy = date("Y-m-d");
$where = " WHERE " . $table . ".IDClub = '" . SIMUser::get("club") . "'" . $condicion_restaurante . " AND " . $table . ".IDSocio <> '' AND date(FechaTrCr)='" . $hoy . "'";
$script = "domicilios" . $version;
    ...
    case "search":
        ...
        foreach ($array_buqueda->rules as $key_busqueda => $search_object) {
            switch ($search_object->field) {
                    ...
                    //se busca por campo en los filtros
                case 'FechaInicio':
                ...
                case 'ValorPropina':
                    $where .= " AND IF(Propina ='S',(Total*0.10),0) LIKE '%" . $search_object->data . "%'";
                    break;
                ...
                case 'Accion':
                    //busco el area
                    $sql_soc = "SELECT IDSocio FROM Socio Where Accion LIKE '%" . $search_object->data . "%'";
                    $result_soc = $dbo->query($sql_soc);
                    while ($row_soc = $dbo->fetchArray($result_soc)) :
                        $array_id_soc[] = $row_soc["IDSocio"];
                    endwhile;
                    if (count($array_id_soc) > 0) :
                        $id_soc_buscar = implode(",", $array_id_soc);
                    else :
                        $id_soc_buscar = 0;
                    endif;

                    $where .= " AND   IDSocio in (" . $id_soc_buscar . ")";

                    break;
                case 'Socio':
                    //busco el area
                    $sql_soc = "SELECT IDSocio FROM Socio Where Nombre LIKE '%" . $search_object->data . "%' or  Apellido LIKE '%" . $search_object->data . "%'";
                    $result_soc = $dbo->query($sql_soc);
                    while ($row_soc = $dbo->fetchArray($result_soc)) :
                        $array_id_soc[] = $row_soc["IDSocio"];
                    endwhile;
...
$sql_socios = "SELECT IDSocio,Accion,Nombre,Apellido FROM Socio WHERE IDClub = '" . SIMUser::get("club") . "'";
...
while ($row = $dbo->fetchArray($result)) {
    ...
    $btn_permitir = '<input type="radio" value="S" class="btnnotificadomicilio" name="permite' . $row[$key] . '" version = "' . $version . '"  IDSocio="' . $array_socios[$row["IDSocio"]]["IDSocio"] . '" ' . $checkPermiteSi . '>Notificar<br>';
    ...
    //verifica si la opcion propina esta activa y calcula su valor con el 10% del valor total. 
    $valPropina = 0;

    if ($row['Propina'] == 'S') {
        $valPropina = $row['Total'] * 0.10;
    }
    ...
    if ($row[EstadoTransaccion] == 'R') :
        $FormaDePago = "Pago Rechazado";
    else :
        if (empty($row[EstadoTransaccion]) && SIMUser::get("club") == 16)
            $FormaDePago = "Sin Pago";
        else
    ...
    if ($origen != "mobile") {
        $responce->rows[$i]['cell'] = array(
            ...
            "Accion" => "<font color='" . $color_linea . "'>" . $array_socios[$row["IDSocio"]]["Accion"] . "</font>",
            "Socio" => "<font color='" . $color_linea . "'>" . $array_socios[$row["IDSocio"]]["Nombre"] . " " . $array_socios[$row["IDSocio"]]["Apellido"] . "</font>",
            ...
            "ValorPropina" => "<font color='" . $color_linea . "'>" . $valPropina . "</font>",
            "Fecha" => "<font color='" . $color_linea . "'>" . $row["FechaTrCr"] . "</font>",
            "FormaDePago" => "<font color='" . $color_linea . "'>" .  $FormaDePago . "</font>",
            "Estado" => "<font color='" . $color_linea . "'>" . $btn_incluir . "</font>",
            "Notificar" => "<font color='" . $color_linea . "'>" .  $btn_permitir . "</font>",

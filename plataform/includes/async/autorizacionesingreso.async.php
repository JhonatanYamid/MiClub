	...
	case "search":
		...
		foreach ($array_buqueda->rules as $key_busqueda => $search_object) {
			switch ($search_object->field) {
				...
				case 'Predio':
					$where .= " AND (  I.Predio LIKE '%" . $search_object->data . "%' )  ";
					break;
...
$sql = "SELECT " . $table . ".*, CONCAT( I.Nombre, ' ', I.Apellido ) AS NombreInvitado FROM " . $table . " , Invitado I " . $where . " AND I.IDInvitado = SocioAutorizacion.IDInvitado and Ingreso = 'S' and DATE_FORMAT(FechaIngreso, '%Y-%m-%d') = '" . $fecha_inicio . "' ORDER BY $key $sord " . $str_limit;
//$sql = "SELECT " . $table . ".*, CONCAT( I.Nombre, ' ', I.Apellido ) AS NombreInvitado FROM " . $table . " , Invitado I " . $where . " AND I.IDInvitado = SocioAutorizacion.IDInvitado and Ingreso = 'S' ORDER BY $key $sord " . $str_limit;
//exit;
//var_dump($sql);

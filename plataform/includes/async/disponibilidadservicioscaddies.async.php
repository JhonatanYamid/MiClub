    ...
    case "search":
        ...
        foreach ($array_buqueda->rules as $key_busqueda => $search_object) {
            switch ($search_object->field) {
                ...
                case 'HoraDesde':
                    $where .= " WHERE ( HoraDesde LIKE '%" . $search_object->data . "%' )";

                    break;

                case 'HoraHasta':

                    $where .= " WHERE ( HoraHasta LIKE '%" . $search_object->data . "%' )";

                    break;


                case 'qryString':
                    $where .= " WHERE ( FechaTrCr LIKE '%" . $search_object->data . "%' )";
...
while ($row = $dbo->fetchArray($result)) {
    ...
    if ($origen <> "mobile")
        $responce->rows[$i]['cell'] = array(
            ...
            "ElementoServiciosCaddies" => $dbo->getFields("ElementoServiciosCaddies", "Nombre", "IDElementoServiciosCaddies = '" . $row["IDElementoServiciosCaddies"] . "'"),

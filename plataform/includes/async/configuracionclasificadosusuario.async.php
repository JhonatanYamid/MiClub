...
$table = "ConfiguracionClasificados";
$key = "IDConfiguracionClasificados";
$where = " WHERE " . $table . ".IDClub = '" . SIMUser::get("club") . "' AND ConfiguracionPara='U' ";
$script = "configuracionclasificadosusuario";
        ...
        foreach ($array_buqueda->rules as $key_busqueda => $search_object) {
            switch ($search_object->field) {
                case 'qryString':
                    ...
                    break;
                case 'PublicarClasificadosAutomaticamente':

                    $where .= " AND ( PublicarClasificadosAutomaticamente LIKE '%" . $search_object->data . "%' )  ";
                    break;
                case 'Activo':

                    $where .= " AND (  Activo LIKE '%" . $search_object->data . "%' )  ";
                    break;
...
if (!$sidx) {
    $sidx = "IDConfiguracionClasificados";
}
...
$sql = "SELECT " . $table . ".* FROM " . $table  . $where  . " ORDER BY $table.$sidx $sord LIMIT " . $start . "," . $limit;
...
while ($row = $dbo->fetchArray($result)) {
    ...
    if ($origen <> "mobile") {
        $responce->rows[$i]['cell'] = array(
            ...
            "PublicarClasificadosAutomaticamente" => $row["PublicarClasificadosAutomaticamente"],
            "Activo" => $row["Activo"],

...
$table = "TipoTarjetaRotativa ";
$key = "IDTipoTarjetaRotativa";
$where = " WHERE TipoTarjetaRotativa.IDClub = '" . SIMUser::get("club") . "' ";
$script = "tipotarjetarotativa";
...
while ($row = $dbo->fetchArray($result)) {
    ...
    if ($origen <> "mobile")
        $responce->rows[$i]['cell'] = array(
            ...
            "Eliminar" => '<a class="red eliminar_registro" rel="TipoTarjetaRotativa" id=' . $row[$key] . ' lang = ' . $script . ' href="#"><i class="ace-icon fa fa-trash-o bigger-130"/></a>'
        );
    else
        $responce->rows[$i]['cell'] = array(
            $key => $row[$key],

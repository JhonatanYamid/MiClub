...
$table = "CampoBicicleta";
$key = "IDCampoBicicleta";
$where = " WHERE IDClub = $IDClub ";
$script = "campobicicleta";
...
switch( $oper )
{
	...
	case "del":
		...
		$sql_delete = "DELETE FROM CampoBicicleta WHERE IDCampoBicicleta = '" . $_POST["id"] . "' LIMIT 1";
		//echo "<br>";
		...
		foreach( $array_buqueda->rules as $key_busqueda => $search_object )
		{
			switch ($search_object->field) {
				case 'qryString':
					...
					$where .= " AND ( EtiquetaCampo LIKE '%" . $search_object->data . "%' )";
				break;
	...
	case "searchurl":
		...
		if(!empty($qryString)){
			$where .= " AND ( EtiquetaCampo LIKE '%" . $qryString . "%'  )  ";
		}//end if
	break;
}
...
$sql = "SELECT IDCampoBicicleta, EtiquetaCampo, TipoCampo, Obligatorio, Orden, Activo  FROM " . $table . $where . " ORDER BY $sidx $sord " . $str_limit;
//echo $sql;
//exit;
// var_dump($sql);
...
while($row = $dbo->fetchArray($result)) {
	...
	if( $origen <> "mobile" )
		$responce->rows[$i]['cell'] = array( 
			...
			"EtiquetaCampo" => $row["EtiquetaCampo"],	

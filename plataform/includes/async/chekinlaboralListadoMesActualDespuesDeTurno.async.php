...
$where = " WHERE  " . $table . ".IDClub = '" . SIMUser::get("club") . "' AND ID" . $type . "=" . $id . " AND MONTH(FechaMovimientoEntrada)='" . $mesActual . "'";
...
while ($row = $dbo->fetchArray($result)) {
    ...
    if ($row["IDSocio"] > 0) {
        ...
        $TipoTiempoEntrada = SIMUtil::tiempo_laboral_despues_de_turno($row['FechaMovimientoEntrada'], $row['FechaMovimientoSalida']);
    ...
    $TiempoExtraEntrada = SIMUtil::Calcular_diferencia_horas($row['FechaMovimientoEntrada'], $row['FechaMovimientoSalida'], "", "ExtrasDespuesDeTurno");
    ...
    if ($origen <> "mobile")
        $responce->rows[$i]['cell'] = array(
            ...
            "Guardar" => '<i class="ace-icon fa fa-save bigger-130 blue update_registro_horas_extras_despues_de_turno "  rel=' . $table . ' id=' . $row[$key] . ' lang = ' . $script . '/>',
            "FechaMovimientoSalida" => $row["FechaMovimientoSalida"],
            //"Editar" => '<a class="green fancybox" href="' . $script . '.php?action=edit&idchekindespuesdeturno=' . $row[$key] . '&id=' . $id . '&type=' . $type . '' . '" data-fancybox-type="iframe"><i class="ace-icon fa fa-pencil bigger-130"/></a>',
            "Editar" => '<a class="green " href="' . $script . '.php?action=edit&idchekindespuesdeturno=' . $row[$key] . '&id=' . $id . '&type=' . $type . '' . '" ><i class="ace-icon fa fa-pencil bigger-130"/></a>',
            "Eliminar" => '<a class="red eliminar_registro" rel=' . $table . ' id=' . $row[$key] . ' lang =chekinlaboral' . ' idCheckinLaboralHorasExtras=' . $id . ' type=' . $type . ' href="#"><i class="ace-icon fa fa-trash-o bigger-130"/></a>'

...
$table = "RegistroContacto";
$key = "IDRegistroContacto";
$where = " WHERE " . $table . ".IDClub = '" . SIMUser::get("club") . "' ";
$script ="registrocontacto";
...
while($row = $dbo->fetchArray($result)) {
	...
	if($row["IDSocio"]>0){
		$datos_persona = $dbo->fetchAll( "Socio", " IDSocio = '" . $row["IDSocio"] . "' ", "array" );
		$Nombre=$datos_persona["Nombre"] . " " . $datos_persona["Apellido"];
	}
	elseif($row["IDUsuario"]>0){
	 $datos_persona = $dbo->fetchAll( "Usuario", " IDUsuario = '" . $row["IDUsuario"] . "' ", "array" );

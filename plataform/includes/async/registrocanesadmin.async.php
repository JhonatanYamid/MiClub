...
switch ($oper) {
    ...
    case "del":
        ...
        $sql_delete = "DELETE FROM Mascota WHERE IDMascota = '" . $_POST["id"] . "' LIMIT 1";
        //echo "<br>";
    ...
    case "search":
        ...
        foreach ($array_buqueda->rules as $key_busqueda => $search_object) {
            switch ($search_object->field) {
                ...
                case 'FechaDeIngreso':
                    $where .= " AND ( FechaDeIngreso LIKE '%" . $search_object->data . "%' )";

                    break;
                case 'FechaFin':
                    $where .= " AND ( FechaFin LIKE '%" . $search_object->data . "%' )";
                ...
                case 'Nombre':
                    ...
                    break;
                case 'Raza':
                    $where .= " AND ( Raza LIKE '%" . $search_object->data . "%' )";

                    break;
                case 'AQuienPertenece':
                    $where .= " AND ( AQuienPertenece LIKE '%" . $search_object->data . "%' )";
...
while ($row = $dbo->fetchArray($result)) {
    ...
    if ($origen <> "mobile")
        $responce->rows[$i]['cell'] = array(
            ...
            "Nombre" => $row["Nombre"],
            "Raza" => $row["Raza"],
            "FechaDeIngreso" => $row["FechaDeIngreso"],
            "FechaFin" => $row["FechaFin"],
            "AQuienPertenece" => $row["AQuienPertenece"],

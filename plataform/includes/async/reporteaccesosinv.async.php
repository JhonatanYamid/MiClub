		...
		foreach ($array_buqueda->rules as $key_busqueda => $search_object) {
			switch ($search_object->field) {
				...
				case 'Tipo':
					...
					$where .= " AND (  SocioInvitadoEspecial.TipoAutorizacion LIKE '%" . $search_object->data . "%' )  ";
					break;
...
$result = $dbo->query("SELECT COUNT(*) AS count  FROM " . $table . " , Invitado I, Socio S " . $where . " AND SocioInvitadoEspecial.IDSocio = S.IDSocio AND I.IDInvitado = SocioInvitadoEspecial.IDInvitado");
...
//$sql = "SELECT " . $table . ".*, CONCAT( Invitado.Nombre, ' ', Invitado.Apellido ) AS Socio FROM " . $table . " , Socio " . $where . " AND Invitado.IDClub = '" . SIMUser::get("club")  . "' AND Invitado.IDSocio = SocioInvitadoEspecial.IDSocio ORDER BY $key $sord " . $str_limit;
$sql = "SELECT " . $table . ".*, CONCAT( I.Nombre, ' ', I.Apellido ) AS NombreInvitado FROM " . $table . " , Invitado I, Socio S " . $where . " AND SocioInvitadoEspecial.IDSocio = S.IDSocio AND I.IDInvitado = SocioInvitadoEspecial.IDInvitado ORDER BY $key $sord LIMIT " . $start . "," . $limit;

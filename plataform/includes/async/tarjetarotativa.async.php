...
$table = "TarjetaRotativa, Socio";
$key = "IDTarjetaRotativa";
$where = " WHERE TarjetaRotativa.IDSocio=Socio.IDSocio AND Socio.IDClub = '" . SIMUser::get("club") . "' ";
...
$sql = "SELECT TarjetaRotativa.*,Socio.Nombre,Socio.Apellido FROM {$table} {$where} ORDER BY $sidx $sord " . $str_limit;
// exit;
//var_dump($sql);
...
while ($row = $dbo->fetchArray($result)) {
    ...
    if ($origen <> "mobile")
        $responce->rows[$i]['cell'] = array(
            ...
            "Cupos" => $row["Cupos"],
            "Eliminar" => '<a class="red eliminar_registro" rel="TarjetaRotativa" id=' . $row[$key] . ' lang = ' . $script . ' href="#"><i class="ace-icon fa fa-trash-o bigger-130"/></a>'
        );
    else
        $responce->rows[$i]['cell'] = array(
            $key => $row[$key],

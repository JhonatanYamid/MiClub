...
$table = "NoticiaComentario";
$key = "IDNoticiaComentario";
$where = " WHERE " . $table . ".IDNoticia = '" . $_GET["IDNoticia"] . "'";
$script ="comentarios";
...
$sql_asociada="SELECT  IDNoticiaComentario, Publicar FROM NoticiaComentario WHERE IDNoticia = '".$_GET["IDNoticia"]."'";
$r_asociada=$dbo->query($sql_asociada);
while($row_asociada=$dbo->fetchArray($r_asociada))
{
	if($row_asociada["Publicar"]=="S")
		$array_id_votacion[]=$row_asociada["IDNoticiaComentario"];
}
...
while($row = $dbo->fetchArray($result)) {
	...
	if(in_array($row["IDNoticiaComentario"],$array_id_votacion)){
		$checksi="checked";
		$checkno="";
	}
	else
	...
	$btn_incluir='	<input type="radio" value="S" class="btnpublicarcomentario" name="Publicar'.$row[$key].'"  idnoticiacomentario="'.$row[$key].'" '.$checksi.'>Si	<input type="radio" value="N" class="btnpublicarcomentario" name="Publicar'.$row[$key].'"  idnoticiacomentario="'.$row[$key].'" '.$checkno.'>No';

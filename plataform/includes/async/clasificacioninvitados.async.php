...
$table = "ClasificacionInvitado";
$key = "IDClasificacionInvitado";
$where = " WHERE ".$table.".IDTipoInvitado=TipoInvitado.IDTipoInvitado and TipoInvitado.IDClub = '" . SIMUser::get("club") . "' ";
$script ="clasificacioninvitados";
		...
		foreach( $array_buqueda->rules as $key_busqueda => $search_object )
		{
			...
			switch ($search_object->field) {
				...
				case 'Nombre':
					...
					$where .= " AND ( ClasificacionInvitado.Nombre LIKE '%" . $search_object->data . "%' )";
				break;
				
				case 'Tipo':
					
					$where .= " AND ( TipoInvitado.Nombre LIKE '%" . $search_object->data . "%' )";
				break;
...
$result = $dbo->query("SELECT COUNT(*) AS count FROM " . $table . ",TipoInvitado" . $where . "    ");
...
$sql = "SELECT " . $table . ".*,TipoInvitado.Nombre as NombreTipo FROM " . $table . ",TipoInvitado" . $where . " ORDER BY $sidx $sord LIMIT " . $start . ",".$limit;
//exit;
//var_dump($sql);

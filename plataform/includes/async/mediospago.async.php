...
switch( $oper )
{
	...
	case "del":
		...
		$sql_delete = "DELETE FROM MediosPago WHERE IDMediosPago = '" . $_POST["id"] . "' LIMIT 1";
		//echo "<br>";
		...
		foreach( $array_buqueda->rules as $key_busqueda => $search_object )
		{
			switch ($search_object->field) {
				case 'qryString':
					...
					$where .= " AND ( m.Nombre LIKE '%" . $search_object->data . "%' )";
				break;
...
$sql = "SELECT m.IDMediosPago, m.Nombre, d.Nombre as NombreDian, m.Activo 
		FROM MediosPago as m, MediosPagoDian as d ".
		$where . " AND m.IDMediosPagoDian = d.IDMediosPagoDian ORDER BY $sidx $sord " . $str_limit;
//echo $sql;
//exit;
//var_dump($sql);

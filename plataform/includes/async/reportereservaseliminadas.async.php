			...
			switch ($search_object->field) {
				...
				default:
					if($search_object->field == "Servicio"):
						$id_servicio_maestro = $dbo->getFields( "ServicioClub" , "IDServicioMaestro" , "TituloServicio LIKE '%" . $search_object->data . "%' AND IDClub= '".SIMUser::get("club")."' AND Activo = 'S'");
						...
						if($search_object->field=="UsuarioTrCr"):
							$identificador_tabla = "ReservaGeneralEliminada.";
						endif;
...
$where .= " and ReservaGeneralEliminada.IDSocio = S.IDSocio ";
$sql = "SELECT ReservaGeneralEliminada.*,  S.IDSocio, S.Nombre, S.Apellido, S.Accion, S.TipoSocio, CONCAT( S.Nombre, ' ', S.Apellido ) AS Socio FROM " . $table . ", Socio S " . $where . " ORDER BY IDReservaGeneral DESC LIMIT " . $limit;
...
$sql_servicio="SELECT * FROM Servicio Where 1";
...
$sql_servicio_c="SELECT * FROM ServicioClub Where 1 ";

...
$table = "ConfiguracionPqr";
$key = "IDConfiguracionPqr";
$where = " WHERE " . $table . ".IDClub = '" . SIMUser::get("club") . "' ";
$script = "configuracionpqr";
...
switch ($oper) {
    ...
    case "del":
        ...
        $sql_delete = "DELETE FROM ConfiguracionPqr WHERE IDConfiguracionPqr = '" . $_POST["id"] . "' LIMIT 1";
        //echo "<br>";
        ...
        foreach ($array_buqueda->rules as $key_busqueda => $search_object) {
            switch ($search_object->field) {
                ...
                case 'Nombre':
                    ...
                    break;
                case 'TextoIntroPqr':
                    $where .= " AND (  TextoIntroPqr LIKE '%" . $search_object->data . "%' )";
                    break;
                case 'Activo':
...
while ($row = $dbo->fetchArray($result)) {
    ...
    if ($origen <> "mobile")
        $responce->rows[$i]['cell'] = array(
            ...
            "Nombre" => $row["Nombre"],
            "TextoIntroPqr" => $row["TextoIntroPqr"],
            "Activo" => $row["Activo"],

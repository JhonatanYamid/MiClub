...
$table = "ConfiguracionTalegas";
$key = "IDConfiguracionTalegas";
$where = " WHERE " . $table . ".IDClub = '" . SIMUser::get("club") . "' ";
$script = "configuraciontalegas";
...
switch ($oper) {
    ...
    case "del":
        ...
        $sql_delete = "DELETE FROM ConfiguracionTalegas WHERE IDConfiguracionTalegas = '" . $_POST["id"] . "' LIMIT 1";
        //echo "<br>";
        $qry_delete = $dbo->query($sql_delete);

        $_GET["page"] = 1;
        $_GET['rows'] = 100;
        $_GET['sidx'] = "LabelBotonSolicitarTalega ASC";
        $_GET['sord'] = "ASC";
        ...
        foreach ($array_buqueda->rules as $key_busqueda => $search_object) {
            switch ($search_object->field) {
                case 'qryString':
                    ...
                    $where .= " AND ( LabelBotonSolicitarTalega LIKE '%" . $search_object->data . "%' )";
                    break;
    ...
    case "searchurl":
        ...
        if (!empty($qryString)) {
            ...
            $where .= " AND ( LabelBotonSolicitarTalega LIKE '%" . $qryString . "%'  )  ";
        } //end if
        break;
}
...
if (!$sidx) $sidx = "LabelBotonSolicitarTalega";
...
while ($row = $dbo->fetchArray($result)) {
    ...
    if ($origen <> "mobile")
        $responce->rows[$i]['cell'] = array(
            ...
            "LabelBotonSolicitarTalega" => $row["LabelBotonSolicitarTalega"],
            "LabelLugarSolicitarTalega" => $row["LabelLugarSolicitarTalega"],
            "LabelFechaSolicitarTalega" => $row["LabelFechaSolicitarTalega"],
            "Activo" => $row["Activo"],

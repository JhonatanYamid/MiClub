...
switch ($oper) {
    ...
    case "searchurl":
        ...
        if (!empty($qryString)) {
            ...
            $where .= " AND ( Socio.Nombre LIKE '%" . $qryString . "%' OR SocioInvitado.Nombre LIKE '%" . $qryString . "%' OR Socio.Apellido LIKE '%" . $qryString . "%' OR Socio.NumeroDocumento LIKE '%" . $qryString . "%'  OR SocioInvitado.NumeroDocumento LIKE '%" . $qryString . "%' OR Accion LIKE '%" . $qryString . "%' )  ";
        } //end if
        break;
}
...
if (!empty($_GET["FechaInicio"]) && !empty($_GET["FechaFin"])) {
    $where_a .= " AND ( SocioInvitado.FechaIngreso >= '" . $_GET["FechaInicio"] . " 00:00:00' and SocioInvitado.FechaIngreso <= '" . $_GET["FechaFin"] . " 23:59:59') ";
}
...
$result = $dbo->query("SELECT COUNT(*) AS count FROM " . $table . $where . " " . $condicion_fecha . "    " . $where_a . "  ");
...
$sql = "SELECT " . $table . ".*, Socio.IDSocio, Accion, TipoSocio, Socio.NumeroDocumento as DocumentoSocio, Email, CONCAT( Socio.Nombre, ' ', Socio.Apellido ) AS Socio FROM " . $table . ",Socio" . $where . " and Socio.IDSocio=SocioInvitado.IDSocio " . $condicion_fecha . "   "  . $where_a . " ORDER BY $sidx $sord LIMIT " . $start . "," . $limit;
//exit;
//var_dump($sql);
    ...
    while ($row = $dbo->fetchArray($result)) {
        ...
        if ($origen != "mobile") {
            $responce->rows[$i]['cell'] = array(
                ...
                "NombreInvitado" => "<font color='$color_fila'>" . utf8_encode($row["Nombre"]) . "</font>",

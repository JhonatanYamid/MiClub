...
$table = "ConfiguracionCanjes";
$key = "IDConfiguracionCanjes";
$where = " WHERE " . $table . ".IDClub = '" . SIMUser::get("club") . "' ";
$script = "configuracioncanjes";
...
switch ($oper) {
    ...
    case "del":
        ...
        $sql_delete = "DELETE FROM ConfiguracionCanjes WHERE IDConfiguracionCanjes = '" . $_POST["id"] . "' LIMIT 1";
        //echo "<br>";
    ...
    case "search":
        ...
        foreach ($array_buqueda->rules as $key_busqueda => $search_object) {
            switch ($search_object->field) {
                ...
                case 'PermitePais':
                    $where .= " AND ( PermitePais LIKE '%" . $search_object->data . "%' )";

                    break;

                case 'PermiteCiudad':
                    $where .= " AND ( PermiteCiudad LIKE '%" . $search_object->data . "%' )";

                    break;


                case 'LabelBotonPais':
                    $where .= " AND ( LabelBotonPais LIKE '%" . $search_object->data . "%' )";

                    break;
                case 'LabelBotonCiudad':
                    $where .= " AND ( LabelBotonCiudad LIKE '%" . $search_object->data . "%' )";
...
while ($row = $dbo->fetchArray($result)) {
    ...
    if ($origen <> "mobile")
        $responce->rows[$i]['cell'] = array(
            ...
            "PermitePais" => $row["PermitePais"],
            "PermiteCiudad" => $row["PermiteCiudad"],
            "LabelBotonPais" => $row["LabelBotonPais"],
            "LabelBotonCiudad" => $row["LabelBotonCiudad"],
            "Activo" => $Activo,

		...
		foreach( $array_buqueda->rules as $key_busqueda => $search_object )
		{
			switch ($search_object->field) {
				...
				case 'TotalRegistrado':
					$where .= " AND(SELECT COUNT(IDEventoRegistro2) FROM EventoRegistro2 WHERE IDEvento2 = Evento2.IDEvento2) = ".$search_object->data;
				break;
...
while($row = $dbo->fetchArray($result)) {
	...
	$total = $dbo->getFields("EventoRegistro2", "COUNT(IDEventoRegistro2)", "IDEvento2 = '" . $row[$key] . "'"); 
	...
	if( $origen <> "mobile" )
		...
		$responce->rows[$i]['cell'] = array(
										...
										"Seccion" => $dbo->getFields( "SeccionEvento2" , "Nombre" , "IDSeccionEvento2 = '" . $row["IDSeccionEvento2"] . "'" ),
										"Titular" => $row["Titular"],

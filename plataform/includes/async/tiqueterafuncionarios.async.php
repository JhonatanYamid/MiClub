...
switch ($oper) {
    case "autofill":
        $sql = "SELECT CantidadEntradas FROM TaloneraFunc WHERE IDTaloneraFunc = ".$_GET['IDTaloneraFunc'];
        $result = $dbo->query($sql);
        echo $dbo->fetch($result)['CantidadEntradas'];
        exit;
        break;
...
while ($row = $dbo->fetchArray($result)) {
    ...
    if ($origen <> "mobile")
        $responce->rows[$i]['cell'] = array(
            ...
            "Nombre" => $row["Nombre"],
            "IDTaloneraFunc" => $row["IDTaloneraFunc"],

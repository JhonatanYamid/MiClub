...
switch ($oper) {
        ...
        foreach ($array_buqueda->rules as $key_busqueda => $search_object) {
            switch ($search_object->field) {
                case 'qryString':
                    ...
                    $where .= " AND ( Nombre LIKE '%" . $search_object->data . "%' OR Descripcion LIKE '% )";
                    break;
    ...
    case "searchurl":
        ...
        if (!empty($qryString)) {
            ...
            $where .= " AND ( Nombre LIKE '%" . $qryString . "%' OR Descripcion LIKE '%" . $qryString . "%' )  ";
        } //end if
        break;
}
...
while ($row = $dbo->fetchArray($result)) {
    ...
    //Enviados
    $sql_enviado = "Select IDLogNotificacion, Leido From LogNotificacion Where IDNotificacionesGenerales = '" . $row[$key] . "'";
    $result_enviado = $dbo->query($sql_enviado);
    $total_enviados = $dbo->rows($result_enviado);
    $row_enviados = $dbo->fetchArray($result_enviado);

    if($row_enviados[Leido] == 'S')
        $leido = 'Si';
    else
        $leido = '';
    ...
    if ($origen != "mobile") {
        $responce->rows[$i]['cell'] = array(
            ...
            "TituloMensaje" => $row["TituloMensaje"],
            "Mensaje" => $row["Mensaje"],

...
$table = "SocioPagosPendientesPoloClub";
$key = "IDSocioPagosPendientesPoloClub";
$where = " ";
...
switch ($oper) {
    ...
    case "del":
        ...
        $sql_delete = "DELETE FROM SocioPagosPendientesPoloClub WHERE IDSocioPagosPendientesPoloClub = '" . $_POST["id"] . "' LIMIT 1";
        //echo "<br>";
...
$sql = "SELECT " . $table . ".* FROM " . $table . $where . " GROUP BY Fecha ORDER BY $sidx $sord " . $str_limit;
...
while ($row = $dbo->fetchArray($result)) {
    ...
    if ($origen <> "mobile")
        $responce->rows[$i]['cell'] = array(
            ...
            "Eliminar" => '<a class="red eliminar_registro" rel=' . $table . ' id=' . $row['Fecha'] . ' lang = ' . $script . ' href="#"><i class="ace-icon fa fa-trash-o bigger-130"/></a>'
        );
    else
        $responce->rows[$i]['cell'] = array(
            $key => $row[$key],

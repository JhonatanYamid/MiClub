...
$table = "TipoSocio ";
$key = "IDTipoSocio";
$where = " WHERE TipoSocio.IDTipoSocio=ClubTipoSocio.IDTipoSocio AND ClubTipoSocio.IDClub = '" . SIMUser::get("club") . "' ";
$script = "tiposocio";
...
switch ($oper) {
    ...
    case "del":
        ...
        $sql_delete = "DELETE FROM ClubTipoSocio WHERE IDClub = '" . SIMUser::get("club") . "' AND IDTipoSocio = '" . $_POST["id"] . "' LIMIT 1";
        //echo "<br>";
...
$sql = "SELECT " . $table .  ".* FROM " . $table . ",ClubTipoSocio" . $where . " ORDER BY $sidx $sord " . $str_limit;
//exit;
//var_dump($sql);
...
while ($row = $dbo->fetchArray($result)) {
    ...
    if ($origen <> "mobile")
        $responce->rows[$i]['cell'] = array(
            ...
            "Eliminar" => '<a class="red eliminar_registro" rel="ClubTipoSocio" id=' . $row[$key] . ' lang = ' . $script . ' href="#"><i class="ace-icon fa fa-trash-o bigger-130"/></a>'
        );
    else
        $responce->rows[$i]['cell'] = array(
            $key => $row[$key],

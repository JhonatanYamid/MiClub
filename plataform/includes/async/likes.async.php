	...
	case "search":
		...
		foreach( $array_buqueda->rules as $key_busqueda => $search_object )
		{
			switch ($search_object->field) {				
				...
				default:
					$where .=  $array_buqueda->groupOp . "  NoticiaLike." . $search_object->field . " LIKE '%" . $search_object->data . "%' ";
				break;
			}
...
$sql = "SELECT " . $table . ".* FROM " . $table . $where . " ORDER BY FechaTrCr DESC LIMIT " . $start . ",".$limit;
/* echo $sql; */
//exit;
//var_dump($sql);
...
while($row = $dbo->fetchArray($result)) 
{
	...
	if( $origen <> "mobile" )
		...
		$responce->rows[$i]['cell'] = array(
										$key => $row[$key],
										"Like" => "<i class='green ace-icon fa fa-thumbs-up bigger-120'></i>",

...
$table = "ConfiguracionDomiciliario";
$key = "IDConfiguracionDomiciliario";
$where = " WHERE " . $table . ".IDClub = '" . SIMUser::get("club") . "' ";
$script = "configuraciondomiciliario";
...
switch ($oper) {
	...
	case "del":
		...
		$sql_delete = "DELETE FROM ConfiguracionDomiciliario WHERE IDConfiguracionDomiciliario = '" . $_POST["id"] . "' LIMIT 1";
		//echo "<br>";
		...
		foreach ($array_buqueda->rules as $key_busqueda => $search_object) {
			switch ($search_object->field) {
				case 'qryString':
					...
					$where .= " AND ( LabelDocumento LIKE '%" . $search_object->data . "%' )";
					break;
...
$sql = "SELECT * FROM ConfiguracionDomiciliario" . $where . "  " . $str_limit;
//exit;
//var_dump($sql);
...
while ($row = $dbo->fetchArray($result)) {
	...
	if ($origen <> "mobile")
		$responce->rows[$i]['cell'] = array(
			...
			"Nombre_Documento" => $row["LabelDocumento"],

			"Nombre_Fecha_Ingreso" => $row["LabelFechaIngreso"],

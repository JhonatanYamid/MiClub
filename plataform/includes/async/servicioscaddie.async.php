...
$table = "ServiciosCaddie";
$key = "IDServiciosCaddie";
$where = " WHERE " . $table . ".IDClub = '" . SIMUser::get("club") . "' ";
$script = "servicioscaddie";
    ...
    case "search":
        ...
        foreach ($array_buqueda->rules as $key_busqueda => $search_object) {
            switch ($search_object->field) {
                ...
                case 'LabelSeleccionarElemento':
                    $where .= " AND ( LabelSeleccionarElemento LIKE '%" . $search_object->data . "%' )";

                    break;
                case 'NumeroDiasMostrar':
                    $where .= " AND ( NumeroDiasMostrar LIKE '%" . $search_object->data . "%' )";

                    break;
                case 'HoraApertura':
                    $where .= " AND ( HoraApertura LIKE '%" . $search_object->data . "%' )";
...
while ($row = $dbo->fetchArray($result)) {
    ...
    if ($origen <> "mobile")
        $responce->rows[$i]['cell'] = array(
            ...
            "Nombre" => $row["Nombre"],
            "LabelSeleccionarElemento" => $row["LabelSeleccionarElemento"],
            "NumeroDiasMostrar" => $row["NumeroDiasMostrar"],

    ...
    case "search":
        ...
        foreach ($array_buqueda->rules as $key_busqueda => $search_object) {
            switch ($search_object->field) {
                ...
                case 'Descripcion':
                    $where .= " AND ( Descripcion LIKE '%" . $search_object->data . "%' )";

                    break;

                case 'HoraDesde':

                    $where .= " AND ( HoraDesde LIKE '%" . $search_object->data  . "%' )";

                    break;
                case 'HoraHasta':

                    $where .= " AND ( HoraHasta LIKE '%" . $search_object->data  . "%' )";

                    break;
                case 'FechaDesde':

                    $where .= " AND ( FechaDesde LIKE '%" . $search_object->data  . "%' )";

                    break;
                case 'FechaHasta':

                    $where .= " AND ( FechaHasta LIKE '%" . $search_object->data  . "%' )";

                    break;
                case 'Categoria':

                    $where .= " AND ( Categoria LIKE '%" . $search_object->data  . "%' )";
...
while ($row = $dbo->fetchArray($result)) {
    ...
    if ($origen <> "mobile")
        $responce->rows[$i]['cell'] = array(
            ...
            "Usuario" => $dbo->getFields("Usuario", "Nombre", "IDUsuario = '" . $row["IDUsuario"] . "'") . " " . $dbo->getFields("Usuario", "Apellido", "IDUsuario = '" . $row["IDUsuario"] . "'"),

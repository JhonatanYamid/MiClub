...
switch ($oper) {
	...
	case "searchurl":
		...
		// buscador socios
		if (count($datos) == 1) {
			$where .= " AND ( Socio.Nombre LIKE '%" . $datos[0] . "%' OR Socio.Apellido LIKE '%" . $datos[0] . "%' OR Socio.NumeroDocumento LIKE '" . $qryString . "%' OR Accion LIKE '" . $qryString . "%' OR Email LIKE '" . $qryString . "%' OR Predio LIKE '" . $qryString . "%')";
		} else 
		if (count($datos) == 2) {
			$where .= " AND ( Socio.Nombre LIKE '%" . $datos[0] . "%' AND Socio.Apellido LIKE '%" . $datos[1] . "%' OR Socio.NumeroDocumento LIKE '" . $qryString . "%' OR Accion LIKE '" . $qryString . "%' OR Email LIKE '" . $qryString . "%' OR Predio LIKE '" . $qryString . "%')";
		}
...
//$limit = 58;
$sql = "SELECT Socio.IDSocio, IDEstadoSocio, Nombre, Apellido, Accion, TipoSocio, Email,PermiteReservar, CONCAT( Socio.Nombre, ' ', Socio.Apellido ) AS Socio FROM " . $table . $where . " ORDER BY $sidx $sord LIMIT " . $start . "," . $limit;
//var_dump($sql);
/* echo $sql;
exit; */

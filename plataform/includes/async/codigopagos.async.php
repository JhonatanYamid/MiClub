...
$table = "ClubCodigoPago";
$key = "IDClubCodigoPago";
...
switch( $oper )
{
	case "search":
		...
		foreach( $array_buqueda->rules as $key_busqueda => $search_object )
		{
			switch ($search_object->field) {
				case 'Socio':	
					$Tabla_join=" ,Socio S";
					$Join = " and S.IDSocio=ClubCodigoPago.IDSocio ";
	...
	case "searchurl":
		...
		if( !empty( $qryString ) )
		{
			...
			$where .= " AND ( Codigo LIKE '%" . $qryString . "%'  )  ";
		}//end if
	break;
}
...
$sql = "SELECT " . $table . ".* FROM " . $table . $Tabla_join . $where . $Join . " ORDER BY $sidx $sord " . $str_limit;
//exit;
//var_dump($sql);
...
while($row = $dbo->fetchArray($result)) {
	...
	if( $origen <> "mobile" )
		$responce->rows[$i]['cell'] = array( 
										...
										"CreadoPor" => $row["UsuarioTrCr"],										
										"FechaCreacion" => $row["FechaTrCr"],
										"FechaRedimido" => $row["FechaTrEd"],
										"Eliminar" => ''

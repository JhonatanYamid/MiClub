...
// echo "SELECT COUNT(SocioInvitadoEspecial.IDSocioInvitadoEspecial) FROM " . $table . " , Invitado I, Socio S WHERE SocioInvitadoEspecial.IDSocio = S.IDSocio AND I.IDInvitado = SocioInvitadoEspecial.IDInvitado AND " . $where . $condicion_id_invitacion;
// $result = $dbo->query("SELECT COUNT(SocioInvitadoEspecial.IDSocioInvitadoEspecial) FROM " . $table . " , Invitado I, Socio S WHERE SocioInvitadoEspecial.IDSocio = S.IDSocio AND I.IDInvitado = SocioInvitadoEspecial.IDInvitado AND " . $where . $condicion_id_invitacion);
$result = $dbo->query("SELECT COUNT(SocioInvitadoEspecial.IDSocioInvitadoEspecial) FROM " . $table . " , Invitado I, Socio S WHERE SocioInvitadoEspecial.IDSocio = S.IDSocio AND I.IDInvitado = SocioInvitadoEspecial.IDInvitado AND " . $where . " AND ( SocioInvitadoEspecial.Ingreso !='S' OR DATE(SocioInvitadoEspecial.FechaIngreso) != CURDATE()) AND (DATE(SocioInvitadoEspecial.FechaSalida)!= CURDATE() OR SocioInvitadoEspecial.FechaSalida is null) " . $condicion_id_invitacion);
...
// $sql = "SELECT " . $table . ".*, CONCAT( I.Nombre, ' ', I.Apellido ) AS NombreInvitado FROM " . $table . " , Invitado I, Socio S WHERE SocioInvitadoEspecial.IDSocio = S.IDSocio AND I.IDInvitado = SocioInvitadoEspecial.IDInvitado AND " . $where . " AND SocioInvitadoEspecial.Ingreso = 'N'  " . $condicion_id_invitacion . "  ORDER BY $key $sord " . $str_limit;

$sql = "SELECT " . $table . ".*, CONCAT( I.Nombre, ' ', I.Apellido ) AS NombreInvitado FROM " . $table . " , Invitado I, Socio S WHERE SocioInvitadoEspecial.IDSocio = S.IDSocio AND I.IDInvitado = SocioInvitadoEspecial.IDInvitado AND " . $where . " AND ( SocioInvitadoEspecial.Ingreso !='S' OR DATE(SocioInvitadoEspecial.FechaIngreso) != CURDATE()) AND (DATE(SocioInvitadoEspecial.FechaSalida)!= CURDATE() OR SocioInvitadoEspecial.FechaSalida is null)  " . $condicion_id_invitacion . "  ORDER BY $key $sord " . $str_limit;
...
while ($row = $dbo->fetchArray($result)) {
    ...
    if ($origen != "mobile") :
        ...
        // *Validaciones comentadas porque retrazan la carga de invitados 20230105*
        ...
        // $sql1 = "Select IDSocioInvitadoEspecial From SocioInvitadoEspecial Where IDInvitado = '" . $row["IDInvitado"] . "' and MONTH(FechaInicio) = '" . $mes_invitacion . "' and YEAR(FechaInicio) = '" . $year_invitacion . "' AND IDClub = '" . SIMUser::get("club") . "' and Ingreso = 'S'";
        ...
        // $sql2 = "Select IDSocioInvitadoEspecial From SocioInvitadoEspecial Where IDSocio = '" . $row["IDSocio"] . "' and MONTH(FechaInicio) = '" . $mes_invitacion . "' and YEAR(FechaInicio) = '" . $year_invitacion . "' and IDClub = '" . SIMUser::get("club") . "' and Ingreso = 'S'";
        ...
        // $sql3 = "Select IDSocioInvitadoEspecial From SocioInvitadoEspecial Where IDSocio = '" . $row["IDSocio"] . "' and FechaInicio = '" . $hoy_invitacion . "' and IDClub = '" . SIMUser::get("club") . "' and Ingreso = 'S'";
        ...
        $responce->rows[$i]['cell'] = array(
            ...
            "Placa" => "<font color='$color_fila'></font>",

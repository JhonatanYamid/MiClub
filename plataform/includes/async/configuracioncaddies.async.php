    ...
    case "search":
        ...
        foreach ($array_buqueda->rules as $key_busqueda => $search_object) {
            switch ($search_object->field) {
                ...
                case 'CantidadCaddiesEspecial':
                    $where .= " AND ( CantidadCaddiesEspecial LIKE '%" . $search_object->data . "%' )";

                    break;

                case 'TextoEspera':
                    $where .= " AND ( TextoEspera LIKE '%" . $search_object->data . "%' )";

                    break;

                case 'LabelExplicacionSeleccionCaddie':
                    $where .= " AND ( LabelExplicacionSeleccionCaddie LIKE '%" . $search_object->data . "%' )";

                    break;

                case 'LabelSeleccionServicio':
                    $where .= " AND ( LabelSeleccionServicio LIKE '%" . $search_object->data . "%' )";

                    break;

                case 'LabelMisReservasCaddies':
                    $where .= " AND ( LabelMisReservasCaddies LIKE '%" . $search_object->data . "%' )";
...
while ($row = $dbo->fetchArray($result)) {
    ...
    if ($origen <> "mobile")
        $responce->rows[$i]['cell'] = array(
            ...
            "CantidadCaddiesEspecial" => $row["CantidadCaddiesEspecial"],
            "TextoEspera" => $row["TextoEspera"],
            "LabelExplicacionSeleccionCaddie" => $row["LabelExplicacionSeleccionCaddie"],
            "LabelSeleccionServicio" => $row["LabelSeleccionServicio"],
            "LabelMisReservasCaddies" => $row["LabelMisReservasCaddies"],

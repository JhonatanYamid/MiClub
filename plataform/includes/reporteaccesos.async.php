...
if(!empty($_GET["DocumentoContratista"])){
	$array_where [] = "I.Documento = '".$_GET["DocumentoContratista"]."'";
}

if(!empty($_GET["NombreContratista"])){
	$array_where [] = "I.Nombre like '%".$_GET["NombreContratista"]."%'";
}


if(!empty($_GET["ApellidoContratista"])){
	$array_where [] = "I.Apellido like '%".$_GET["ApellidoContratista"]."%'";
}
...
if(!empty($_GET["FechaInicio"])){
	$array_where [] = "FechaInicio = '".$_GET["FechaInicio"]."'";
}

if(!empty($_GET["FechaFin"])){
	$array_where [] = "FechaFin = '".$_GET["FechaFin"]."'";
}
...
$result = $dbo->query("SELECT COUNT(*) AS count FROM " . $table . " , Invitado I, Socio S " . $where . " AND SocioAutorizacion.IDSocio = S.IDSocio AND I.IDInvitado = SocioAutorizacion.IDInvitado");
...
echo $sql = "SELECT " . $table . ".*, CONCAT( I.Nombre, ' ', I.Apellido ) AS NombreInvitado,I.NumeroDocumento,I.IDTipoInvitado FROM " . $table . " , Invitado I, Socio S " . $where . " AND SocioAutorizacion.IDSocio = S.IDSocio AND I.IDInvitado = SocioAutorizacion.IDInvitado ". $where_filtro. "  ORDER BY $key $sord LIMIT " . $start . ",".$limit;
//exit;
//var_dump($sql);
...
while($row = $dbo->fetchArray($result)) {
	...
	if( $origen <> "mobile" ):
		...
		$responce->rows[$i]['cell'] = array( 										
										...
										"Predio" => "<font color='$color_fila'>" .$datos_invitado["Predio"] . "</font>",

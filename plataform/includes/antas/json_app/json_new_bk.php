...
//Datos Recetas
	...
	$SqlCategoria = "SELECT * FROM CategoriaProducto WHERE Publicar='S' AND IDPadre = 0";
	...
	if( $NumCategoria > 0 )
	{
		...
		while( $TipoCategoria = $dbo->fetchArray( $QryCategoria ) )
		{
			...
			$SqlCategoriaHija = "SELECT * FROM CategoriaProducto WHERE Publicar='S' AND IDPadre = ".$TipoCategoria[IDCategoria]."";
			$QryCategoriaHija = $dbo->query( $SqlCategoriaHija );
			$NumCategoriaHija = $dbo->rows( $QryCategoriaHija );
			$contador=0;
		
			if( $NumCategoriaHija > 0 )
			{
				...
				while( $TipoCategoriaHija = $dbo->fetchArray( $QryCategoriaHija ) )
				{
					...
					$arraySubLinea["Descripcion"] = htmlspecialchars_decode(htmlentities(utf8_encode($TipoCategoriaHija[Nombre]), ENT_NOQUOTES, 'UTF-8', true));
					$arraySubLinea["URLImagen"] = CONTENIDO_ROOT.$TipoCategoriaHija[Foto];


			
						$SqlCategoriaProducto = "SELECT * FROM Producto WHERE Publicar='S' AND IDCategoria = '".$TipoCategoriaHija[IDCategoria]."'";
						...
						if( $NumCategoriaProducto > 0 )
						{
							...
							while( $TipoCategoriaProducto = $dbo->fetchArray( $QryCategoriaProducto ) )
							{
								...
								$array['IDLinea'] = $TipoCategoriaProducto[IDCategoria];
								...
								array_push($arraySubLinea['Producto'],$array);
							}
						}

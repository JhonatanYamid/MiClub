...
if( $NumTipo > 0 )
{
	...
	{		
		...
		echo "insert into tiporecetas (id, nombre, publicar) VALUES (".$Tipo[IDTipo].",'".$Tipo[Nombre]."','S');";
		if( $NumTipoReceta > 0 )
		{
			$i = 0;
			...
			foreach( $arrayTipoReceta as $key => $value )
			{
				...
				//	$file = file_get_contents($Receta["Foto"]);
				//	$Receta["Foto"] = base64_encode($file);
					$arrayReceta[$Receta["IDReceta"]] = $Receta;
					$Tipo["Receta"][$Receta["IDReceta"]] = $arrayReceta[$Receta["IDReceta"]];
					echo "insert into receta (id, nombre, foto, preparacion, ingredientes, porciones, publicar, orden, destacado, introduccion, idtipo) VALUES (".$Receta["IDReceta"].", '".$Receta["Nombre"]."', '".$Receta["Foto"]."', '".$Receta["Preparacion"]."', '".$Receta["Ingredientes"]."', '".$Receta["Porciones"]."', 'S', '".$Receta["Orden"]."', '".$Receta["Destacado"]."', '".$Receta["Introduccion"]."', '".$Tipo["IDTipo"]."');";
				}
...
while( $Marcas = $dbo->fetchArray( $QryMarcas ) )
{
	...
	echo "insert into marcas (id, nombre, foto) values (".$Marcas[IDCategoria].", '".$Marcas[Nombre]."', '".$Marcas[Foto]."');";

...
SIMReg::setFromStructure( array(
					"title" => $dbo->getFields( "Club" , "Nombre" , "IDClub = '".$_SESSION[IDClub]."'")." :: " ." Invitados",
...
$array_valida = array(  
	 "IDSocio" => "IDSocio" , "IDServicio" => "IDServicio", 	"IDServicioElemento" => "IDServicioElemento",
	 "IDEstadoReserva" => "IDEstadoReserva", "Fecha" => "Fecha", "Hora" => "Hora"
); 
...
SIMNotify::capture( SIMResources::$mensajes[ SIMNet::req("m") ]["msg"] , SIMResources::$mensajes[ SIMNet::req("m") ]["type"] );	
		...
		switch ( SIMNet::req( "action" ) ) {
			...
			case "list" :
				...
				$fieldInt = array("IDClub","IDServicio","IDServicioElemento", "IDEstadoReserva");
						
				$fieldStr = array ( "Fecha" , "Hora" );
				
				$listjoin = array( "Socio" => "V.IDSocio"); 
				
				$fromjoin = array("Socio" => "Socio.IDSocio=V.IDSocio");
				
				$wherejoin = array( "Nombre" => "Nombre like '%".$_GET["Nombre"]."%'",
								   "Apellido" => "Apellido like '%".$_GET["Apellido"]."%'" );	 
...
<tr>
			<td  class="columnafija" > Socio </td><td>
					<select name="IDSocio" id="IDSocio">
                    	<option value="">[Seleccione]</option>
					<?php 
						$qry_socio = $dbo->all( "Socio", " IDClub = '".$_SESSION[IDClub]."' Order By Nombre" );
						while ( $r_socio = $dbo->object( $qry_socio ) ): ?>
							<option value="<?php echo $r_socio->IDSocio?>" <?php if($r_socio->IDSocio==$frm[IDSocio]) echo "selected"; ?>> <?php echo $r_socio->Nombre . " " . $r_socio->Apellido;   ?></option>
                        <?php	
						endwhile;
					?>
                    </select>
...
<tr>
  ...
  <td><select name="IDServicio" id="IDServicio" class="popup">
    <option value="">[Seleccione]</option>
    <?php 
						$qry_servicio = $dbo->all( "Servicio", " IDClub = '".$_SESSION[IDClub]."' Order By Nombre" );
						while ( $r_servicio = $dbo->object( $qry_servicio ) ): 
						$array_servicio_club []=  $r_servicio->IDServicio; 
						?>
    <option value="<?php echo $r_servicio->IDServicio?>" <?php if($r_servicio->IDServicio==$frm[IDServicio]) echo "selected"; ?>> <?php echo $r_servicio->Nombre;    ?></option>
    <?php	
						endwhile;
					?>
  </select></td>
...
<tr>
  <td  class="columnafija" >Elemento</td>
  <td>
  <?php
  	if(count($array_servicio_club)>0):
		$id_servicio_club = implode(",",$array_servicio_club);
	endif;
	
  ?>
  
  <select name="IDServicioElemento" id="IDServicioElmento" class="popup">
    <option value="">[Seleccione]</option>
    <?php 
						$qry_servicio_elemento = $dbo->all( "ServicioElemento", " IDServicio in (".$id_servicio_club.") Order By Nombre" );
						while ( $r_servicio_elemento = $dbo->object( $qry_servicio_elemento ) ): ?>
    <option value="<?php echo $r_servicio_elemento->IDServicioElemento?>" <?php if($r_servicio_elemento->IDServicioElemento==$frm[IDServicioElemento]) echo "selected"; ?>> <?php echo $r_servicio_elemento->Nombre;    ?></option>
    <?php	
						endwhile;
					?>
  </select></td>
...
<tr>
  <td  class="columnafija" >Hora</td>
  <td><input id="Hora" type="time" size="10" title="Hora" name="Hora" class="input mandatory" value="<?php echo $frm["Hora"] ?>" /></td>
</tr>
<tr>
  <td  class="columnafija" >Tee</td>
  <td>
  <select name="Tee" id="Tee" class="popup">
  		<option value="">[Seleccione]</option>
        <option value="Tee1" <?php if($frm["Tee"]=="Tee1") echo "selected";  ?>>Tee 1</option>
        <option value="Tee10" <?php if($frm["Tee"]=="Tee10") echo "selected";  ?>>Tee 10</option>
  </select>
  </td>
</tr>
<tr>
  <td  class="columnafija" >Estado</td>
  <td><?php echo SIMHTML::formPopUp( "EstadoReserva" , "Nombre" , "Nombre" , "IDEstadoReserva" , $frm["IDEstadoReserva"] , "[Seleccione]" , "popup mandatory" , "title = \"Estado\"" )?></td>
</tr>
<tr>
...
<td nowrap><?php echo $dbo->getFields( "Socio" , "Nombre" , "IDSocio = '".$r->IDSocio."'") . " " . $dbo->getFields( "Socio" , "Apellido" , "IDSocio = '".$r->IDSocio."'"); ?></td> <td nowrap><?php echo $dbo->getFields( "Servicio" , "Nombre" , "IDServicio = '".$r->IDServicio."'"); ?></td>
<td nowrap><?php echo $dbo->getFields( "ServicioElemento" , "Nombre" , "IDServicioElemento = '".$r->IDServicioElemento."'"); ?></td>
<td nowrap><? echo $r->Fecha ?></td>
<td nowrap><? echo $r->Hora ?></td>
<td nowrap><?php echo $dbo->getFields( "EstadoReserva" , "Nombre" , "IDEstadoReserva = '".$r->IDEstadoReserva."'"); ?></td> 
</tr>
<? } // END for
?>
<tr>
			...
			<td align="center">
				<table width="100%" border="0" cellspacing="2" cellpadding="0">
					<tr>
						...
						<td width="131"><select name="IDServicio" id="IDServicio" class="popup">
						  <option value="">[Seleccione]</option>
						  <?php 
						$qry_servicio = $dbo->all( "Servicio", " IDClub = '".$_SESSION[IDClub]."' Order By Nombre" );
						while ( $r_servicio = $dbo->object( $qry_servicio ) ): 
  ...
  <select name="IDServicioElemento" id="IDServicioElmento" class="popup">
    ...
    <?php	
					...
					?>
					  ...
					  <td>Estado</td>
					  <td><input id="Hora" type="time" size="10" title="Hora" name="Hora" class="input" value="<?php echo $frm["Hora"] ?>" /></td>
				  </tr>
					<tr>
						<td width="100"></td>	
						<td width="131"></td>

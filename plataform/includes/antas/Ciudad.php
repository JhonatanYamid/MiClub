		...
		switch ( $action ) {
			...
			case "insert" :	
				if( !SIMNotify::capture( SIMUtil::valida( $_POST , $array_valida ) , "error" ) )
				{
					...
					$files =  SIMFile::upload( $_FILES["Foto"] , CIUDAD_DIR , "IMAGE" );
			...
			break;
			case "DelImgNot":
				$doceliminar = CIUDAD_DIR.$dbo->getFields( $table , $_GET["img"] , "IDCiudad = '" . $_GET[id] . "'" );
				unlink($doceliminar);
				$dbo->query("UPDATE $table SET " . $_GET["img"] . " = '' WHERE IDCiudad = $_GET[id] LIMIT 1 ;");
				SIMHTML::jsAlert("Imagen Eliminada Correctamente");
				SIMHTML::jsRedirect( "?mod=" . $mod . "&action=edit&id=".$id."&idlang=1" );
				exit;
			break;
	...
	<div id="">
		...
		<table class="adminform">
			...
			<tr>
			  ...
			  <td><?php echo SIMHTML::formPopUp( "Departamento" , "Nombre" , "Nombre" , "IDDepartamento" , $frm["IDDepartamento"] , "[Seleccione el Departamento]" , "popup mandatory" , "title = \"Departamento\"" )?></td>
			  </tr>
			<tr>

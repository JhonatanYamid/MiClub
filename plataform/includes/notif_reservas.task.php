 ...
 <?
  ...
  /*
  $sql_club_pais="SELECT IDClub, IDPais FROM ConfiguracionClub WHERE IDPais > 0 ";
  $r_club_pais=$dbo->query($sql_club_pais);
  while($row_club_pais=$dbo->fetchArray($r_club_pais)){
    echo "<br>".$row_club_pais["IDClub"];
    $array_club_pais[$row_club_pais["IDPais"]][]=$row_club_pais["IDClub"];
  }

  foreach($array_club_pais as $id_pais => $clubes){
    foreach($clubes as $idclub_pais){
      switch ($id_pais):
        case "1": // Colombia
        break;
        case "7": // Uruguay
      ...
      endswitch;  
      echo "<br>PAIS: " . $id_pais . " Club: " . $idclub_pais;
    } 
  ...
  */
  ...
  $sql_reservas = "SELECT * FROM ReservaGeneral WHERE IDClub in (101,107,112) and Fecha = CURDATE() AND Hora > '".$HoraActual."' AND Hora <= AddTime('".$HoraActual."', '00:59:00') AND Notificado = 'N' AND Tipo <> 'Automatica' ";
  envia_push_recordacion($sql_reservas);

  //Uruguay
  date_default_timezone_set('America/Montevideo');
  $HoraActual=date("H:i:s");
  $sql_reservas = "SELECT * FROM ReservaGeneral WHERE IDClub in (125,185) and Fecha = CURDATE() AND Hora > '".$HoraActual."' AND Hora <= AddTime('".$HoraActual."', '00:59:00') AND Notificado = 'N' AND Tipo <> 'Automatica' ";
  envia_push_recordacion($sql_reservas);

  //Venezuela
  date_default_timezone_set('America/Caracas');
  $HoraActual=date("H:i:s");
  $sql_reservas = "SELECT * FROM ReservaGeneral WHERE IDClub in (141,183,189,208,201,219,225,232) and Fecha = CURDATE() AND Hora > '".$HoraActual."' AND Hora <= AddTime('".$HoraActual."', '00:59:00') AND Notificado = 'N' AND Tipo <> 'Automatica' ";
  envia_push_recordacion($sql_reservas);
  ...
  $HoraActual=date("H:i:s");
  $sql_reservas = "SELECT * FROM ReservaGeneral WHERE IDClub not in (101,107,112,125,185,141,189,141,183,189,208,201,219,225,232) and Fecha = CURDATE() AND Hora > '".$HoraActual."' AND Hora <= AddTime('".$HoraActual."', '00:59:00') AND Notificado = 'N' AND Tipo <> 'Automatica' ";
  envia_push_recordacion($sql_reservas);

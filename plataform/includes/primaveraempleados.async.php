...
$table = "LukerEmpleados";
$key = "IDLukerEmpleados";
$where = " WHERE " . $table . ".IDClub = '" . SIMUser::get("club") . "' ";
$script = "primaveraempleaados";
...
switch ($oper) {
    ...
    case "del":
        ...
        $sql_delete = "DELETE FROM LukerEmpleados WHERE IDLukerEmpleados = '" . $_POST["id"] . "' LIMIT 1";
        //echo "<br>";
        $qry_delete = $dbo->query($sql_delete);

        $_GET["page"] = 1;
        $_GET['rows'] = 100;
        $_GET['sidx'] = "EMP_NOMBRE";
        $_GET['sord'] = "ASC";
        ...
        foreach ($array_buqueda->rules as $key_busqueda => $search_object) {
            switch ($search_object->field) {
                case 'qryString':
                    ...
                    $where .= " AND ( EMP_NOMBRE LIKE '%" . $search_object->data . "%' )";
                    break;
            }
        } //end for
    ...
    case "searchurl":
        ...
        if (!empty($qryString)) {
            ...
            $where .= " AND ( EMP_NOMBRE LIKE '%" . $qryString . "%'  )  ";
        } //end if
        break;
}

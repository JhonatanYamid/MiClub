...
<script type="text/javascript">
	jQuery(function($) {
		...
		$('.autocomplete-ajax').autocomplete({
			...
			onSelect: function(suggestion) {
				...
				$('#Accion_Socio').val(suggestion.accion);
		...
		$('.autocomplete-ajax-funcionario-cedula').autocomplete({
			serviceUrl: 'includes/async/usuarios.async.php',
			//lookup: countriesArray,
			dataType: "json",
			transformResult: function(response) {
				return {
					suggestions: $.map(response.rows, function(dataItem) {
						return {
							value: dataItem.cell.Nombre,
							data: dataItem.cell.NumeroDocumento,
							id: dataItem.cell.IDUsuario
						};
					})
				};
			},
			...
			onSelect: function(suggestion) {
				...
				$('#NumeroDocumento').val(suggestion.data);
				$('#Nombre').val(suggestion.value);
				$('#IDUsuarioTalonera').val(suggestion.id);
			}
		...
		// Initialize ajax autocomplete correspondencia casa + socios en ellas
		$('.autocomplete-ajax_predio').autocomplete({
			serviceUrl: 'includes/async/predio.async.php',
			//lookup: countriesArray,
			dataType: "json",
			transformResult: function(response) {
				return {
					suggestions: $.map(response.rows, function(dataItem) {
						return {
							value: dataItem.cell.Torre + " | " + dataItem.cell.Predio + " - " + dataItem.cell.Nombre + " " + dataItem.cell.Apellido,
							data: dataItem.cell.IDSocio,
							predio: dataItem.cell.Predio + " | " + dataItem.cell.Torre,
			...
			onSelect: function(suggestion) {
				...
				$('#IDSocios').val(suggestion.data);
				$('#Nombre').val(suggestion.value);

				$('#Vivienda').val(suggestion.predio);
	...
	function validahorasdisponibilidadcaddie(contador_horas) {

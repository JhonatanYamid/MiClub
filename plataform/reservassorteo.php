...
<body class="no-skin">
										...
										<div class="widget-box transparent" id="recent-box">
											<div class="widget-header">
												<h4 class="widget-title lighter smaller">
													...
													MOSTRANDO INSCRITOS SORTEO DE <span id="contentFechaActual"><? echo strtoupper(SIMUtil::tiempo($fecha)) ?></span> -
													<input type="hidden" id="fechareserva" value="" />
													<input type="hidden" id="fechaseleccion" value="<?php echo $fecha; ?>" />
													<a href="javascript:void(0);" class="calendar_reservas2">
																		...
																		<div class="tab-content">
																			...
																			?>
																				<div id="tab<?= $key_elemento ?>" class="tab-pane <?= $active ?>">
																					...
																					<table id="grid-table2<?= $key_elemento ?>"></table>

																					<div id="grid-pager2<?= $key_elemento ?>"></div>
	...
	<script type="text/javascript">
		jQuery(function($) {
			...
			?>
				grid_selector = "#grid-table2<?= $key_elemento ?>";
				pager_selector = "#grid-pager2<?= $key_elemento ?>";


				jQuery(grid_selector).jqGrid({

					url: 'includes/async/reservassorteo.async.php?idservicio=<?= $ids ?>&idelemento=<?= $datos_elemento["IDElemento"] ?><?= $url_search ?>',
					datatype: "json",
					colNames: ['Detalle', 'Fecha', 'Hora', 'Socio', /* 'Accion', */ 'Posicion Sorteo','Tee', 'Cancelar Reserva.'],
					colModel: [{
							name: 'Detalle',
							index: 'Detalle',
							align: "center"
							...
							if (confirm("Esta seguro que desea cancelar la reserva?")) {
								jQuery.ajax({
									...
									"success": function(data) {
										...
										$("#grid-table2<?= $key_elemento ?>").trigger("reloadGrid");
										return false;

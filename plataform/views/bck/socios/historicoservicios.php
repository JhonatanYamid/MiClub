...
<div class="modal fade" id="modalDetalle" tabindex="-1" role="dialog" aria-labelledby="LabelDetalle" aria-hidden="true">
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			<div class="modal-header">               
				<h5 class="modal-title" id="LabelDetalle">Histórico de Inventario</h5>
				<button type="button" class="close" style="line-height:0px !important;" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
...
<script type="text/javascript">
	...
	jQuery(grid_selector_service).jqGrid({
		...
		colModel: [
			{name: 'Producto',index: 'pf.Nombre',align: "left"},
			{name: 'Sede',index: 'c.Nombre',align: "left"},
			{name: 'FechaCreacion',index: 'FechaCreacion',align: "left"},
			{name: 'FechaInicio',index: 'FechaInicio',align: "left"},
			{name: 'FechaFin',index: 'FechaFin',align: "left"},
			{name: 'Disponible',index: 'Disponible',align: "left",search: false},
	...
	$(document).on("click", ".btnDetalle", function (){ 
        ...
        let  Names = ['Fecha', 'Evento','Fecha Inicia','Usuario','Observaciones'];
        let  Model = [
            {name: 'Fecha', index: 'Fecha', align: "left"},
            {name: 'Evento', index: 'Evento', align: "left"},
            {name: 'FechaIniciaEvento', index: 'FechaIniciaEvento', align: "left"},
            {name: 'Usuario', index: 'u.Nombre', align: "left"},
            {name: 'Observacion', index: 'Observacion', align: "left",width:300},
        ]
        ...
        jQuery("#tablaDetalle").jqGrid('filterToolbar', {defaultSearch: true, stringResult: true})
	...
	$("#guardarConf").on("click", function (){
		...
		if(idConf == 1){
			...
			if($("#FechaInicioNew").val() != ""){
				...
				alert('Por favor ingrese una fecha para continuar.');
			}
		}
		else if(idConf == 2){
			...
			if($("#FechaFinNew").val() != ""){
				if(FechaFinNew > FechaInicio){
					...
					alert('La fecha final no puede ser menor o igual a la fecha de inicio, Por favor verifiquela');
				}
			}
			else{
				alert('Por favor ingrese una fecha para continuar.');
			...
			if(($("#numCongela").val() != 0 || $("#numCongela").val() != "") && $("#FechaCongela").val() != ""){
				if(FechaCongela >= FechaInicio && FechaCongela < FechaFin){
					...
					alert("Por favor ingrese una fecha valida, entre " + infoConfig.FechaInicio + " y " + infoConfig.FechaFin);
				}
			}
			else{
				alert('Por favor ingrese numero o fecha validos para continuar.');
			}
		}
		else if(idConf == 4){
			...
			if($("#IDSocioTransfiere").val() != "" && $("#FechaTransfiere").val() != ""){
				...
				alert('Por favor seleccione un socio o fecha validos para continuar.');
			}
		}
		else{
			alert('Por favor seleccione una opcion de configuración');
		}

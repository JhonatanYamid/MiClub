		...
		<div class="col-xs-12 col-sm-6">
			...
			<div class="col-sm-8">
				<select name="IDTipoInvitado" id="IDTipoInvitado" title="Tipo Invitado" class="form-control mandatory">
					<option value=""></option>
					<?php
					$sql_tipoinv_club = "Select * From ParametroAcceso Where IDClub = '" . SIMUser::get("club") . "'";
					$qry_tipoinv_club = $dbo->query($sql_tipoinv_club);
					$r_tipoinv = $dbo->fetchArray($qry_tipoinv_club);
					$array_tipo_invitado = explode('|', $r_tipoinv["TipoInvitado"]);



					foreach ($array_tipo_invitado as $nombre_tipo) :
						$sqlTipoInvitado = "SELECT * FROM TipoInvitado WHERE Nombre='" . $nombre_tipo . "'";
						$qryTipoInvitado = $dbo->query($sqlTipoInvitado);
						$Tipo = $dbo->fetchArray($qryTipoInvitado);

						if (!empty($Tipo["Nombre"])) { ?>

							<option value="<?php echo $Tipo["IDTipoInvitado"]; ?>" <?php if ($Tipo["IDTipoInvitado"] == $frm["IDTipoInvitado"]) echo "selected";  ?>><?php echo $Tipo["Nombre"]; ?></option>

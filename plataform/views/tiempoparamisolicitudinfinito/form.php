...
if ($AuxiliosInfinitoSolicitud["IDUsuario"] > 0) {
    $sqlUsuario = "SELECT IDUsuario,Nombre,NumeroDocumento FROM Usuario WHERE IDUsuario = " . $AuxiliosInfinitoSolicitud['IDUsuario'];
    $queryUsuario = $dbo->query($sqlUsuario);
    $rowUser = $dbo->assoc($queryUsuario);
    $user = "Empleado";
} else {
    $sqlSocio = "SELECT IDSocio,Nombre,Apellido,NumeroDocumento FROM Socio WHERE IDSocio = " . $AuxiliosInfinitoSolicitud['IDSocio'];
                ...
                <div class="col-xs-12">
                    <form class="form-horizontal formvalida" role="form" method="post" id="frm<?php echo $script; ?>" action="<?php echo SIMUtil::lastURI() ?>" enctype="multipart/form-data">
                        <div class="form-group first ">
                            ...
                            <div class="col-xs-12 col-sm-6 first">
                                <label class="col-sm-4 control-label no-padding-right" for="FechaTrCr"> <?= SIMUtil::get_traduccion('', '', 'Númerodedocumento', LANGSESSION); ?> </label>
                                <div class="col-sm-8"><input type="text" id="NumeroDocumento" name="NumeroDocumento" placeholder="<?= SIMUtil::get_traduccion('', '', 'Fechasolicitud', LANGSESSION); ?>" class="col-xs-12 mandatory" title="<?= SIMUtil::get_traduccion('', '', 'Númerodedocumento', LANGSESSION); ?>" value="<?php echo $NumeroDocumento; ?>" readonly="readonly"></div>
                            </div>
                            <div class="col-xs-12 col-sm-6 first">

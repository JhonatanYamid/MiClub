...
if (SIMNet::req("action") == "searchTravel") {

	$inicio = SIMNet::get("inicio");
	$fin = SIMNet::get("fin");
	$IDTipoVehiculo = SIMNet::get("IDTipoVehiculo");
	$calificacioninicial = SIMNet::get("calificacioninicial");
	$calificacionfinal = SIMNet::get("calificacionfinal");
	$IDMotivosCalificacion = SIMNet::get("IDMotivosCalificacion");
	$Persona = SIMNet::get("Persona");
	$Estado = SIMNet::get("Estado");

	$url_search = "?oper=searchTravel&inicio=$inicio&fin=$fin&IDTipoVehiculo=$IDTipoVehiculo&calificacioninicial=$calificacioninicial&calificacionfinal=$calificacionfinal&IDMotivosCalificacion=$IDMotivosCalificacion&Persona=$Persona&Estado=$Estado";
}
				...
				<div class="col-xs-12">
					<form name="frmexportaviaje" id="frmexportaviaje" method="post" enctype="multipart/form-data" action="procedures/excel-carpool.php">
						<table id="simple-table" class="table table-striped table-bordered table-hover">
							<tr>
								<td><?= SIMUtil::get_traduccion('', '', 'Fechadeviajeinicial', LANGSESSION); ?>:</td>
								...
								<td><?= SIMUtil::get_traduccion('', '', 'Fechadeviajefinal', LANGSESSION); ?>:</td>
								...
								<td><?= SIMUtil::get_traduccion('', '', 'Vehiculo', LANGSESSION); ?>:</td>
								<td><?php echo SIMHTML::formPopUp("TipoVehiculo", "Nombre", "Nombre", "IDTipoVehiculo", "", "[Seleccione]", "form-control", "", ""); ?></td>
							</tr>
							<tr>
								<td><?= SIMUtil::get_traduccion('', '', 'Calificacióndesde', LANGSESSION); ?>:</td>
								<td><input type="number" name="calificacioninicial" id="calificacioninicial" class="form-control" value=""></td>
								<td><?= SIMUtil::get_traduccion('', '', 'Calificaciónhasta', LANGSESSION); ?>:</td>
								<td><input type="number" name="calificacionfinal" id="calificacionfinal" class="form-control" value=""></td>
								<td><?= SIMUtil::get_traduccion('', '', 'Motivodecalificación', LANGSESSION); ?>:</td>
								<td><?php echo SIMHTML::formPopUp("MotivosCalificacion", "Nombre", "Nombre", "IDMotivosCalificacion", "", "[Seleccione]", "form-control", "", "AND IDClub=" . SIMUser::get("club")); ?></td>
							</tr>
							<tr>
								<td>Creado por:</td>
								...
								<td>
									<select name="Estado" id="Estado">
										<option value=1><?= SIMUtil::get_traduccion('', '', 'Abierto', LANGSESSION); ?></option>
										<option value=2><?= SIMUtil::get_traduccion('', '', 'Cerrado', LANGSESSION); ?></option>
										<option value=3><?= SIMUtil::get_traduccion('', '', 'Cancelado', LANGSESSION); ?></option>
									</select>
								</td>
								<td></td>
								<td></td>
	...
	jQuery(function($) {
		...
		jQuery("#tablaSolicitudes").jqGrid({
			...
			colModel: [{
				...
				{
					name: 'MotivosCalificacion',
					index: 'MotivosCalificacion',
					align: "left",
					width: 270
				},
			],
		...
		jQuery("#tablaSolicitudes").jqGrid('filterToolbar', {
			defaultSearch: true,
			stringResult: true
		})
		...
		$(document).on("click", ".btnSolicitudes", function() {
			...
			jQuery("#tablaSolicitudes").jqGrid('setGridParam', {
				url: 'includes/async/solicitudesviaje.async.php?idViaje=' + idViaje
			}).trigger("reloadGrid");

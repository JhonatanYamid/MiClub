          ...
          <form name="frmpro" id="frmpro" action="?mod=<?php echo SIMReg::get( "mod" )?>" method="post" class="formvalida" enctype="multipart/form-data">
              <table id="simple-table" class="table table-striped table-bordered table-hover">
                  ...
                  if( $_GET[IDServicioPropiedad] )
                  {
                          ...
                          ?> <input type="hidden" name="IDServicioPropiedad" id="IDServicioPropiedad" value="<?php echo $EditServicioPropiedad[IDServicioPropiedad]?>" /> <?php
                  }
                  ?> <table cellspacing="0" cellpadding="0" border="0" width="100%" class="adminform">
                                          ...
                                          <tr>
                                              <td>Obligatorio?</td>
                                              <td>
                                                  <? echo SIMHTML::formradiogroup( array_flip( SIMResources::$sino ) , $EditServicioPropiedad["Obligatorio"] , 'Obligatorio' , "class='input'" ) ?>
                                              </td>
                                          </tr>
                                          <tr>
                                          ...
                                          <tr>
                                              <td>Publicar</td>
                                              <td>
                                                  <? echo SIMHTML::formradiogroup( array_flip( SIMResources::$sino ) , $EditServicioPropiedad["Publicar"] , 'Publicar' , "class='input'" ) ?>
                                              </td>
                                          </tr>
                                          <tr>
                                              <td>Tipo reserva al que aplica (si se deja vacio aplica para todos)</td>
                                              <td>
                                                  <?php
                                                    $SQLTiposReserva = "SELECT * FROM ServicioTipoReserva WHERE IDServicio = $frm[$key]";
                                                    $QRYTiposReserva = $dbo->query($SQLTiposReserva);?>
                                                    <select name=" IDServicioTipoReserva" id="IDServicioTipoReserva" class="form-control">
                                                      <option value="0">[TODOS LOS TIPOS]</option>
                                                      <?php
                                                      while($Datos = $dbo->fetchArray($QRYTiposReserva)):
                                                        ?>
                                                        <option value="<?=$Datos[IDServicioTipoReserva]?>" <?php if($EditServicioPropiedad["IDServicioTipoReserva"] == $Datos[IDServicioTipoReserva]) echo "selected"; ?>><?=$Datos[Nombre]?></option>
                                                        <?php
                                                      endwhile;
                                                      ?>
                                                    </select>
          ...
          <table id="simple-table" class="table table-striped table-bordered table-hover">
              ...
              <tbody id="listacontactosanunciante"> <?php
                      ...
                      <td align="center" width="64">
                          <a href="<?php echo "?mod=" . SIMReg::get( "mod" ) . "&action=edit&ids=" . $frm[$key] ."&IDServicioPropiedad=".$r->IDServicioPropiedad?>&tab=categoriaserv" class="ace-icon glyphicon glyphicon-pencil"></a>
                      </td>
                      <td><?php echo $r->Nombre; ?></td>
                      <td><?php echo $r->Tipo; ?></td>
                      <td><?php echo $r->Publicar; ?></td>
                      <td align="center" width="64">
                          <a class="ace-icon glyphicon glyphicon-remove confirma_eliminacion" href="?mod=<?php echo SIMReg::get( "mod" )?>&action=EliminaServicioPropiedad&ids=<?php echo $_GET["ids"];?>&IDServicioPropiedad=<? echo $r->IDServicioPropiedad ?>&tab=categoriaserv"></a>
                      </td>
                  </tr> <?php
                      }
                      ?> </tbody>

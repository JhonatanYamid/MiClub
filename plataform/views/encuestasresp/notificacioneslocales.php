<form name="frmnotifica" class="form-horizontal formvalida" role="form" method="post" id="EditNotificacionLocal<?php echo $script; ?>" action="<?php echo SIMUtil::lastURI() ?>" enctype="multipart/form-data">
	...
	<div class="form-group first ">
		...
		<div class="col-xs-12 col-sm-6">
			...
			<div class="col-sm-8">
				<input type="number" id="Periodicidad" name="Periodicidad" placeholder="<?= SIMUtil::get_traduccion('', '', 'Periodicidad', LANGSESSION); ?>" class="col-xs-12 mandatory" title="<?= SIMUtil::get_traduccion('', '', 'Periodicidad', LANGSESSION); ?>" value="<?php echo $EditNotificacionLocal["Periodicidad"]; ?>">minutos
			</div>
		</div>
...
<table id="simple-table" class="table table-striped table-bordered table-hover">
	...
	<tbody id="listacontactosanunciante">
		<?php
		$r_documento = &$dbo->all("NotificacionLocal", "IDClub = '" . $frm["IDClub"]  . "' and IDModulo ='58' and IDDetalle = '" . $frm[$key] . "'");
		while ($r = $dbo->object($r_documento)) {
		?>

			<tr class="<?php echo SIMUtil::repetition() ? 'row0' : 'row1' ?>">
				<td align="center" width="64">
					<a href="<?php echo $script . ".php" . "?action=edit&id=" . $frm[$key] . "&IDNotificacionLocal=" . $r->IDNotificacionLocal . "&IDModulo=" . $_GET["IDModulo"] ?>&tabencuesta=notificacionlocal" class="ace-icon glyphicon glyphicon-pencil"></a>
				</td>
				...
				<td align="center" width="64">
					<a class="ace-icon glyphicon glyphicon-remove confirma_eliminacion" href="?mod=<?php echo SIMReg::get("mod") ?>&action=EliminaNotificacionLocal&id=<?php echo $frm[$key]; ?>&IDNotificacionLocal=<? echo $r->IDNotificacionLocal ?>&IDModulo=<? echo $_GET["IDModulo"]; ?>&tabNotificacionLocal=notificacionlocal"></a>
				</td>
			</tr>
		<?php
		}

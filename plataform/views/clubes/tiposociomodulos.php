<form class="form-horizontal formvalida" role="form" method="post" id="EditTipoSocioModulo<?php echo $script; ?>" action="<?php echo SIMUtil::lastURI()?>" enctype="multipart/form-data"> <?php
    ...
    <div class="form-group first ">
        <div class="col-xs-12 col-sm-6">
            ...
            <div class="col-xs-12 col-sm-6">
            <label class="col-sm-4 control-label no-padding-right" for="form-field-1"> Categorias de servicios </label>
            <div class="col-sm-8">
                <div style="width:300px;">
                    <select multiple class="chosen-select form-control" name="IDCategoriaServicio[]" id="IDCategoriaServicio" data-placeholder="Selecciones valores..."> <?php
			
																	unset($array_valores_guardados);
 $r_valor_tabla = $dbo->all( "CategoriasServicios" , "IDClub = '".$frm[ $key ]."' and Activo='1'  ");
 $valores_guardados = $EditTipoSocioModulo[IDCategoriaServicio] ;
 if(!empty($valores_guardados)):
 $array_valores_guardados = explode("|",$valores_guardados);
 endif;

 while( $r_valor = $dbo->object( $r_valor_tabla ) ){
  if(empty($r_valor->Nombre)){
																					$NombreServicio = utf8_encode($dbo->getFields( "CategoriasServicios" , "Nombre" , "IDCategoriasServicios = '" . $r_valor->IDCategoriasServicios . "'" ));
 }
 else{
																				$NombreServicio = $r_valor->Nombre;
 } 																	  if(empty($valores_guardados)):
																				$seleccionar = "";
																			elseif(in_array($r_valor->IDCategoriasServicios,$array_valores_guardados)):
																				$seleccionar = "selected";
 else: 																			$seleccionar = "";
 endif; ?>
  <option value="<?php echo $r_valor->IDCategoriasServicios ?>" <?php echo $seleccionar; ?>><?php echo $NombreServicio; ?></option>
                        <? }	?>
                    </select>
	...
	<div class="form-group first ">
        <div class="col-xs-12 col-sm-6">
            ...
            <div class="col-sm-8">
				...
				<?php echo SIMHTML::formradiogroup( SIMResources::$sinoNum , $EditTipoSocioModulo["Ocultar"] , 'Ocultar' , "class='input'" ) ?>
...
<table id="simple-table" class="table table-striped table-bordered table-hover">
    ...
    <tbody id="listacontactosanunciante"> <?php
            ...
            <td><?php
															...
															?></td>
															  <td><?php
															$array_id_cat=explode("|",$r->IDCategoriaServicio);
															$nombre_cat="";
															if(count($array_id_modulo)>0){
																		foreach($array_id_cat as $id_cat){
																				$nombre_cat = $dbo->getFields( "CategoriasServicios" , "Nombre" , "IDCategoriasServicios = '" . $id_cat . "' and IDClub = '".$r->IDClub."'" );
																				if(empty(trim($nombre_cat))){
																						$nombre_cat = $dbo->getFields( "CategoriasServicios" , "Nombre" , "IDCategoriasServicios = '" . $id_cat . "'" );
																				}
																				echo utf8_encode($nombre_cat)."<br>";
																		}
															}

		...
		?>
			$('#myTABS a[href="#invitaciones"]').on('click', function(e) {
				jQuery(grid_selector).jqGrid({
					url: 'includes/async/get_Encuesta.async.php?id=<?php echo $_GET["id"]; ?>&Cedula=' + Cedula,
					datatype: "json",
					//colNames:['Usuario','Valor','Fecha' ],
					<?php

					echo "colNames: ['Eliminar','Usuario','Fecha','Hora','Predio','Numero de casa','" . str_replace(",", "','", implode(",", $array_preguntas)) . '\'],'; ?>
					colModel: [{
							name: 'Eliminar',
							index: '',
							width: '70',

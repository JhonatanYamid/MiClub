                            ...
                            <table>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <!--  <input class="btn btn-danger" type="button" name="borrarCheckin" id="borrarCheckin" club="<?= SIMUser::get('club') ?>" table="<?= $table ?>" data-id="<?= $id ?>" value="<?= SIMUtil::get_traduccion('', '', 'BorrarTodo', LANGSESSION); ?>"> -->
...
<script type="text/javascript">
    jQuery(function($) {
        ...
        $('#aprobarCheckin').click(function() {
            ...
            var extrasdespuesdelturno = "extrasdespuesdelturno";
            if (confirm("Esta seguro que desea aprobar todas las horas extras?")) {
                $.post('includes/async/aprobarHorasExtra.async.php', {
                    'club': club,
                    'table': table,
                    'id': id,
                    'extrasdespuesdelturno': extrasdespuesdelturno
                }, function(response) {
                    if (response == 1) {
        ...
        jQuery(grid_selector).jqGrid({
            ...
            url: 'includes/async/<?php echo $script; ?>HorasExtrasDespuesDeTurno.async.php<?= $url_search ?>',
            datatype: "json",
            colNames: ['<?= SIMUtil::get_traduccion('', '', 'Horainiciolaboral', LANGSESSION); ?>', '<?= SIMUtil::get_traduccion('', '', 'Horafinallaboral', LANGSESSION); ?>',
                ...
                '<?= SIMUtil::get_traduccion('', '', 'día', LANGSESSION); ?>', 'Tiempo Extra',

                '<?= SIMUtil::get_traduccion('', '', 'Estado', LANGSESSION); ?>', 'Observacion Entrada', 'Observacion Salida', '<?= SIMUtil::get_traduccion('', '', 'Comentario', LANGSESSION); ?>', 'Registro',
            ...
            width: 855,
            multiselect: true,
            editurl: "includes/async/<?php echo $script; ?>HorasExtrasDespuesDeTurno.async.php",
        ...
        jQuery(grid_selector).jqGrid('navGrid', pager_selector, { //navbar options
            ...
            */
            ...
            onClickButton: function() {
                ...
                location.href = "<?php echo $script; ?>.php?action=detalle&id=<?php echo $_GET["id"]; ?>&type=<?php echo $_GET["type"]; ?>";
        ...
        jQuery(grid_selectoringresado).jqGrid({
            ...
            url: 'includes/async/chekinlaboralListadoMesActualDespuesDeTurno.async.php<?= $url_search ?>',
            datatype: "json",
            colNames: ['<?= SIMUtil::get_traduccion('', '', 'Horainiciolaboral', LANGSESSION); ?>', '<?= SIMUtil::get_traduccion('', '', 'Horafinallaboral', LANGSESSION); ?>',
                '<?= SIMUtil::get_traduccion('', '', 'Fechaentrada', LANGSESSION); ?>', 'Fecha Salida',
                '<?= SIMUtil::get_traduccion('', '', 'día', LANGSESSION); ?>', 'Tiempo Extra',
            ...
            viewrecords: true,
            sortorder: "ASC",
            caption: "Registros mes actual despues del turno",
            height: "100%",
            width: 855,
            multiselect: true,
            editurl: "includes/chekinlaboralListadoMesActualDespuesDeTurno.async.php",

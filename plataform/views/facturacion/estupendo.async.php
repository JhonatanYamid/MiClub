    ...
    switch($oper){
        ...
        case "crearArchivo":
            ...
            $sql_info = "SELECT Nit, MatriculaMercantil, Nombre1, Nombre2, i.IDPaisDian, p.Nombre as Pais, p.Codigo as CodPais,  IF(i.IDDepartamentoDian < 10, CONCAT('0',i.IDDepartamentoDian), i.IDDepartamentoDian) as IDDepartamento, 
                            d.Nombre as Departamento, i.IDCiudadDian,c.Nombre as Ciudad, CodigoPostal, Direccion, Regimen, TipoOrganizacion
            ...
            $sql_socio = "SELECT CONCAT(s.Nombre,' ',s.Apellido) as Nombre, s.NumeroDocumento, s.TipoDocumento, s.CorreoElectronico, s.Celular, s.Direccion, s.IDPaisDian, p.Nombre as Pais,
                            p.Codigo as CodPais, IF(s.IDDepartamentoDian < 10, CONCAT('0',s.IDDepartamentoDian), s.IDDepartamentoDian) as IDDepartamentoDian, d.Nombre as Departamento, s.IDCiudadDian, c.Nombre as Ciudad, s.CodigoPostal
            ...
            while($rowP = $dbo->fetchArray($qry_productos)){
                ...
                if (array_key_exists($prcIm, $impuestos)) {
                    $impuestos[$prcIm]['BaseTot'] = $impuestos[$prcIm]['Base'] + $rowP['Base'];
                    $impuestos[$prcIm]['ImpuestoTot'] = $impuestos[$prcIm]['Impuesto'] + $rowP['Impuesto'];
                }else{
            ...
            $codigoDepartamento = trim($info['IDDepartamento']);//Codigo del departamento segun tabla de la Dian 
            $nombreDepartamento = trim($info['Departamento']);//Nombre del departamento en donde se encuentra el emisor
            $codigoMunicipio = trim($info['IDCiudadDian']);//Codigo del municipio segun tabla de la Dian 
            $nombreCiudad = trim($info['Ciudad']);//Nombre del ciudad en donde se encuentra el emisor
            ...
            $tipoIdentificacion = "31";// trim($tipoDocEstupendo[$socio['TipoDocumento']]);//Tipo de documento del receptor(opciones en config.inc.php)
            $identificacion = "900984424";//trim($socio['NumeroDocumento']);//identificacion del receptor
            ...
            $codigoDpto = "11"; //trim($socio['IDDepartamentoDian']);//Codigo de departamento del receptor
            $nmDepto = "Bogota D.C"; //trim($socio['Departamento']);//departamento del receptor
            $codMncpio = "11001"; //trim($socio['IDCiudadDian']);//Codigo de la Ciudad del receptor
            $nmMncpio = "Bogota D.C"; //trim($socio['Ciudad']);//Nombre de la Ciudad del receptor
            $dirReceptor = "Calle 169a #45-54 Ofc 2"; //trim($socio['Direccion']);//Direccion del receptor
            $codPais ="CO"; //trim($socio['CodPais']);//Codigo del pais del receptor
            $nmPais = "Colombia";//trim($socio['Pais']);//Nombre del pais del receptor
            $regimenReceptor = '49';//Tipo de regimen del receptor, 49 = no responsable de Iva 
            $emailReceptor = "jlospino@gmail.com"; //trim($socio['CorreoElectronico']);//Email del receptor
            $telefono = "316451793";//trim($socio['Celular']);//Telefono celular del receptor
            $codigoPostal = "110111";//$socio['CodigoPostal'] != '' ? trim($socio['CodigoPostal']) : '000000';//Codigo Postal del receptor
            $obligaReceptor = 'R-99-PN';//Obligaciones del contribuyente, R-99-PN = No aplica
            $detReceptor = 'ZZ=No Causa';//Detalles tributarios del receptor 
            $idAutorizado = "1015451793";//ID de la persona autorizada para descargar documentos(opcional)
            $tipoIdAutorizado = "31";//Tipo de identificacion del autorizado para descargar(opcional)
            ...
            foreach($impuestos as $key => $values){
                ...
                $valorSinImpuesto = trim($values['Base']);//Valor del subtotal sin impuesto.
                $porcentajeImpuesto = trim($key);//Porcentaje del impuesto aplicado
                $totalImpuesto = trim($values['Impuesto']);//Total del impuesto aplicado
            ...
            $totalBaseImponible = trim($factura['SubTotal']);//Total valor base imponible: base imponible para el calculo de los tributos
            ...
            $descuento = trim($factura['Descuento']);//Valor total del descuento aplicado a la factura
            ...
            foreach($productos as $producto){
                ...
                $codigoUnidadMedida = "NIU";//Identificacion de la unidad de medida NIU-número de unidades internacionales
                ...
                $valorTotalItem = trim($producto['SubTotal']);//Cantidad de producto por el precio unitario 
            ...
            foreach($pagos as $pago){
                ...
                $idPago = "";//Identificador del pago(opcional)
            ...
            //SECCION 40||Campos Adicionales(opcional)
            ...
            $archivo = base64_encode(utf8_encode($secciones));

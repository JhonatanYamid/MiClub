<form class="form-horizontal formvalida" role="form" method="post" id="EditPregunta<?php echo $script; ?>" action="<?php echo SIMUtil::lastURI() ?>" enctype="multipart/form-data">
	...
	if ($_GET[IDPregunta]) {
		$EditPregunta = $dbo->fetchAll("PreguntaVotacion", " IDPregunta = '" . $_GET["IDPregunta"] . "' ", "array");
			...
			<div class="col-sm-8">
				<select class="form-control" id="TipoCampo" name="TipoCampo">
					<optgroup label="Estándar">
						...
						<option value="sociounico" <?php if ($EditPregunta["TipoCampo"] == "sociounico") echo "selected"; ?>>Seleccionar usuarios</option>
	...
	<div class="col-xs-12 col-sm-12">
		...
		<table id="simple-table" class="table table-bordered table-hover">
			...
			<?php
			$sql_opciones = "SELECT * FROM VotacionOpcionesRespuesta WHERE IDPreguntaVotacion = '" . $_GET["IDPregunta"] . "' Order by Orden";
...
<table id="simple-table" class="table table-striped table-bordered table-hover">
	...
	<tbody id="listacontactosanunciante">
		...
		$r_documento = &$dbo->all("PreguntaVotacion", "IDVotacion = '" . $frm[$key]  . "'");
		...
		?>
			...
			<tr class="<?php echo SIMUtil::repetition() ? 'row0' : 'row1' ?>">
				<td align="center" width="64">
					<a href="<?php echo $script . ".php" . "?action=edit&id=" . $frm[$key] . "&IDPregunta=" . $r->IDPregunta ?>&tabencuesta=formulario" class="ace-icon glyphicon glyphicon-pencil"></a>
				</td>
				...
				<td align="center" width="64">
					<a class="ace-icon glyphicon glyphicon-remove confirma_eliminacion" href="?mod=<?php echo SIMReg::get("mod") ?>&action=EliminaPregunta&id=<?php echo $EditPregunta[$key]; ?>&IDPregunta=<? echo $r->IDPregunta ?>&tabpregunta=formulario"></a>
				</td>
			</tr>
		<?php
		}

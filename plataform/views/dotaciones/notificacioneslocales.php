<form name="frmnotifica" class="form-horizontal formvalida" role="form" method="post" id="EditNotificacionLocal<?php echo $script; ?>" action="<?php echo SIMUtil::lastURI() ?>" enctype="multipart/form-data">
	...
	<div class="form-group first ">
		...
		<div class="col-xs-12 col-sm-6">
			...
			<div class="col-sm-8">
				<input type="number" id="Periodicidad" name="Periodicidad" placeholder="<?= SIMUtil::get_traduccion('', '', 'Periodicidad', LANGSESSION); ?>" class="col-xs-12 mandatory" title="<?= SIMUtil::get_traduccion('', '', 'Recordarcada', LANGSESSION); ?>" value="<?php echo $EditNotificacionLocal["Periodicidad"]; ?>"><?= SIMUtil::get_traduccion('', '', 'minutos', LANGSESSION); ?>
			</div>
		</div>
...
<table id="simple-table" class="table table-striped table-bordered table-hover">
	...
	<tbody id="listacontactosanunciante">
		<?php
		$r_documento = &$dbo->all("NotificacionLocal", "IDClub = '" . $frm["IDClub"]  . "' and IDModulo ='102' and IDDetalle = '" . $frm[$key] . "'");

...
<form class="form-horizontal formvalida" role="form" method="post" id="frm<?php echo $script; ?>" action="<?php echo SIMUtil::lastURI() ?>" enctype="multipart/form-data">
	...
	<div class="form-group first ">
		...
		<div class="col-xs-12 col-sm-6">
			...
			<div class="col-sm-8">
				<input type="text" id="FechaFin" name="FechaFin" placeholder="<?= SIMUtil::get_traduccion('', '', 'FechaFin', LANGSESSION); ?>" class="col-xs-12 calendar" title="<?= SIMUtil::get_traduccion('', '', 'FechaFinPublicacion', LANGSESSION); ?>" value="<?php echo $frm["FechaFin"] ?>">
			</div>
		</div>
	...
	<div class="form-group first ">
		...
		<div class="col-xs-12 col-sm-6">
			<label class="col-sm-4 control-label no-padding-right" for="form-field-1"> <?= SIMUtil::get_traduccion('', '', 'Imagenlistadodotacion', LANGSESSION); ?> </label>

			<div class="col-sm-8">
				<? if (!empty($frm[ImagenDotacion])) {
					echo "<img src='" . BANNERAPP_ROOT . "$frm[ImagenDotacion]' width=55 >";
				?>
					<a href="<? echo $script . ".php?action=delfoto&foto=$frm[ImagenDotacion]&campo=ImagenDotacion&id=" . $frm[$key]; ?>" class="ace-icon glyphicon glyphicon-trash">&nbsp;</a>
				<?
				} // END if
				?>

				<input name="ImagenDotacion" id=file class="" title="<?= SIMUtil::get_traduccion('', '', 'Imagenlistadodotacion', LANGSESSION); ?>" type="file" size="25" style="font-size: 10px">

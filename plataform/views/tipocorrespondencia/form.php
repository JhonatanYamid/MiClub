					...
					<form class="form-horizontal formvalida" role="form" method="post" id="frm<?php echo $script; ?>" action="<?php echo SIMUtil::lastURI() ?>" enctype="multipart/form-data">
						...
						<div class="form-group first ">
							...
							<div class="col-xs-12 col-sm-6">
								...
								<div class="col-sm-8">
									<select name="IDCategoriaCorrespondencia" id="IDCategoriaCorrespondencia" class="form-control mandatory" title="<?= SIMUtil::get_traduccion('', '', 'Categoria', LANGSESSION); ?>">
										<option value=""></option>
										<?php
										$sql_cat_corresp = string;
										$sql_cat_corresp = "Select * From CategoriaCorrespondencia Where IDClub = '" . SIMUser::get("club") . "' order by Nombre";
										$qry_cat_corresp = $dbo->query($sql_cat_corresp);
										while ($r_cat_corresp = $dbo->fetchArray($qry_cat_corresp)) : ?>
											<option value="<?php echo $r_cat_corresp["IDCategoriaCorrespondencia"]; ?>" <?php if ($r_cat_corresp["IDCategoriaCorrespondencia"] == $frm["IDCategoriaCorrespondencia"]) echo "selected";  ?>><?php echo $r_cat_corresp["Nombre"]; ?></option>
										<?php
										endwhile;    ?>
									</select>
								</div>

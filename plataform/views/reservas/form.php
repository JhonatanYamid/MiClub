<div class="widget-box transparent" id="recent-box">
    ...
    <?php
            ...
            $PermiteAdicionales = $datos_servicio_config[PermiteAdicionarServicios];
    ...
    <div class="row">
        <div class="col-xs-12">
            ...
            <form class="form-horizontal formvalida" role="form" method="post" id="frmReservaGeneral" name="frmReservaGeneral" action="<?php echo SIMUtil::lastURI()?>">
                ...
                <div class="form-group first ">
                    ...
                    <?php endif; ?>
                     ...
                     <div class="col-xs-12 col-sm-6">
                        ...
                        <div class="col-sm-8">
                            <input type="tel" name="Telefono" value="" id="Telefono" class=" " title="Socio">
                        </div>
                ...
                <?php
                if($PermiteAdicionales=='S'):                
                    ?>
                    <div class="form-group first ">
                        <div class="col-xs-12 col-sm-6">
                            <label class="col-sm-4 control-label no-padding-right" for="form-field-1"> Adicionales Socio </label>

                            <div class="col-sm-8">
                                <?php
                                $sql = "SELECT * FROM ServicioPropiedad WHERE IDClub = '".SIMUser::get("club")."' AND IDServicio =  $_GET[ids] AND Publicar = 'S'";
                                $query = $dbo->query($sql);
                                while ($row = $dbo->fetchArray($query))
                                {
                                    ...
                                    <div class="form-group first ">
                                        ...
                                        $checkgroup = "";
                                        $checkgroup .= '<script language="JavaScript">
                                        ...
                                        $checkgroup .= "<table id='simple-table' class='table table-striped table-bordered table-hover'>
                                                            <tr>
                                                                <td> <input type=\"hidden\" name=\"IDCaracteristica[]\"value=\"$row[IDServicioPropiedad]\"> ";
                                        $columnas = 0;
                                        ...
                                        {                                       
                                            $columnas++;
                                            if($row[Tipo] == "Radio"):
                                                $checkgroup .= "<label class=\"radiogroup\">
                                                                    <input type=\"radio\" onclick=\"uncheckRadio(this)\" name=\"ServicioAdicional_$row[IDServicioPropiedad][]\"  value=\"". $rowProductos[IDServicioAdicional] ."\" ";                                                    
                                            ...
                                            elseif($row[Tipo] == "Checkbox"):
                                                $checkgroup .= "<label class=\"checkgroup\">
                                                                    <input type=\"checkbox\" name=\"ServicioAdicional_$row[IDServicioPropiedad][]\"  value=\"". $rowProductos[IDServicioAdicional] ."\" ";                                                    
                                                                    $checkgroup .= "> ". $rowProductos['Nombre'];
                                                                    $checkgroup .= "
                                                                </label>";
                        ...
                        <div class="col-xs-12 col-sm-6">
                            ...
                            <div class="col-sm-8">
                                ...
                                <select name="IDAuxiliar" id="IDIDAuxiliar">
                                    <option value="">Seleccione</option>
                                    <?php
                                            $sql_auxiliar = "Select * From Auxiliar Where IDServicio = '".$ids."' AND Activo  = 'S'";
                                            $r_auxiliar = $dbo->query($sql_auxiliar);
                                            while($row_auxiliar = $dbo->fetchArray($r_auxiliar)):
                                                //Verifico si el auxiliar esta disponible en el dia
                                                $dia_fecha= date('w', strtotime($fecha));
                ...
                $MaximoInvitadosSalon = $datos_servicio_config[MaximoInvitadosSalon];
                ...
                <!-- VALIDAMOS QUE EXISTA UN TALONERA PARA EL SERVICIO Y SI SE QUIERE PAGAR -->
                <?php
                    $SQLTalonera = "SELECT * FROM Talonera WHERE (IDServicio = $_GET[ids] AND Activa = 1 AND CantidadEntradas > 0) || (IDClub = '".SIMUser::get("club")."' and TodosLosServicios = 1 AND Activa = 1 AND CantidadEntradas > 0 )";
                    $QRYTalonera = $dbo->query($SQLTalonera);
                    if($dbo->rows($QRYTalonera) > 0):
                        ?>
                        <div class="form-group first ">
                            <div class="col-xs-12 col-sm-6">
                                <label class="col-sm-4 control-label no-padding-right" for="form-field-1"> Hay Taloneras habilitadas para realizar el pago de la reserva, desea hacerlo? </label>

                                <div class="col-sm-8">
                                    <?php echo SIMHTML::formradiogroup(SIMResources::$sinoNum, $frm["PagoConTalonera"], 'PagoConTalonera', "class='input mandatory'") ?>
                                </div>
                            </div>
                        </div>
                        <?php
                ...
                ?>
                <!-- VALIDAMOS SI HAY ABONOS PARA PAGAR LA RESERVA -->
                <?php
                    $SQLAbonos = "SELECT * FROM  PorcentajeAbono WHERE IDServicio = $_GET[ids] AND Activo  = 1";
                    $QRYAbonos = $dbo->query($SQLAbonos);

                    if($dbo->rows($QRYAbonos)):
                        ?>
                        <div class="form-group first ">
                            <div class="col-xs-12 col-sm-6">
                                <label class="col-sm-4 control-label no-padding-right" for="form-field-1"> Si desea abonar a la reserva, especifique que abono </label>

                                <div class="col-sm-8">
                                    <select name="IDPorcentajeAbono">
                                        <option value="0">[Sin Abono]</option>
                                        <?php
                                        while($DatosAbono = $dbo->fetchArray($QRYAbonos)):
                                            ?>
                                            <option value="<?php echo $DatosAbono[IDPorcentajeAbono]; ?>"><?php echo $DatosAbono[Nombre]; ?></option>
                                            <?php
                                        endwhile;
                                        ?>
                                    </select>

		...
		<div class="widget-header">
           ...
           <table id="simple-table" class="table table-striped table-bordered table-hover">
           	<tr>
                ...
                <td>
                <?php
                	$sql_entregados = "Select * From PqrFuncionario Where IDClub =  '".SIMUser::get( "club" )."' ".$condiciones."";
 ...
 <script>
        ...
        var barChartData = {
			...
			$sql_area = "Select IDArea,Nombre From AreaFuncionario Where IDClub = '".SIMUser::get( "club" )."' and Activo = 'S' ";
			$qry_area = $dbo->query($sql_area);
			while($row_area = $dbo->fetchArray($qry_area)):
				$array_areas[] = "'".$row_area["Nombre"]."'";
				// Consulto total de pqr
				$sql_total = "Select Count(IDPqr) as Total From PqrFuncionario Where IDClub = '".SIMUser::get( "club" )."' and IDArea = '".$row_area["IDArea"]."' " . $condiciones;
        ...
        data: {
            datasets: [{
				...
				while($row_estado = $dbo->fetchArray($qry_estado)):
					...
					$sql_total = "Select Count(IDPqr) as Total From PqrFuncionario Where IDClub = '".SIMUser::get( "club" )."' and IDPqrEstado = '".$row_estado["IDPqrEstado"]."' " . $condiciones;
					$qry_total = $dbo->query($sql_total);
					$row_total = $dbo->fetchArray($qry_total);
        ...
        window.onload = function() {
            ...
            window.myBar = new Chart(ctx, {
                ...
                options: {
					...
					animation: { duration: 500, easing: "easeOutQuart", onComplete: function () { 
						...
						for (var i = 0; i < dataset.data.length; i++)
						 { 
						 	var model = dataset._meta[Object.keys(dataset._meta)[0]].data[i]._model, scale_max = dataset._meta[Object.keys(dataset._meta)[0]].data[i]._yScale.maxHeight; ctx.fillStyle = '#444'; 

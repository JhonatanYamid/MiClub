...
//Dato list Vacunado
$queryVacunado = $dbo->query("SELECT * FROM Vacunado WHERE IDUsuario=" . $get['id']);
...
<div class="widget-box transparent" id="recent-box">
	<div class="widget-header">
		<h4 class="widget-title lighter smaller">
			<i class="ace-icon fa fa-users orange"></i><?= SIMUtil::get_traduccion('', '', 'InformacióndeVacuna2', LANGSESSION); ?> <?= SIMUtil::get_traduccion('', '', SIMReg::get("title"), LANGSESSION); ?>
		</h4>
	</div>
				...
				<div class="col-xs-12">
					...
					<div class="widget-header widget-header-large registrarDosis">
						<h3 class="widget-title grey lighter">
							...
							<?= SIMUtil::get_traduccion('', '', 'RegistrarCertificadoDigital', LANGSESSION); ?>
						</h3>
					</div>
					...
					<form class="form-horizontal formvalida" role="form" method="post" id="frmCertificadoDigital" action="<?php echo SIMUtil::lastURI() ?>" enctype="multipart/form-data">
						<?php if ($r_ConfiguraionVacunacion2['MostrarEstaVacunado'] == 'S') { ?>
							<div class="form-group first ">
								<div class="col-xs-12 col-sm-6">
									<label class="col-sm-4 control-label no-padding-right" for="form-field-1"> <?= SIMUtil::get_traduccion('', '', 'Elusuarioestávacunado', LANGSESSION); ?>? </label>
									<div class="col-sm-8">
							...
							<div class="col-xs-12 col-sm-6">
								...
								<div class="col-sm-8">
									<?php if ($ArchivoVacuna != '') { ?>
										...
										<a href="<? echo $script . ".php?action=del-archivo-vacunado&archivo=" . $ArchivoVacuna . "&id=" . $Vacunado['IDVacunado'] . "&IDUsuario=" . $get["id"] ?>" class="ace-icon glyphicon glyphicon-trash">&nbsp;</a>
									<?php } ?>
						...
						<div class="clearfix form-actions">
							<div class="col-xs-12 text-center">
								...
								<input type="hidden" name="IDUsuario" id="IDUsuario" value="<?php echo $get["id"] ?>" />
					...
					while ($r_Dosis = $dbo->assoc($q_Dosis)) {
						$query = $dbo->query("SELECT V.* FROM Vacuna2 V WHERE V.IDUsuario=" . $get['id'] . " AND V.IDClub = " . SIMUser::get('club') . " AND IDDosis = '" . $r_Dosis['IDDosis'] . "' ORDER BY IDVacuna ASC");
						...
						<form class="form-horizontal formvalida" role="form" method="post" id="frm<?php echo $script . $cont; ?>" action="<?php echo SIMUtil::lastURI() ?>" enctype="multipart/form-data">
							...
							<div class="form-group first ">
								<div class="col-xs-12 col-sm-6">
									...
									<div class="col-sm-8">
										<input type="text" id="EntidadDosis<?= $cont ?>" name="EntidadDosis" placeholder="<?= SIMUtil::get_traduccion('', '', 'Entidad', LANGSESSION); ?>" class="col-xs-12" title="<?= SIMUtil::get_traduccion('', '', 'Entidad', LANGSESSION); ?>" value="<?php echo $frm_vacuna["EntidadDosis"]; ?>">
									</div>
								</div>
							</div>
							<div class="form-group first ">
								<div class="col-xs-12 col-sm-6">
									...
									<div class="col-sm-8">
										<input type="text" id="Lugar<?= $cont ?>" name="Lugar" placeholder="<?= SIMUtil::get_traduccion('', '', 'LugarVacunación', LANGSESSION); ?>" class="col-xs-12" title="<?= SIMUtil::get_traduccion('', '', 'LugarVacunación', LANGSESSION); ?>" value="<?php echo $frm_vacuna["Lugar"]; ?>">
									</div>
								</div>
								<div class="col-xs-12 col-sm-6">
									<label class="col-sm-4 control-label no-padding-right" for="form-field-1"><?= SIMUtil::get_traduccion('', '', 'Fechacita', LANGSESSION); ?></label>
									<div class="col-sm-8">
										<input type="text" id="FechaCitaVacuna<?= $cont ?>" name="FechaCitaVacuna" placeholder="<?= SIMUtil::get_traduccion('', '', 'Fechacita', LANGSESSION); ?>" class="col-xs-12 calendar" title="<?= SIMUtil::get_traduccion('', '', 'Fechacita', LANGSESSION); ?>" value="<?php echo $frm_vacuna["FechaCitaVacuna"]; ?>">
									</div>
								</div>
							</div>
							<div class="form-group first ">
								<div class="col-xs-12 col-sm-6">
									<label class="col-sm-4 control-label no-padding-right" for="form-field-1"><?= SIMUtil::get_traduccion('', '', 'Certificado', LANGSESSION); ?></label>
									<div class="col-sm-8">
										<?php if ($frm_vacuna["Certificado"]) { ?>
											...
											<a href="<?= $script . ".php?action=del-vacuna2-image&archivo=" . $frm_vacuna["Certificado"] . "&num_img=Primera&id=" . $frm_vacuna["IDVacuna"] . "&IDUsuario=" . $frm_vacuna["IDUsuario"] ?>" class="ace-icon glyphicon glyphicon-trash">&nbsp;</a>
										<?php } ?>
										<br />
										<br />
										<input type="file" id="Certificado<?= $cont ?>" name="Foto" class="" title="<?= SIMUtil::get_traduccion('', '', 'Certificado', LANGSESSION); ?>" value="<?= $frm_vacuna["Certificado"]; ?>">
									</div>
								</div>
							</div>
							<?php
										...
										<div class="col-sm-8">
											...
											if ($r['Tipo'] == "imagen") { ?>
												...
												<?php if ($q_VacunaCampoVacunacion2) { ?>
													...
													<a href="<?= $script . ".php?action=del-vacunaCampoVacuncion2-image&archivo=" . $q_VacunaCampoVacunacion2 . "&num_img=Primera&id=" . $frm_vacuna["IDVacuna"] . "&campo=" . $r['IDCampoVacunacion'] . "&IDUsuario=" . $frm_vacuna["IDUsuario"] ?>" class="ace-icon glyphicon glyphicon-trash">&nbsp;</a>
							...
							<div class="clearfix form-actions">
								<div class="col-xs-12 text-center">
									...
									<input type="hidden" name="IDDosis[]" id="IDDosis" value="<?php echo $r_Dosis["IDDosis"] ?>" />
									...
									<button class="btn btn-info btnEnviar" type="button" rel="frm<?php echo $script . $cont; ?>">
										<i class="ace-icon fa fa-check bigger-110"></i>
										<?php echo $titulo_accion; ?> <?= SIMUtil::get_traduccion('', '', 'Dosis', LANGSESSION); ?>
									</button>

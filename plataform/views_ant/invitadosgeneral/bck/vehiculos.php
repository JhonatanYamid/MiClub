<form class="form-horizontal formvalida" role="form" method="post" id="EditVehiculo<?php echo $script; ?>" action="<?php echo SIMUtil::lastURI()?>" enctype="multipart/form-data">
                  ...
                  if( $_GET[IDVehiculo] )
                  {
                          ...
                          ?>
                          <input type="hidden" name="IDVehiculo" id="IDVehiculo" value="<?php echo $EditVehiculo[IDVehiculo]?>" />
                          <?php
                  }
                  ?>
							...
							<div  class="form-group first ">
                                ...
                                <label class="col-sm-4 control-label no-padding-right" for="form-field-1"> Tipo Vehiculo:  </label>
								  <div class="col-sm-8">
										  <?php echo SIMHTML::formPopUp( "TipoVehiculo" , "Nombre" , "Nombre" , "IDTipoVehiculo" , $EditVehiculo["IDTipoVehiculo"] , "[Seleccione el Tipo]" , "popup mandatory" , "title = \"Tipo Vehiculo\"" )?>
									</div>
								</div>
              ...
              <table id="simple-table" class="table table-striped table-bordered table-hover">
                      ...
                      <?php
															...
															if((int)$frm[$key]>0){
															$r_datos =& $dbo->all( "Vehiculo" , "IDInvitado = '" . $frm[$key]  ."'");
                      ...
                      <tr class="<?php echo SIMUtil::repetition() ? 'row0' : 'row1'?>">
                              <td align="center" width="64">
                                      <a href="<?php echo $script.".php" . "?action=edit&id=" . $frm[$key] ."&IDVehiculo=".$r->IDVehiculo?>&tabinvitado=vehiculos&refiere=porteria" class="ace-icon glyphicon glyphicon-pencil"></a>                                </td>
                              <td><?php echo $r->Placa; ?></td>
                              <td><?php echo $r->FechaTecnomecanica; ?></td>
                              <td><?php echo $r->FechaSeguro; ?></td>
                              <td align="center" width="64">
                                      <a class="ace-icon glyphicon glyphicon-remove confirma_eliminacion" href="?mod=<?php echo SIMReg::get( "mod" )?>&action=EliminaVehiculo&id=<?php echo $frm[$key];?>&IDVehiculo=<? echo $r->IDVehiculo ?>&tabinvitado=vehiculos" ></a>                                </td>
                      </tr>
                      <?php
                      }
										}

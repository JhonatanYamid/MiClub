<?
	...
	if( SIMNet::req("action") == "search" )
	{
		...
		if(!empty($_GET["IDInvitacion"]) || !empty($_GET["Placa"]) ){
			$url_search = "?IDInvitacion=".$_GET["IDInvitacion"]."&Placa=" . $_GET["Placa"];
		}
	}
...
?>
	...
	<div class="widget-box transparent" id="recent-box">
       ...
       <div class="page-header">
							<h1>
								<a href="#" id="veringresado">Ver Invitados que ya ingresaron</a>
								<small>
		...
		<script type="text/javascript">
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					...
					url:'includes/async/invitadosespeciales.async.php<?=$url_search ?>',
					datatype: "json",
					colNames:['Registrar Ingreso','Documento Invitado', 'Nombre Invitado', 'Tipo','Placa','Fecha Desde', 'Fecha de Hasta', 'Socio', 'Accion','Creado Por'],
					colModel:[
						{name:'Ingreso',index:'Ingreso', align:"center"},
						{name:'NumeroDocumento',index:'NumeroDocumento', align:"left"},
						{name:'Nombre',index:'Nombre', align:"center"},
						{name:'Tipo',index:'Tipo', align:"center"},
						{name:'Placa',index:'Placa', align:"center"},
						{
							name:'FechaInicio',
							index:'FechaInicio',
							align:"center",
						...
						{name:'CreadoPor',index:'CreadoPor', align:"left"},
					...
					width:855,
					multiselect: true,
					editurl: "includes/invitadosespeciales.async.php",
								 ...
								 if(icol==1){
									...
									//var IDClub = $(this).attr("lang");
										if(confirm("Esta seguro de registrar el ingreso?")){
											jQuery.ajax( {
												"type" : "POST",
												"data" : { "IDSocioInvitadoEspecial" : rowid },
												"dataType" : "json",
									...
									if(icol==2 || icol==3 || icol==4 || icol==5 || icol==6 || icol==7 ){
										$("#detalle"+rowid).click();
										window.open("invitadosespeciales.php?action=edit&id="+rowid,'_blank');
										//window.location.href="invitadosespeciales.php?action=edit&id="+rowid;
										//$('.fancybox').click();
										//$.fancybox.open
				...
				jQuery(grid_selectoringresado).jqGrid({
					...
					url:'includes/async/invitadosespecialingreso.async.php<?=$url_search ?>',
					datatype: "json",
					 ...
					 if(icol==1){
									...
									//var IDClub = $(this).attr("lang");
										if(confirm("Esta seguro de registrar la salida?")){
											jQuery.ajax( {
												"type" : "POST",
												"data" : { "IDSocioAutorizacion" : rowid,"Tipo" : "Salida" },
												"dataType" : "json",

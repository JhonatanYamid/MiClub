				...
				<div class="row">
					<div class="col-xs-12">
						...
						if (count($array_parametro) > 0) :
							$otros_parametros = "&" . implode("&", $array_parametro);
						endif;
						?>
						<a href="procedures/excel-reporteingresosalidavehiculos.php?IDClub=<?php echo SIMUser::get("club") . $otros_parametros; ?>"><img src="assets/img/xls.gif">Exportar</a>
...
?>
<script>
	colNames = ['Nombre', 'Placa Vehiculo', 'Tipo Vehiculo', 'Movimiento', 'Fecha/Hora', 'Usuario'];
	colModel = [{
			name: 'Nombre',
			index: 'Nombre',
			align: "left",
...
<script type="text/javascript">
	jQuery(function($) {
		...
		jQuery(grid_selectoringresado).jqGrid({
			...
			url: 'includes/async/reporteingresosalidavehiculos.async.php<?= $url_search ?>',
			datatype: "json",
			colNames: colNames,
			colModel: colModel,
			rowNum: 100,
			...
			width: 855,
			multiselect: true,
			editurl: "includes/reporteingresosalidavehiculos.async.php",

				...
				<div class="col-xs-12">
					...
					<form class="form-horizontal formvalida" role="form" method="post" id="frm<?php echo $script; ?>" action="<?php echo SIMUtil::lastURI() ?>" enctype="multipart/form-data">
						...
						<div class="form-group first ">
							...
							<div class="col-xs-12 col-sm-6">
								<label class="col-sm-4 control-label no-padding-right" for="form-field-1"> <?= SIMUtil::get_traduccion('', '', 'Empleado', LANGSESSION); ?> </label>

								<div class="col-sm-8">
									<input type="text" id="AccionInvitado" name="AccionInvitado" placeholder="<?= SIMUtil::get_traduccion('', '', 'Documento', LANGSESSION); ?>" class="col-xs-12 autocomplete-ajax-usuarios" title="<?= SIMUtil::get_traduccion('', '', 'Documento', LANGSESSION); ?>">
									...
									<select name="SocioInvitado[]" id="SocioInvitado" class="col-xs-8" multiple style="height:300px">
										...
										$array_invitados = explode("|||", $frm["IDUsuario"]);
										sort($array_invitados);
										foreach ($array_invitados as $id_socio) :
											if ($id_socio > 0) :
												$nombre_socio = utf8_encode($dbo->getFields("Usuario", "Nombre", "IDUsuario = '" . $id_socio . "'"));
										?>
												<option value="<?php echo "usuario-" . $id_socio; ?>"><?php echo $nombre_socio; ?></option>
										<?php
											endif;
										endforeach;
										?>

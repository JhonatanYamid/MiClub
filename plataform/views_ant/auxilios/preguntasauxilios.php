<form class="form-horizontal formvalida" role="form" method="post" id="EditPregunta<?php echo $script; ?>" action="<?php echo SIMUtil::lastURI() ?>" enctype="multipart/form-data">
    ...
    if ($_GET[IDPreguntaAuxilios]) {
        $EditPregunta = $dbo->fetchAll("PreguntaAuxilios", " IDPreguntaAuxilios = '" . $_GET["IDPreguntaAuxilios"] . "' ", "array");
        $action = "ModificaPregunta";
    ?>
        <input type="hidden" name="IDPreguntaAuxilios" id="IDPreguntaAuxilios" value="<?php echo $EditPregunta[IDPreguntaAuxilios] ?>" />
    <?php
    }
    ?>
    ...
    <div class="clearfix form-actions">
        <div class="col-xs-12 text-center">
            ...
            <input type="hidden" name="IDAuxilios" id="IDAuxilios" value="<?php echo $frm[$key] ?>" />
...
<table id="simple-table" class="table table-striped table-bordered table-hover">
    ...
    <tbody id="listacontactosanunciante">
        ...
        $r_documento = &$dbo->all("PreguntaAuxilios", "IDAuxilios = '" . $frm[$key]  . "'");
        ...
        ?>
            ...
            <tr class="<?php echo SIMUtil::repetition() ? 'row0' : 'row1' ?>">
                <td align="center" width="64">
                    <a href="<?php echo $script . ".php" . "?action=edit&id=" . $frm[$key] . "&IDPreguntaAuxilios=" . $r->IDPreguntaAuxilios ?>&tabencuesta=formulario" class="ace-icon glyphicon glyphicon-pencil"></a>
                </td>
                ...
                <td align="center" width="64">
                    <a class="ace-icon glyphicon glyphicon-remove confirma_eliminacion" href="?mod=<?php echo SIMReg::get("mod") ?>&action=EliminaPregunta&id=<?php echo $EditPregunta[$key]; ?>&IDPreguntaAuxilios=<? echo $r->IDPreguntaAuxilios ?>&tabpreguntaauxilios=formulario"></a>
                </td>
            </tr>
        <?php
        }

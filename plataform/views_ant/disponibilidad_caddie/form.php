...
<div class="widget-box transparent" id="recent-box">
	<div class="widget-header">
		<h4 class="widget-title lighter smaller">
			<i class="ace-icon fa fa-users orange"></i>DISPONIBILIDAD CADDIE  - <?=$dbo->getFields( "ServicioMaestro" , "Nombre" , "IDServicioMaestro = '" . $datos_servicio[$ids]["IDServicioMaestro"] . "'") ?>
		</h4>
			...
			<div class="row">
				<div class="col-xs-12">
			      ...
			      <div id="CaddieoDisponibilidad">

          <form name="frmdisponibilidadCaddie" id="frmdisponibilidadCaddie" action="?mod=<?php echo SIMReg::get( "mod" )?>" method="post" class="formvalida" enctype="multipart/form-data">

                  <?php
                  $action = "insert";
	              if( $_GET[IDCaddieDisponibilidad] )
                  {
	                          $EditDisponibilidad =$dbo->fetchAll("CaddieDisponibilidad"," IDCaddieDisponibilidad = '".$_GET[IDCaddieDisponibilidad]."' ","array");
                          $action = "update";
                          ?>
                          <input type="hidden" name="IDCaddieDisponibilidad" id="IDCaddieDisponibilidad" value="<?php echo $EditDisponibilidad[IDCaddieDisponibilidad]?>" />
                          <?php
                  }
                  ?>
                  ...
                  <tr>
                    ...
                    <?php
					echo $dias_aplica = $dbo->getFields( "CaddieDisponibilidadDetalle" , "IDDia" , "IDCaddieDisponibilidad = '" . $EditDisponibilidad["IDCaddieDisponibilidad"] . "'")?>
                    <!--
                  ...
                  <tr>
                    <td colspan="2"><table width="100%" border="0">
                      <tbody>
						...
						if ((int)$EditDisponibilidad["IDCaddieDisponibilidad"]>0):
							$contador_hora_guardada=1;
							$sql_servicio_disponibilidad = $dbo->query("Select * From CaddieDisponibilidadDetalle Where IDCaddieDisponibilidad = '".$EditDisponibilidad["IDCaddieDisponibilidad"]."'");
                  ...
                  <tr>
                    <td colspan="2" class="columnafija">Aplica a los Caddiees</td>
                    </tr>
                  <tr>
                    <td colspan="2" class="columnafija">
                        	...
                        	<tr>
									...
									<?php
                                    $Caddie_aplica = $dbo->getFields( "CaddieDisponibilidadDetalle" , "IDCaddie" , "IDCaddieDisponibilidad = '" . $EditDisponibilidad["IDCaddieDisponibilidad"] . "'");
                                    $array_Caddie_guardados=explode("|",$Caddie_aplica);
																		$r_Caddie_servicio =& $dbo->all( "Caddie2" , "IDServicio = '" .$_GET["ids"]  ."'");
																		$contador_elementos = 0;
																		  while( $r = $dbo->object( $r_Caddie_servicio ) ): ?>
                                      <td>
                                      <input type="checkbox" name="IDCaddie[]" id="IDCaddie" value="<?php echo $r->IDCaddie; ?>" <?php if(in_array( $r->IDCaddie,$array_Caddie_guardados)) echo "checked"; ?>><?php echo $r->Nombre; ?>
                                       </td>
                                    <?php
									$contador_elementos++;

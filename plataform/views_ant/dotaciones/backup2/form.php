...
<script type="text/javascript">
	jQuery(function($) {
		...
		$("#btExporta").click(function() {
			...
			window.location.href = "./procedures/excel-Dotacion.php?id=<?php echo $_GET["id"]; ?>&" + '&FechaInicio=' + Inicio + '&FechaFin=' + Fin;
		...
		?>
			$('#myTABS a[href="#invitaciones"]').on('click', function(e) {
				jQuery(grid_selector).jqGrid({
					url: 'includes/async/get_Dotacion.async.php?id=<?php echo $_GET["id"]; ?>',
					datatype: "json",
					...
					echo "colNames: ['$usuario','$fecha','" . str_replace(",", "','", implode(",", $array_preguntas)) . '\'],'; ?>
					colModel: [{
							name: 'Nombre',
							index: 'Nombre',
							width: '190',

<form class="form-horizontal formvalida" role="form" method="post" id="EditPregunta<?php echo $script; ?>" action="<?php echo SIMUtil::lastURI() ?>" enctype="multipart/form-data">
	...
	if ($_GET["IDPreguntaEncuesta2"]) {
		$EditPregunta = $dbo->fetchAll("PreguntaEncuesta2", " IDPreguntaEncuesta2 = '" . $_GET["IDPreguntaEncuesta2"] . "' ", "array");
		$action = "ModificaPregunta";
	?>
		<input type="hidden" name="IDPreguntaEncuesta2" id="IDPreguntaEncuesta2" value="<?php echo $EditPregunta[IDPreguntaEncuesta2] ?>" />
	<?php
	}
	?>
	...
	<div class="col-xs-12 col-sm-12">
		...
		<table id="simple-table" class="table table-bordered table-hover">
			<tr>
				...
				<td><?= SIMUtil::get_traduccion('', '', 'RespuestaCorrecta', LANGSESSION); ?>?</td>
				<!--<td>Ir a pregunta</td>-->
				<td><?= SIMUtil::get_traduccion('', '', 'Puntos', LANGSESSION); ?></td>
				<td><?= SIMUtil::get_traduccion('', '', 'Orden', LANGSESSION); ?></td>
			</tr>
			<?php
			$sql_opciones = "SELECT * FROM Encuesta2OpcionesRespuesta WHERE IDEncuesta2Pregunta = '" . $_GET["IDPreguntaEncuesta2"] . "'";
			...
			?>
				<tr>
					...
					<td><input type="radio" name="RespuestaCorrecta<?php echo $i; ?>" value="S" <?php if ($valor_opciones["RespuestaCorrecta"] == "S") echo "checked"; ?>>Si<input type="radio" name="RespuestaCorrecta<?php echo $i; ?>" value="N" <?php if ($valor_opciones["RespuestaCorrecta"] == "N") echo "checked"; ?>></td>
					<!--<td></td>-->
					<td><input type="text" name="Puntos<?php echo $i; ?>" value="<?php echo $valor_opciones["Puntos"]; ?>"></td>
	...
	<div class="clearfix form-actions">
		<div class="col-xs-12 text-center">
			...
			<input type="hidden" name="IDEncuesta2" id="IDEncuesta2" value="<?php echo $frm[$key] ?>" />
...
<table id="simple-table" class="table table-striped table-bordered table-hover">
	...
	<tbody id="listacontactosanunciante">
		...
		$r_documento = &$dbo->all("PreguntaEncuesta2", "IDEncuesta2 = '" . $frm[$key]  . "'");
		...
		?>
			...
			<tr class="<?php echo SIMUtil::repetition() ? 'row0' : 'row1' ?>">
				<td align="center" width="64">
					<a href="<?php echo $script . ".php" . "?action=edit&id=" . $frm[$key] . "&IDPreguntaEncuesta2=" . $r->IDPreguntaEncuesta2 ?>&tabencuesta=formulario" class="ace-icon glyphicon glyphicon-pencil"></a>
				</td>
				...
				<td align="center" width="64">
					<a class="ace-icon glyphicon glyphicon-remove confirma_eliminacion" href="?mod=<?php echo SIMReg::get("mod") ?>&action=EliminaPregunta&id=<?php echo $frm[$key]; ?>&IDPreguntaEncuesta2=<? echo $r->IDPreguntaEncuesta2 ?>&tabpregunta=formulario"></a>
				</td>
			</tr>
		<?php
		}

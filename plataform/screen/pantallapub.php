...
if (count($servicios_relacionados) > 1) :
	...
	if (!empty($siguiente_servicio)) :
		...
		header("Refresh: 30; URL='pantallapub.php?action=new&ids=" . $siguiente_servicio . "&IDClub=" . $_GET[IDClub] . "'");
	else :
		$id_servicio = $servicios_relacionados[0];
		header("Refresh: 30; URL='pantallapub.php?action=new&ids=" . $id_servicio . "&IDClub=" . $_GET[IDClub] . "'");
	endif;
else :
	...
	<script>
		jQuery(document).ready(function() {
			...
			<?php if (count($elementos[$ids]) >= 1) :	?>
				setTimeout("location.href='pantallapub.php?ids=<?php echo $ids ?>&pagina=<?php echo $proxima_pagina; ?>&action=new&id=<?php echo rand(0, 10000); ?>&IDClub=<?php echo $_GET[IDClub]; ?>'", 40000);
			<?php endif; ?>

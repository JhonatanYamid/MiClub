...
if (count($servicios_relacionados) > 1) :
	...
	if (!empty($siguiente_servicio)) :
		...
		header("Refresh: 30; URL='pantallaGolf.php?action=new&ids=" . $siguiente_servicio . "&IDClub=" . $_GET[IDClub] . "'");
	else :
		$id_servicio = $servicios_relacionados[0];
		header("Refresh: 30; URL='pantallaGolf.php?action=new&ids=" . $id_servicio . "&IDClub=" . $_GET[IDClub] . "'");
	endif;
else :
	...
	<table class="fixed">
		<thead>
			<tr>
				...
				<th colspan="2">
					<!--  aca esta el titulo -->
					<h3 class="fecha" style="font-weight:bold; font-family: 'Arial', serif; font-size:40px !important"> <? echo $servicio_personalizado . " " . strtolower(SIMUtil::tiempo(date("Y-m-d"))) ?></h3>
				</th>
	...
	<script>
		jQuery(document).ready(function() {
			...
			<?php if (count($elementos[$ids]) >= 1) :	?>
				setTimeout("location.href='pantallaGolf.php?ids=<?php echo $ids ?>&pagina=<?php echo $proxima_pagina; ?>&action=new&id=<?php echo rand(0, 10000); ?>&IDClub=<?php echo $_GET[IDClub]; ?>'", 40000);
			<?php endif; ?>

...
class SIMPasarelaPagos
{
    ...
    public function sonda_place_to_pay($IDSocio = "")
    {
        ...
        while ($row_transaccion = $dbo->fetchArray($r_transacciones)) :
            ...
            $datos_club_configuracion_general = $dbo->fetchAll("ConfiguracionClub", " IDClub = '" . $row_transaccion["IDClub"] . "' ", "array");
            ...
            if ($row_transaccion["IDClub"] == 51) {
                ...
                if ($datos_club["IsTestPlaceToPay"] == 1) {
                    // $url_place_to_pay = ENDPOINT_PLACE_TO_PAY_TEST;
                    $url_place_to_pay = $datos_club_configuracion_general["EndpointPlaceToPayPruebas"];
                } else {
                    // $url_place_to_pay = ENDPOINT_PLACE_TO_PAY;
                    $url_place_to_pay = $datos_club_configuracion_general["EndpointPlaceToPayProduccion"];
                }
            }
            ...
            $Authorization = $respuesta["payment"][0]["authorization"];
            ...
            $update_transacc = "UPDATE PeticionesPlacetoPay
										SET estado_transaccion = '" . $respuesta["status"]["status"] . "',Authorization='" . $Authorization . "',
    ...
    public function PlaceToPay($DATOS)
    {
        ...
        $datos_club = $dbo->fetchAll("Club", " IDClub = '" . $IDClub . "' ", "array");
        $datos_club_configuracionGeneral = $dbo->fetchAll("ConfiguracionClub", " IDClub = '" . $IDClub . "' ", "array");
        ...
        //calcular iva con el porcentaje ingresado en la configuracion
        if ($datos_club_configuracionGeneral["PorcentajeIva"] > 0) {
            $valor_iva_base = (float)$DATOS['valor']  / 1.19;
            $valor_iva = (float)$DATOS['valor'] - $valor_iva_base;
        }

        //SELECCIONAR TIPO MONEDA COP O CUALQUIER OTRA
        if (!empty($datos_club_configuracionGeneral["Moneda"])) {
            $currency = $datos_club_configuracionGeneral["Moneda"];
        } else {
            $currency = "USD";
        }
        ...
        if ($datos_club["IsTestPlaceToPay"] == 1) {
            // $url_place_to_pay = ENDPOINT_PLACE_TO_PAY_TEST;
            $url_place_to_pay = $datos_club_configuracionGeneral["EndpointPlaceToPayPruebas"];
        } else {
            //$url_place_to_pay = ENDPOINT_PLACE_TO_PAY;
            $url_place_to_pay = $datos_club_configuracionGeneral["EndpointPlaceToPayProduccion"];
        }
    ...
    public function respuestaPlaceToPay($referencia)
    {
        ...
        $datos_club_configuracion_general = $dbo->fetchAll("ConfiguracionClub", " IDClub = '" . $datos_transaccion["IDClub"] . "' ", "array");
        ...
        $Authorization = $DATA["payment"][0]["authorization"];

        $sql_actualiza = "UPDATE PeticionesPlacetoPay SET estado_transaccion = '" . $estado_transaccion . "', razon='" . $mensaje . "', request_id = '" . $requestId . "',Authorization='" . $Authorization . "'  WHERE referencia = '" . $reference . "'";
    ...
    public function CredibancoSolicitudV2($Datos)
    {
        ...
        //insertamos los datos en la tabla CarreraAtleticaLaPradera para el club La pradera
        if ($Datos[Modulo] == "CarreraAtleticaLaPradera") {

            $sql_insertar_datos = "INSERT INTO CarreraAtleticaLaPradera(IDSocio,Modulo,Externo,NombreContactoEmergencia,TelefonoContactoEmergencia,Categoria,Valor,NombreParticipante,TallaCamisa,Accion,TelefonoParticipante,Kit,Correo,CargoSocio,Terminos,InvitadoDe,NombreInvitado,EdadParticipante,Club,FechaTrCr) VALUES('" . $Datos[IDSocio] . "','" . $Datos[Modulo] . "','" . $Datos[Externo] . "','" . $Datos[NombreContactoEmergencia] . "','" . $Datos[TelefonoContactoEmergencia] . "','" . $Datos[Categoria] . "','" . $Datos[valor] . "','" .  $Datos[NombreParticipante] . "','" . $Datos[TallaCamisa]  . "','" . $Datos[Accion] . "','" . $Datos[TelefonoParticipante] . "','" . $Datos[Kit] . "','" . $Datos[Correo] . "','" . $Datos[CargoSocio] . "','" . $Datos[Terminos] . "','" . $Datos[InvitadoDe] . "','" . $Datos[NombreInvitado] . "','" . $Datos[EdadParticipante] . "','" .  $Datos[Club] . "',NOW())";

            $dbo->query($sql_insertar_datos);
            $Datos[extra1] =  $dbo->lastID("IDCarreraAtleticaLaPradera");
        }


        //insertamos los datos en la tabla EscuelaSerenaDelMar para el club el SDM
        if ($Datos[Modulo] == "EscuelasSerenaDelMar") {
            $sql_insertar_datos = "INSERT INTO EscuelaSerenaDelMar(IDSocio,IDClub,Clase,Nombres,Apellidos,DocumentoIdentificacion,Edad,Celular,CorreoElectronico,TipoDeSalud,Parentesco,NombresPadres,ApellidosPadres,CedulaPadres,CelularPadres,CorreoPadres,Edificio,NumeroDeTorre,NumeroDeApto,Valor,NumeroDocumentoIdentificacion,FechaTrCr) VALUES('" . $Datos[IDSocio] . "','" . $Datos[extra2] . "','" . $Datos[Clase] . "','" . $Datos[Nombres] . "','" . $Datos[Apellidos] . "','" .  $Datos[DocumentoIdentificacion] . "','" . $Datos[Edad]  . "','" . $Datos[Celular] . "','" . $Datos[CorreoElectronico] . "','" . $Datos[TipoDeSalud] . "','" . $Datos[Parentesco] . "','" . $Datos[NombresPadres] . "','" . $Datos[ApellidosPadres] . "','" . $Datos[CedulaPadres] . "','" . $Datos[CelularPadres] . "','" . $Datos[CorreoPadres] . "','" . $Datos[Edificio] . "','" . $Datos[NumeroDeTorre] . "','" . $Datos[NumeroDeApto] . "','" . $Datos[valor] . "','" . $Datos[NumeroDocumentoIdentificacion] . "',NOW())";
            $dbo->query($sql_insertar_datos);

            $Datos[extra1] =  $dbo->lastID("IDEscuelaSerenaDelMar");

            //notificamos por correo
            $Asunto = "Registro Nuevo Clubes Mi Comunidad";
            $CorreoRegistro = "andysortega@stralink.com,anare1236@gmail.com";
            $IDClub = $Datos[extra2];
            $Mensaje = "Se ha creado un nuevo registro de clubes deportivos Mi Comunidad Serena del Mar <br>
            Nombre:" . $Datos[Nombres] . "<br>
            Apellidos:" . $Datos[Apellidos] . "<br>
            para validar ingrese a la plataforma.";
        ...
        $insert = "INSERT INTO PagoCredibanco (IDClub, IDSocio, Modulo, NumeroTransaccion, NumeroDocumento, ValorPago, Email, NombreSocio, ApellidoSocio, NumeroAccion, Telefono, Direccion, Genero, FechaTransaccion, reserved12, reserved13, reserved14, Factura, FechaTrCr, UsuarioTrCr, UsuarioTrEd)
									...
									'" . $datos_socio[Nombre] . "', '" . $datos_socio[Apellido] . "', '" . $datos_socio[Accion] . "', '" . $datos_socio[Celular] . "', '" . $datos_socio[Direccion] . "', '" . $datos_socio[Genero] . "', NOW(), '" . $Datos[extra1] . "', '" . $version . "','$Propina','" . $Datos['Factura'] . "',NOW(), 'SIMPasarelaPagos', 'API')";
        $dbo->query($insert);
        $lastid = $dbo->lastID();
    ...
    public function CredibancoRespuestaV2($orden)
    {
        ...
        if ($DATOS["errorCode"] == 0) {
            if (isset($DATOS['cardAuthInfo'])) {
                $FormaPago = $DATOS['cardAuthInfo']['paymentSystem'];
            } else {
                $FormaPago = $DATOS['paymentWay'];
            }
            ...
            switch ($Modulo) {
                case "EscuelasSerenaDelMar":

                    $sql_pedido = "UPDATE EscuelaSerenaDelMar
											SET Pagado = '" . $pagado . "',
											...
											WHERE IDEscuelaSerenaDelMar = '" . $datos_pago["reserved12"] . "' and IDClub = '" . $datos_pago["IDClub"] . "'";
                    $dbo->query($sql_pedido);
                    break;

                case "CarreraAtleticaLaPradera":

                    $sql_pedido = "UPDATE CarreraAtleticaLaPradera
                                                SET Pagado = '" . $pagado . "',
                                                    CodigoPago='" . $datos_pago["NumeroTransaccion"] . "',
                                                    EstadoTransaccion='" . $estadoTransaccion . "',
                                                    FechaTransaccion= NOW(),
                                                    ...
                                                    MedioPago='CREDIBANCO V API',
                                                    TipoMedioPago='CREDIBANCO V API'
                                                WHERE IDCarreraAtleticaLaPradera = '" . $datos_pago["reserved12"] . "'";
                    $dbo->query($sql_pedido);
                    break;
                ...
                case "Domicilio":
                    ...
                    //BOGOTA TENIS CLUB IMPRIMO EN DOMICILIOS DESPUES DE HABER PAGADO
                    /* if ($datos_pago["IDClub"] == 72 && $pagado == "S") {
                        $id_domicilio = $datos_pago["reserved12"];
                        $impreso = $dbo->getFields("Domicilio", "Impreso", "IDDomicilio = $id_domicilio");
                        if (empty($impreso)) {
                ...
                case "FacturasPraderaPotosi":
                    ...
                    //SOLO SI ES APROBADA LA TRANSACCION SE ENVIA A LA BASE DE DATOS DE LA PRADERA
                    if ($estadoTransaccion == "A") {
                ...
                case "Taloneras":
                    ...
                    if ($pagado == 'S') :
                        ...
                        $SQLBuscarTalonera = "SELECT * FROM SocioTalonera WHERE IDSocioTalonera = '$datos_pago[reserved12]' ";
                        $QRYBuscarTalonera = $dbo->query($SQLBuscarTalonera);
                        $DatosSocioTalonera = $dbo->fetchArray($QRYBuscarTalonera);

                        $SaldoTotalPasarela = $DatosSocioTalonera[SaldoTotalPasarela];

                        $actualiza = ", Activo = 1 , SaldoMonedero='$SaldoTotalPasarela'";
                    endif;
                    ...
                    if ($pagado == 'S') {
                        ...
                        foreach ($arr_Factura as  $facturaPagada) {
                            if (!empty($facturaPagada)) :
                                $Factura = explode('|', $facturaPagada);
                                $sqlFactura = "update CarteraVencida set FechaPago = CURRENT_DATE() where Factura = '" . $Factura[0] . "'";
    ...
    public function Wompi($DATOS)
    {
        ...
        $DatosAuteticacion = SIMPasarelaPagos::AutenticacionWompi($DATOS["extra2"]);
    ...
    public function confirmacion($IDClub, $IDPagosWompi, $Tipo = "")
    {
        $dbo = SIMDB::get();
        ...
        if ($Tipo == "DonacionCampestreMedellin") {
            $datos_club[PrivKeyWompi] = "prv_prod_qhbiK48DtbTJy7boEuZ2DLKgdyqiHe3Q";
        }
    ...
    public function RespuestaWompi($IDClub, $IDPagosWompi, $ReferenciaPago, $Sonda = 0, $WompiBilletera = "", $Tipo = "")
    {
        $dbo = SIMDB::get();
        //echo "Wompi billetera hola:" . $WompiBilletera;


        if ($Sonda == 0) :
            $DATA = SIMPasarelaPagos::TransaccionWompi($IDClub, $ReferenciaPago);
        else :
            $DATA = SIMPasarelaPagos::confirmacion($IDClub, $IDPagosWompi, $Tipo);
        endif;
        ...
        switch ($Modulo):
            ...
            case "DonacionCampestreMedellin":

                $sql_donacion = "UPDATE DonacionCampestreMedellin
                SET Pagado = '" . $pagado . "',
                    CodigoPago='" . $datos_pago["NumeroTransaccion"] . "',
                    EstadoTransaccion='" . $estadoTransaccion . "',
                    CodigoRespuesta='" . $Estado . "',
                    Estado = '$Estado',
                    MetodoPago='" . $MetodoPago . "' 
                WHERE IDPagosWompi = '" . $datos_pago["IDPagosWompi"] . "' and IDClub = '" . $datos_pago["IDClub"] . "'";


                $dbo->query($sql_donacion);
            ...
            case "Taloneras":
                if ($pagado == "S") {
                    $SQLBuscarTalonera = "SELECT * FROM SocioTalonera WHERE IDSocioTalonera = '$datos_pago[IDReserva]' ";
                    $QRYBuscarTalonera = $dbo->query($SQLBuscarTalonera);
                    $DatosSocioTalonera = $dbo->fetchArray($QRYBuscarTalonera);

                    $SaldoTotalPasarela = $DatosSocioTalonera[SaldoTotalPasarela];
                    $estado_talonera = ", Activo = 1 , SaldoMonedero='$SaldoTotalPasarela'";
                } else {
        ...
        //WOMPI BILLETERA
        if (!empty($WompiBilletera)) {

            //echo "Entro en el if";

            $UpdatePagos = "UPDATE TransaccionWompi 
            SET Estado = '$Estado', 
                MensajeEstado = '$MensajeEstado', 
                ReferenciaWompi = '$ReferenciaWompi', 
                FechaTransaccion = '$FechaTransaccion', 
                ...
                Response = '$Response',
                FechaTrEd = NOW(),
                UsuarioTrEd = 'PASARELA WOMPI BILLETERA'
                WHERE IDTransaccionWompi = $IDPagosWompi";
            // echo $UpdatePagos;
            $dbo->query($UpdatePagos);


            $datos_pago = $dbo->fetchAll("TransaccionWompi", " IDTransaccionWompi = '$IDPagosWompi' ", "array");

            $IDModulo = $datos_pago[IDModulo];
            //SACAMOS LA INFORMACION IMPORTANTE DE CADA ESTADO QUE NO RESULTA EN CREDIBANCO.
            switch ($Estado):
    ...
    public function TransaccionPayWay($IDClub, $ReferenciaPago)
    {
        $dbo = SIMDB::get();

        $datos_club = $dbo->fetchAll("ConfiguracionClub", " IDClub = '$IDClub' ", "array");

        $solicitud = "https://merchant.paymentsway.co/api/external/v1/getbyexternal/" . $ReferenciaPago;
    ...
    public function confirmacionPayWay($IDClub, $ReferenciaPago)
    {
        $dbo = SIMDB::get();
    ...
    public function RespuestaPayWay($IDClub, $IDPagosPayWay, $ReferenciaPago, $Sonda = 0)
    {
        ...
        $DATA = SIMPasarelaPagos::TransaccionPayWay($IDClub, $ReferenciaPago);
        $DATA = $DATA[0];
        // $DATA['idstatus']['id'] = 34;
        ...
        $UpdatePagos = "UPDATE PagosPayWay 
                        SET Estado = '$Estado', 
                            ...
                            WHERE ReferenciaTransaccion = $ReferenciaPago";
        $dbo->query($UpdatePagos);

        $datos_pago = $dbo->fetchAll("PagosPayWay", " ReferenciaTransaccion = '$ReferenciaPago' ", "array");
        $Modulo = $datos_pago['Tipo'];
        //SACAMOS LA INFORMACION IMPORTANTE DE CADA ESTADO QUE NO RESULTA EN CREDIBANCO.
        /*
        Estados Pay Way
        ...
        */
        ...
        switch ($Modulo):
            ...
            case "Ecaddie":
                ...
                $IDSolicitudCaddieRappi = $datos_pago['IDReserva'];

                $datos_solicitud = $dbo->fetchAll("SolicitudCaddieRappi", " IDSolicitudCaddieRappi = '" . $IDSolicitudCaddieRappi . "' ", "array");

                if ($pagado == "S") {
                    if ((int)$datos_solicitud["IDCaddiesEcaddie"] > 0) {
                        $EstadoT = " EstadoSolicitud = 'Confirmada' , ";
                    } else {
                        $EstadoT = " EstadoSolicitud = 'EsperandoCaddie' , ";
                    }
                } else {
                    $condicion_no_aprobado = " and EstadoSolicitud <> 'EsperandoCaddie' and  EstadoSolicitud <> 'Confirmada' ";
                }

                if ($IDSolicitudCaddieRappi == 585) {
                    $pagado = "S";
                    $EstadoT = " EstadoSolicitud = 'EsperandoCaddie' , ";
                }


                $actualizaEcaddie = "UPDATE SolicitudCaddieRappi SET " . $EstadoT . " Pagado = '$pagado',
                                    FechaTrEd = NOW()
                                    WHERE IDSolicitudCaddieRappi ='$IDSolicitudCaddieRappi' " . $condicion_no_aprobado;

                $dbo->query($actualizaEcaddie);

                if ($pagado == "S") {
                    require LIBDIR . "SIMWebServiceEcaddie.inc.php";
                    SIMWebServiceEcaddie::enviar_push_caddie($IDSolicitudCaddieRappi);
                }
    ...
    /* .... LUKAPAY .... */
    public function getTokenLukaPay($IDClub)
    {
        $dbo = SIMDB::get();
        ...
        if ($IsTestLukaPay == 1) {
            $url = $URLLukaPayPruebas . "servicio/login";
        } else {
            $url = $URLLukaPay . "servicio/login";
        }

        $arrKey = array("Username" => "$UsuarioLukaPay", "Password" => "$ClaveLukaPay");
        $jsonKey = json_encode($arrKey);
        $curl = curl_init();
        curl_setopt_array($curl, array(
            CURLOPT_URL => $url,
            CURLOPT_ENCODING => '',
        ...
        if ($IsTestLukaPay == 1) {
            $response = explode(":", $response[6]);
        } else {
            $response = explode(":", $response[5]);
        }
        $Token = $response[1];
        return $Token;
    }
    public function getLinkLukaPay($IDClub, $IDSocio = "", $IDUsuario = "", $IDPagosLukaPay)
    {
        $dbo = SIMDB::get();

        $datos_club = $dbo->fetchAll("ConfiguracionClub", " IDClub = '$IDClub' ", "array");
        $datos_socio = $dbo->fetchAll("Socio", "IDSocio = $IDSocio");
        $datos_reserva = $dbo->fetchAll("PagosLukaPay", "IDPagosLukaPay = $IDPagosLukaPay");
        // $datos_reserva['Valor'] = 10;
        $Token = trim(SIMPasarelaPagos::getTokenLukaPay($IDClub));
        $URLLukaPay = $datos_club['URLLukaPay'];
        $URLLukaPayPruebas = $datos_club['URLLukaPayPruebas'];
        $IsTestLukaPay = $datos_club['IsTestLukaPay'];

        if ($IsTestLukaPay == 1) {
            $url = $URLLukaPayPruebas . "link/token";
        } else {
            $url = $URLLukaPay . "link/token";
        }
        $jsonKey = '{"MonedaCodigo":"USD","Monto":"' . $datos_reserva['Valor'] . '","MetodosPago":[],"Referencia":"' . $datos_socio['NumeroDocumento'] . '","IdTraza":"' . $datos_reserva['ReferenciaTransaccion'] . '","Email":"' . $datos_socio['CorreoElectronico'] . '"}';
        $curl = curl_init();
        curl_setopt_array($curl, array(
            CURLOPT_URL => $url,
            CURLOPT_ENCODING => '',
    ...
    public function TransaccionLukaPay($IDClub, $ReferenciaPago)
    {
        $dbo = SIMDB::get();
        ...
        if ($IsTestLukaPay == 1) {
            $url = $URLLukaPayPruebas . 'transaccion?trazaId=' . $ReferenciaPago;
        } else {
            $url = $URLLukaPay . 'transaccion?trazaId=' . $ReferenciaPago;
        }
        $curl = curl_init();

        $Claveb64 = base64_encode($UsuarioLukaPay . ":" . $ClaveLukaPay);
        curl_setopt_array($curl, array(
            CURLOPT_URL => $url,
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => '',
    ...
    public function RespuestaLukaPay($IDClub, $IDPagosLukaPay, $ReferenciaPago, $Sonda = 0)
    {
        $dbo = SIMDB::get();

        $DATA = SIMPasarelaPagos::TransaccionLukaPay($IDClub, $ReferenciaPago);
        $DATA = $DATA[0];

        // ACTUALIZAMOS LA TABLA DE PAGOS
        if ($DATA['Exitoso'] == 1) {
            $Response = base64_decode($_GET['r']);
            $Exito = 1;
            $Estado = "APROBADA";
        } else {
            $Response = base64_decode($_GET['e']);
            $Exito = 2;
            $Estado = "RECHAZADA";
        }

        $MensajeEstado = $DATA['Descripcion'];
        $ReferenciaLukaPay = $DATA['TransaccionId'];
        $FechaTransaccion = $DATA['idstatus']['createdat'];
        $MetodoPago = json_encode($DATA['MedioDePago']);
        $UrlRespuesta = '';
        $Response = json_encode($DATA);

        $UpdatePagos = "UPDATE PagosLukaPay 
                        SET Estado = '$Estado', 
                            MensajeEstado = '$MensajeEstado', 
                            ReferenciaLukaPay = '$ReferenciaLukaPay', 
                            FechaTransaccion = NOW(), 
                            MetodoPago = '$MetodoPago', 
                            UrlRespuesta = '$UrlRespuesta',
                            Response = '$Response',
                            FechaTrEd = NOW(),
                            UsuarioTrEd = 'PASARELA LUKAPAY'
                            WHERE ReferenciaTransaccion = $ReferenciaPago";
        $dbo->query($UpdatePagos);
        $datos_pago = $dbo->fetchAll("PagosLukaPay", " ReferenciaTransaccion = '$ReferenciaPago' ", "array");
        $Modulo = $datos_pago['Tipo'];
        //SACAMOS LA INFORMACION IMPORTANTE DE CADA ESTADO QUE NO RESULTA EN CREDIBANCO.
        /*
        Estados Luka Pay
        ...
        */
        ...
        switch ($Modulo):
            ...
            case "Reservas":
                ...
                $actualizaReserva = "UPDATE ReservaGeneral SET " . $EstadoT . " Pagado = '$pagado', MedioPago = 'LUKAPAY-$Estado',
                                    FechaTransaccion = NOW(), CodigoRespuesta = '$Estado',
                                    EstadoTransaccion = '$estadoTransaccion', FechaTrEd=NOW(),UsuarioTrEd='PASARELA LUKAPAY'
                                    WHERE IDReservaGeneral ='$reserva' " . $condicion_no_aprobado;
            ...
            case "Taloneras":
                ...
                $update = "UPDATE SocioTalonera SET EstadoTransaccion = '$estadoTransaccion', Pagado = '$pagado', MedioPago = 'LUKAPAY-$Estado' " . $estado_talonera . " WHERE IDSocioTalonera = $datos_pago[IDReserva]";
                $dbo->query($update);
    ...
    {
        $Token = SIMPasarelaPagos::TokenEpayco($IDClub);
    ...
    public function  set_transaccion_wompi($IDClub, $IDSocio, $IDUsuario, $IDModulo, $IDTransaccion, $reference)
    {
        ...
        if (($IDSocio > 0 || $IDUsuario > 0)) {
            ...
            $sql_insertar_transaccion = $dbo->query("INSERT INTO TransaccionWompi(IDClub,IDSocio,IDUsuario,IDModulo,IDTransaccion,reference,Estado,FechaTrCr)VALUES('$IDClub','$IDSocio', '$IDUsuario', '$IDModulo','$IDTransaccion', '$reference','PENDING',NOW())");
    ...
    public function RespuestaMercadoPago($IDClub, $IDSocio, $frm, $IDPagosMercadoPago)
    {
        $dbo = SIMDB::get();

        $DATA = $frm;
        $Estado = $DATA['status'];
        $ReferenciaPago = $frm['ID_REF'];
        $MensajeEstado = $DATA['collection_status'];
        $ReferenciaMercadoPago = $DATA['collection_id'];
        $FechaTransaccion = $DATA['date_created'];
        $MetodoPago = $DATA['payment_type'];
        $UrlRespuesta = '';
        $Response = json_encode($DATA);
        // $datos_pago = $dbo->fetchAll("PagosMercadoPago", "IDPagosMercadoPago=$IDPagosMercadoPago", "array");

        $UpdatePagos = "UPDATE PagosMercadoPago 
                        SET Estado = '$Estado', 
                            MensajeEstado = '$MensajeEstado', 
                            ReferenciaMercadoPago = '$ReferenciaMercadoPago', 
                            FechaTransaccion = NOW(), 
                            MetodoPago = '$MetodoPago', 
                            UrlRespuesta = '$UrlRespuesta',
                            Response = '$Response',
                            FechaTrEd = NOW(),
                            UsuarioTrEd = 'PASARELA MercadoPago'
                            WHERE ReferenciaTransaccion = $ReferenciaPago";
        $dbo->query($UpdatePagos);

        $datos_pago = $dbo->fetchAll("PagosMercadoPago", " ReferenciaTransaccion = '$ReferenciaPago' ", "array");
        $Modulo = $datos_pago['Tipo'];
        //SACAMOS LA INFORMACION IMPORTANTE DE CADA ESTADO.
        switch ($Estado) {
            case "approved":
                $estadoTx = "APROBADA";
                $estadoReserva = 1;
        ...
        switch ($Modulo):
            ...
            case "Reservas":
                ...
                $actualizaReserva = "UPDATE ReservaGeneral SET " . $EstadoT . " Pagado = '$pagado', MedioPago = 'MercadoPago-$estado',
                                    FechaTransaccion = NOW(), CodigoRespuesta = '$estado',
                                    EstadoTransaccion = '$estado', FechaTrEd=NOW(),UsuarioTrEd='PASARELA MercadoPago'
                                    WHERE IDReservaGeneral ='$reserva' " . $condicion_no_aprobado;
            ...
            case "Taloneras":
                ...
                $update = "UPDATE SocioTalonera SET EstadoTransaccion = '$estado', Pagado = '$pagado', MedioPago = 'MercadoPago-$estado' " . $estado_talonera . " WHERE IDSocioTalonera = $datos_pago[IDReserva]";
                $dbo->query($update);

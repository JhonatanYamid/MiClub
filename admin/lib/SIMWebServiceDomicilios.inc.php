...
class SIMWebServiceDomicilios
{
    ...
    public function get_producto_categoria($IDClub, $IDCategoria = "", $Tag = "", $Version = "", $IDRestaurante = "", $TipoApp, $IDSocio = "", $IDUSuario = "")
    {
        ...
        //CLUB LAKE HOUSE
        if ($IDClub == 233) {
            $datos_pedido_peticion = array();
            //LAKE HOUSE VALIDAMOS EL ESTADO DE LA TRANSACCION POR PLACETOPAY 




            //BUSCO EL CODIGO DE AMBIENTE
            $datos_ambientes = $dbo->fetchAll("RestauranteDomicilio" . $Version, " IDRestauranteDomicilio = '" . $IDRestaurante . "' ", "array");
                    ...
                    foreach ($datos_productos as $productos_todos) {
                        ...
                        foreach ($productos_todos->Producto as $info_producto) {
                            ...
                            if ($info_producto->Valor != 0) :
                                ...
                                $Doc_particular = $dbo->getFields("Socio", "NumeroDocumento", "IDSocio = '" . $IDSocio . "'");

                                //saco info del producto img y precio para particular
                                $foto_producto = $dbo->fetchAll("Producto" . $Version, " IDProductoExterno = '" . $info_producto->Codigo . "' ", "array");

                                //SI ES PARTICULAR SE LE SUMA EL PRECIO EN UN 10% MAS
                                if ($Doc_particular == "2222222") :
                                    $precio_particular =  $foto_producto["PrecioNoAfiliado"];
                                    $producto["Precio"] = (string)$precio_particular;
                                endif;
                                ...
                                if (!empty($foto_producto["Foto1"])) :
                                    if (strstr(strtolower($foto_producto["Foto1"]), "http://")) {
                                        $foto = $foto_producto["FotoDestacada"];
                                    } else {
                                        $foto = IMGPRODUCTO_ROOT . $foto_producto["Foto1"];
                                    }
                                endif;
                                ...
                                for ($i_foto = 1; $i_foto <= 6; $i_foto++) :
                                    ...
                                    if (!empty($foto_producto[$campo_foto])) :
                                        $array_dato_foto["Foto"] = IMGPRODUCTO_ROOT . $foto_producto[$campo_foto];
                                ...
                                foreach ($info_producto->Preferencias as $info_preferencias) {
                                    foreach ($info_preferencias->Preferencia as $info_preferencia) {



                                        if ($Nombre_cat != "Preferencias") {
                                            $Nombre_cat = "Preferencias";
                                            if ($contador_cat > 0) {
                                                array_push($response_carac_producto, $categoria_carac);
                                                $response_valores_carac = array();
                                            }

                                            $categoria_carac["IDCaracteristica"] = intval($info_producto->Codigo);
                                            $categoria_carac["TipoCampo"] = "Radio";
                                            $categoria_carac["EtiquetaCampo"] = "Preferencia ";
                                            $categoria_carac["Obligatorio"] = "N";
                                            $categoria_carac["CantidadMaximaSeleccion"] = "1";
                                        }

                                        $valores["OcultarPrecioEnCero"] = "S";
                                        $valores["IDCaracteristicaValor"] = "$info_preferencia->codigo" . "|" . "$info_preferencia->Descripcion" . "|" . "$info_preferencia->Tipo" . "|" . "$info_preferencia->Valor";
                                        $valores["Opcion"] = "$info_preferencia->Descripcion";

                                        if ($info_preferencia->Valor != "" || $info_preferencia->Valor > 0) {
                                            $valores["Precio"] = "$info_preferencia->Valor";
                                        } else {
                                            $valores["Precio"] = "0";
                                        }
    ...
    public function get_domicilio_socio($IDClub, $IDSocio, $Version = "", $IDRestaurante, $IDUsuario)
    {
        ...
        if ($dbo->rows($qry) > 0) {
            ...
            while ($r = $dbo->fetchArray($qry)) {
                ...
                if ($IDClub == 25 && $r["IDEstadoDomicilio"] == 00) {
                    $respuesta["message"] = SIMUtil::get_traduccion('', '', 'Noseencontraronregistros', LANG);
                    $respuesta["success"] = false;
                    $respuesta["response"] = null;
                }
    ...
    public function set_domicilio($IDClub, $IDSocio, $HoraEntrega, $ComentariosSocio, $DetallePedido, $Celular, $Direccion = "", $ValorDomicilio = "", $FormaPago = "", $Version = "", $IDRestaurante, $NumeroMesa = "", $CamposFormulario = "", $Propina = "", $IDUsuario = "", $Qr = "")
    {
        ...
        $SolicitaFechaDomicilio = $resp_config["response"][0]["SolicitaFechaDomicilio"];
        $IDConfiguracionDomicilios = $resp_config["response"][0]["IDConfiguracionDomicilios"];
        ...
        if (!empty($IDSocio) && !empty($HoraEntrega) && count($datos_pedido) > 0) {
            ...
            //VERIFICAMOS SI CUMPLE CON EL HORARIO DEFINIDO
 

// Separar la fecha y la hora en dos variables
list($fecha, $hora) = explode(' ', $HoraEntrega);


// Obtener el día de la semana y la hora actual
$dia_semana_actual = date('N', strtotime($fecha));
$hora_actual = strtotime($hora);

// Verificar si es día de semana y la hora está dentro del rango especificado
if ($dia_semana_actual >= 1 && $dia_semana_actual <= 5 && $hora_actual >= strtotime('7:00') && $hora_actual <= strtotime('13:00')) {
  // Es día de semana y la hora está dentro del rango especificado
  $permite="si";
}

// Verificar si es fin de semana y la hora está dentro del rango especificado
if (($dia_semana_actual == 6 || $dia_semana_actual == 7) && $hora_actual >= strtotime('7:00') && $hora_actual <= strtotime('16:00')) {
  // Es fin de semana y la hora está dentro del rango especificado
  $permite="si";
}

if($permite == "no" ):
                $respuesta["message"] = "Solo se pueden hacer  pedidos para horarios disponibles";
                $respuesta["success"] = false;
                $respuesta["response"] = null;
                return $respuesta;
endif;
*/
                ...
                $accion = $socio_datos["Accion"];
                $secuencia = $socio_datos["IDSecuenciaZeus"];
                ...
                $datos_ambientes = $dbo->fetchAll("RestauranteDomicilio" . $Version, " IDRestauranteDomicilio = '" . $IDRestaurante . "' ", "array");
                $datos_ambiente = $datos_ambientes["CodigoExternoAmbienteZeus"];
                $mesero = $datos_ambientes["CajeroLakeHouse"];
                ...
                if (count($array_Campos) > 0) :
                    foreach ($array_Campos as $id_valor_campo => $valor_campo) :
                        ...
                        if ($valor_campo["Valor"] == "null") :
                            $particular = "0";
                        else :
                            $particular = $valor_campo["Valor"];
                        endif;
                ...
                if ($IDClub == 233) :
                    ...
                    if ($datos_ambiente == "MR") :
                        $mesa = "002";
                    elseif ($datos_ambiente == "AT") :
                        $mesa = "021";
                    elseif ($datos_ambiente == "DP") :
                        $mesa = "022";
                    elseif ($datos_ambiente == "EV") :
                        $mesa = "023";
                    elseif ($datos_ambiente == "MC") :
                        $mesa = "034";
                    elseif ($datos_ambiente == "MV") :
                        $mesa = "035";
                    elseif ($datos_ambiente == "PT") :
                        $mesa = "120";
                    elseif ($datos_ambiente == "RS") :
                        $mesa = "121";
                    elseif ($datos_ambiente == "HY") :
                        $mesa = "HY001";
                    elseif ($datos_ambiente == "LG") :
                        $mesa = "LG01";
                    elseif ($datos_ambiente == "OL") :
                        $mesa = "OL01";
                    elseif ($datos_ambiente == "RT") :
                        $mesa = "RT01";
                    elseif ($datos_ambiente == "SB") :
                        $mesa = "SB01";
                    elseif ($datos_ambiente == "SG") :
                        $mesa = "SG01";
                ...
                $NumeroMesa = $mesa;
            /*       if ($FormaPago1 == "") :
                    $POS_CrearPedido = SIMWebServiceZeus::POS_CrearPedido($urlendpoint, $Token, $datos_pedido, $Celular, $Direccion, $cedula, $nombre_completo, $mesa, $mesero, $FormaPago1, $datos_ambiente, $Propina, $accion, $secuencia, $particular);
                    ...
                    if ($fact != "SUCESS") :
                        $respuesta["message"] = "Lo sentimos, no se pudo crear el pedido " . $POS_CrearPedido;
                        $respuesta["success"] = false;
                        $respuesta["response"] = null;
                        return $respuesta;
                    endif;
            ...
            if ($IDUsuario > 0) :
                ...
                $sql_domicilio = $dbo->query("Insert Into Domicilio" . $Version . " (IDClub, IDSocio, IDEstadoDomicilio, IDRestauranteDomicilio, Numero, Total, HoraEntrega, ComentariosSocio, Celular, Direccion, ValorDomicilio, FormaPago, DetallePedido, NumeroMesa, Propina, UsuarioTrCr, FechaTrCr)
                                        Values ('" . $IDClub . "','" . $IDSocio . "','1','" . $IDRestaurante . "','" . $siguiente_consecutivo . "','" . $GranTotal . "', '" . $HoraEntrega . "','" . $ComentariosSocio . "','" . $Celular . "','" . $Direccion . "','" . $ValorDomicilio . "','" . $FormaPago . "','" . $DetallePedido . "','" . $NumeroMesa . "','" . $Propina . "','App',NOW())");
            /*  $sql_domicilio = "Insert Into Domicilio" . $Version . " (IDClub, IDSocio, IDEstadoDomicilio, IDRestauranteDomicilio, Numero, Total, HoraEntrega, ComentariosSocio, Celular, Direccion, ValorDomicilio, FormaPago, DetallePedido, NumeroMesa, Propina, UsuarioTrCr, FechaTrCr)
                                        Values ('" . $IDClub . "','" . $IDSocio . "','1','" . $IDRestaurante . "','" . $siguiente_consecutivo . "','" . $GranTotal . "', '" . $HoraEntrega . "','" . $ComentariosSocio . "','" . $Celular . "','" . $Direccion . "','" . $ValorDomicilio . "','" . $FormaPago . "','" . $DetallePedido . "','" . $NumeroMesa . "','" . $Propina . "','App',NOW())"; */
            ...
            if (count($array_Campos) > 0) :
                foreach ($array_Campos as $id_valor_campo => $valor_campo) :
                    ...
                    //PROPINA VALOR INGRESADO PARA EL BOGOTA TENIS CLUB
                    if ($valor_campo["IDCampo"] == 50 || $valor_campo["IDCampo"] == 23) {
                        $ValorPropinaIngresado = $valor_campo["Valor"];
                    }
                endforeach;
            endif;
            ...
            foreach ($datos_pedido as $detalle_datos) :
                ...
                if ($Cantidad > 0) {
                    $inserta_detalle = "INSERT INTO DomicilioDetalle" . $Version . " (IDDomicilio, IDProducto, IDProductoExterno, NombreProducto, Comentario, Cantidad, ValorUnitario, Total, IDCategoriaZeus, CategoriaZeus, AmbienteZeus)
                                                        Values('" . $id_domicilio . "', '" . $IDProducto . "', '" . $IDProducto . "', '" . $NombreProducto . "','" . $Comentario . "','" . $Cantidad . "','" . $ValorUnitario . "','" . $Total . "','" . $codigo_categoria_producto . "','" . $nombre_categoria . "','" . $ambiente_usado . "')";
                    ...
                    if (count($datos_respuesta) > 0) :
                        ...
                        endforeach;
                        $TotalSumaEspeciales += $SumaEspeciales;
            ...
            if ($IDClub != 72) { // Para Bogota tenis Club no mando impresion hasta que se pague por la pasarela
                ...
                // SIMWebServiceDomicilios::imprime_recibo_domicilio_categoria($id_domicilio, $Version, "");
            ...
            //CONSULTO LOS TIPOS PAGO QUE TIENE EL CLUB
            $array_tipos_de_pago = array();
            $sql_tipo_pago = "SELECT IDTipoPago FROM DomicilioTipoPago WHERE IDConfiguracionDomicilio='$IDConfiguracionDomicilios'";
            $query_tipo_pago = $dbo->query($sql_tipo_pago);
            while ($DatosPago = $dbo->FetchArray($query_tipo_pago)) {
                array_push($array_tipos_de_pago, $DatosPago["IDTipoPago"]);
            }
            //SI NO HAY AL MENOS UN TIPO DE PAGO SELECCIONADO ENVIO EL CORREO
            if (empty($array_tipos_de_pago)) {
                SIMUtil::notifica_recibo_domicilio($id_domicilio, $Version);
            }
            /*  foreach ($array_tipos_de_pago as $tipo_pago) {
                if ($tipo_pago !=  $IDTipoPagoCrediBanco) {
                    SIMUtil::notifica_recibo_domicilio($id_domicilio, $Version);
                }
            } */
            ...
            //  $Valor = $GranTotal + $SumaEspeciales + $ValorPropinaIngresado;
            $Valor = $GranTotal + $TotalSumaEspeciales + $ValorPropinaIngresado;
            ...
            //CLUB GOLF DE PANAMA SE CALCULA LA PROPINA POR EL 7%
            if($IDClub==253){
                $ValorPropina = $Valor * 7 / 100;
            }
            ...
            //CAMBIO A MILES EL VALOR A PAGAR
            //$ValorReserva = number_format($ValorReserva, 0, ",", ".");

            $datos_reserva["ValorPagoTexto"] = $datos_club_otros["SignoPago"] . " " . number_format($ValorReserva, 0, ",", ".") . " " . $datos_club_otros["TextoPago"];
    ...
    public function elimina_pedido($IDClub, $IDSocio, $IDDomicilio, $Admin = "", $Razon = "", $Version = "")
    {
        ...
        if (!empty($IDClub) && !empty($IDSocio) && !empty($IDDomicilio)) {
            ...
            if (!empty($id_socio)) {
                ...
                if ($IDClub == 233) :
                    $domicilio_id = $dbo->getFields("Domicilio" . $Version, "IDEstadoDomicilio", "IDDomicilio = '" . $IDDomicilio . "'");
                    $pago_id = $dbo->getFields("Domicilio" . $Version, "IDTipoPago", "IDDomicilio = '" . $IDDomicilio . "'");
                    if ($domicilio_id == "1" and $pago_id <> 0) :
                        $respuesta["message"] = "Lo sentimos, no es posible cancelar el pedido.";
                        $respuesta["success"] = false;
                        $respuesta["response"] = null;
                        return $respuesta;
                    endif;
                ...
                if ($fechahora_actual > $hora_inicio_domicilio && empty($Admin) && $ValidarEliminarConTiempo) :
                    ...
                    //PARA EL BOGOTA TENIS NO3SE IMPRIME SI SE CANCELA
                    if ($IDClub != 72) {
    ...
    public function get_configuracion_domicilio($IDClub, $IDSocio = "",  $Fecha = "", $Version = "", $IDRestaurante = "", $IDUsuario = "")
    {
        ...
        if ($dbo->rows($qry) > 0) {
            ...
            while ($r = $dbo->fetchArray($qry)) {
                ...
                $info_socio = $dbo->fetchAll("Socio", " IDSocio = '" . $IDSocio . "' ", "array");
                ...
                if ($dbo->rows($qry_campos) > 0) {
                    while ($r_campos = $dbo->fetchArray($qry_campos)) {
                        ...
                        //VALIDAMOS QUE SOLO SE MUESTRE LA PREGUNTA DE NOMBRE DEL CLIENTE PARA LOS PARTICULARES EN LAKEHOUSE    

                        if ($IDClub == 233 || $IDClub == 00) :
                            if ($info_socio["NumeroDocumento"] == "2222222" and  $r_campos["Nombre"] == "Cliente") :
                            ...
                            elseif ($info_socio["NumeroDocumento"] != "2222222" and  $r_campos["Nombre"] == "Cliente") :
                            ...
                            elseif ($r_campos["Nombre"] == "Formas de pago") :
    ...
    public function get_fechas_domicilio($IDClub, $Version = "", $IDRestaurante = "")
    {
        ...
        for ($i = $fechaInicio; $i <= $fechaFin; $i += 86400) {
            ...
            if ($IDClub == 233) {
                //USAMOS DIAS DE SEMANA DEL 1-7
                //$fecha_cierre = 1; (NO MUESTRA LA FECHA) 
                //SACAMOS EL AMBIENTE DEL RESTAURANTE PARA FILTRAR MESAS         
                $datos_ambientes = $dbo->fetchAll("RestauranteDomicilio" . $Version, " IDRestauranteDomicilio = '" . $IDRestaurante . "' ", "array");
                $datos_ambiente = $datos_ambientes["CodigoExternoAmbienteZeus"];
                // obtener día de la semana en formato numérico (1: lunes, 2: martes, 3: miércoles, etc.)    
                $dia_semana = date('N', strtotime($fecha_validar));


                if ($datos_ambiente == "HY") :
                    //validamos lunes a ver si alguno es festivo
                    if ($dia_semana == 1) :
                        $festivo = SIMUtil::validaDiaFestivo($IDClub, $fecha_validar);
                ...
                //SI ES FESTIVO SE MUESTRA EN EL AMBIENTE DE TENIS
                elseif ($datos_ambiente == "DP") :

                    //validamos lunes,martes,miercoles, jueves y viernes a ver si alguno es festivo
                    if ($dia_semana <= 5) :
                        $festivo = SIMUtil::validaDiaFestivo($IDClub, $fecha_validar);
                ...
                //SI ES FESTIVO SE MUESTRA EN ROTONDA
                elseif ($datos_ambiente == "RT") :

                    //validamos lunes,martes,miercoles y jueves a ver si alguno es festivo
    ...
    public function get_horas_entrega($IDClub, $Fecha, $Version = "", $IDRestaurante = "")
    {
        ...
        if ($dbo->rows($qry) > 0) {
            ...
            while ($r = $dbo->fetchArray($qry)) {
                ...
                if ($IDClub == 233) {
                    //SACAMOS EL AMBIENTE DEL RESTAURANTE PARA FILTRAR RL AMBIENTE MR         
                    $datos_ambientes = $dbo->fetchAll("RestauranteDomicilio" . $Version, " IDRestauranteDomicilio = '" . $IDRestaurante . "' ", "array");
                    $datos_ambiente = $datos_ambientes["CodigoExternoAmbienteZeus"];


                    //SI ES FESTIVO SE MUESTRA EN LAKE HOUSE (HY19)
                    $festivo = SIMUtil::validaDiaFestivo($IDClub, $Fecha);
                    if ($datos_ambiente == "HY") :
                    ...
                    if (date('N', strtotime($Fecha)) != 6) {
                        ...
                        $nueva_fecha = date('Y-m-d', strtotime($Fecha . ' +1 day'));

                        //SI ES FESTIVO SE MUESTRA EN LAKE HOUSE (MIRADOR)
                        $festivo = SIMUtil::validaDiaFestivo($IDClub, $nueva_fecha);
                        if ($datos_ambiente == "MR") :
                            if ($festivo == true) {
                                $hora_fin = '21:00';
                            } else {
    ...
    public function imprime_recibo_domicilio($IDDomicilio, $Version = "", $Tipo = "")
    {
        ...
        $PermiteDescripcionImpresoraDomicilios = $dbo->getFields("ConfiguracionDomicilios" . $Version, "PermiteDescripcionImpresoraDomicilios", "IDClub = '" . $datos_domicilio["IDClub"] . "'");
        ...
        if (!empty($datos_domicilio["IDDomicilio"]) && !empty($IPImpresora) && !empty($PuertoImpresora)) :
            try {
                ...
                while ($r = $dbo->fetchArray($qry)) {
                    ...
                    if ($PermiteDescripcionImpresoraDomicilios == "N") {
                        $DescripcionProductos = "";
                    } else {
                        $DescripcionProductos = $dbo->getFields("Producto" . $Version, "Descripcion", "IDProducto = '" . $r["IDProducto"] . "'");
                    }

                    $pedido .= $r["Producto"] = utf8_encode($dbo->getFields("Producto" . $Version, "Nombre", "IDProducto = '" . $r["IDProducto"] . "'") . " " . $DescripcionProductos);
    ...
    public function imprime_recibo_domicilio_categoria($IDDomicilio, $Version = "", $Tipo = "")
    {
 ...
 */
        ...
        $dbo = &SIMDB::get();
        //SACO EL ID DEL PRODUCTO
        $array_idcategoria = array();
        $sql_domicilio_detalle = "SELECT IDProducto FROM DomicilioDetalle" . $Version . " WHERE IDDomicilio='" . $IDDomicilio . "'";
        $r_domicilio_detalle = $dbo->query($sql_domicilio_detalle);
        while ($row_domicilio_detalle = $dbo->FetchArray($r_domicilio_detalle)) {

            //CON EL ID DEL PRODUCTO BUSCO EL ID DE LA CATEGORIA Y CREO UN ARRAY
            $sql_producto_categoria = "SELECT IDCategoriaProducto FROM ProductoCategoria  WHERE IDProducto = '" . $row_domicilio_detalle["IDProducto"] . "' LIMIT 1";
            $r_producto_categoria = $dbo->query($sql_producto_categoria);
            $row_producto_categoria = $dbo->FetchArray($r_producto_categoria);
            if (!in_array($row_producto_categoria["IDCategoriaProducto"], $array_idcategoria)) {
                array_push($array_idcategoria, $row_producto_categoria["IDCategoriaProducto"]);
            }
        }



        foreach ($array_idcategoria as  $idcategoria) {
            ...
            //Averiguar si la categoria tiene el puerto y la ip impresora
            $datos_categoria = $dbo->fetchAll("CategoriaProducto" . $Version, " IDCategoriaProducto  = '" . $idcategoria . "' ", "array");

            if (!empty($datos_categoria["Ipimpresora"]) && !empty($datos_categoria["PuertoImpresora"])) {
                $IPImpresora = $datos_categoria["Ipimpresora"];
                $PuertoImpresora = $datos_categoria["PuertoImpresora"];
            }
            ...
            if (!empty($datos_domicilio["IDDomicilio"]) && !empty($IPImpresora) && !empty($PuertoImpresora)) :
                ...
                try {
                    ...
                    $FormaPago = $datos_domicilio["FormaPago"];
                    $Propina = $datos_domicilio["Propina"];
                    $Restaurante =  $dbo->getFields("RestauranteDomicilio" . $Version, "Nombre", "IDRestauranteDomicilio = '" . $datos_domicilio["IDRestauranteDomicilio"] . "'");
                    $pedido = "";
                    $sql = "SELECT * FROM DomicilioDetalle" . $Version . " DD,Producto" . $Version . " P,ProductoCategoria" . $Version . " PC WHERE IDDomicilio = '" . $IDDomicilio . "' AND PC.IDCategoriaProducto='" . $idcategoria . "' AND P.IDProducto=PC.IDProducto AND DD.IDProducto=PC.IDProducto";
                    $qry = $dbo->query($sql);
                    while ($r = $dbo->fetchArray($qry)) {
                        $pedido .= $r["Producto"] = utf8_encode($dbo->getFields("Producto" . $Version, "Nombre", "IDProducto = '" . $r["IDProducto"] . "'"));
                        $pedido .= ":" . $r["Cantidad"] . "\n" . "Comentario: " . utf8_encode($r["Comentario"]) . "\n";
                    ...
                    foreach ($array_ip as $IPImpresora) {
                        ...
                        $printer->setJustification(Printer::JUSTIFY_CENTER);
                        $printer->setTextSize(1, 1);
                        $printer->text(SIMUtil::get_traduccion('', '', 'Restaurante', "Es") . ":" . $Restaurante . "\n");
                        $printer->text(SIMUtil::get_traduccion('', '', 'Numero', "Es") . ":" . $numero_pedido . "\n");
                        $printer->text(SIMUtil::get_traduccion('', '', 'NombreSocio', "Es") . ":" . $nombre_socio . "\n");
                        $printer->setTextSize(2, 2);
                        $printer->text(SIMUtil::get_traduccion('', '', 'NumeroAccion', "Es") . ":" . $accion_socio . "\n", True);
                        ...
                        echo "Num pedido:" . $numero_pedido . "\n";
                        echo "Nombre Socio:" . $nombre_socio . "\n";
                        echo "accio:" . $accion_socio . "\n";
                        echo "cel:" . $celular_socio . "\n";
                        echo "hora:" .  $hora_solicitud . " \n";
                        echo "Direccion:" . $Direccion . " \n";
                        echo "fpago" . $FormaPago . " \n"; */
                        ...
                        // echo SIMUtil::get_traduccion('', '', 'NumeroMesa', "Es") . ":" . $NumeroMesa . " \n";

                        //echo "Propina (" . $datos_config["LabelPropina"] . "): " . $Propina . " \n";


                        // echo SIMUtil::get_traduccion('', '', 'DescripcionPedido', "Es") . ":" . "\n\n";
            ...
            while ($r = $dbo->fetchArray($qry)) {
                ...
                if ($r["IDTipoPago"] == 1 || $r["IDTipoPago"] == 4 || $r["IDTipoPago"] == 5) :  // payU
                    ...
                    elseif ($r["EstadoTransaccion"] == "A" || $r["EstadoTransaccion"] == "4" || $r["EstadoTransaccion"] == "AUTHORISED" || $r["EstadoTransaccion"] == "Aprobada") :
                        SIMUtil::notifica_recibo_domicilio($IDDomicilio, $Version);
                            ...
                            if ($repuesta["success"]) {
                                ...
                                switch ($estado) {
                                    ...
                                    case "2":
                                        ...
                                        $respuesta["success"] = true;
                                        $respuesta["response"] = null;
                                        //ENVIO NOTIFICACION CUANDO SE HAGA EL PAGO DE CREDIBANCO
                                        SIMUtil::notifica_recibo_domicilio($IDDomicilio, $Version);
                                        if ($IDClub == 72) {
                                            $impreso = $dbo->getFields("Domicilio", "Impreso", "IDDomicilio = $IDDomicilio");
                                            if (empty($impreso)) {

                                                SIMWebServiceDomicilios::imprime_recibo_domicilio_categoria($IDDomicilio, $Version, "");
                                            }
                                        }
                                        break;
                                    case "3":
        ...
        if (!empty($IDClub) && !empty($IDSocio) && !empty($IDDomicilio) && !empty($IDTipoPago)) {
            ...
            if (!empty($id_reserva)) {
                ...
                if ($IDClub == 233) {
                    ...
                    //BUSCAMOS TODOS LOS PEDIDOS PAGADOS POR PASARELA
                    $sql = "SELECT * FROM Domicilio" . $Version . " WHERE IDTipoPago = '11' or IDTipoPago = '22' and EstadoTransaccion<> 'Enviada a zeus' and IDClub = '" . $IDClub . "'";
                    $qry = $dbo->query($sql);
                    ...
                    if ($dbo->rows($qry) > 0) {
                        ...
                        while ($r = $dbo->fetchArray($qry)) {
                            ...
                            $IDDomicilio_peticion = $r["IDDomicilio"];
                            $FormaPago_peticion = $r["IDTipoPago"];
                            $datos_pedido_peticion = $r["DetallePedido"];

                            $datos_pedido_peticion = trim(preg_replace('/\s+/', ' ', $datos_pedido_peticion));
                            $datos_pedido_peticion = json_decode($datos_pedido_peticion, true);


                            $Propina_peticion = $r["Propina"];
                            //FECHA Y HORA DEL PEDIDO
                            $Hora_entrega = $r["HoraEntrega"];
                            $IDSocio_peticion = $r["IDSocio"];
                            $IDRestaurante_peticion = $r["IDRestauranteDomicilio"];

                            //Base Produccion zeus
                            $urlendpoint_peticion = "http://200.1.126.78:1080/wsZeusGenerico/ServiceWS.asmx?WSDL";
                            $usuariuozeus = "userpos2"; // WSPos1
                            $clavezeus = "userpos2022";
                            $socio_datos_peticion = $dbo->fetchAll("Socio", "IDSocio = '" . $IDSocio_peticion . "'", "array");
                            $cedula_peticion = $socio_datos_peticion["NumeroDocumento"];
                            $nombre_completo_peticion =  $socio_datos_peticion["Nombre"] . "" . $socio_datos_peticion["Apellido"];
                            $Direccion_peticion = $socio_datos_peticion["Direccion"];
                            $Celular_peticion = $socio_datos_peticion["Celular"];

                            $accion_peticion = $socio_datos_peticion["Accion"];
                            $secuencia_peticion = $socio_datos_peticion["IDSecuenciaZeus"];

                            $Token_peticion = SIMWebServiceZeus::obtener_token_club_curl($urlendpoint_peticion, $usuariuozeus, $clavezeus);

                            $datos_ambientes_peticion = $dbo->fetchAll("RestauranteDomicilio" . $Version, " IDRestauranteDomicilio = '" . $IDRestaurante_peticion . "' ", "array");
                            $datos_ambiente_peticion = $datos_ambientes_peticion["CodigoExternoAmbienteZeus"];
                            $mesero_peticion = $datos_ambientes_peticion["CajeroLakeHouse"];

                            $particular_peticion = "0";

                            if ($FormaPago_peticion == 22) : //EFECTIVO
                                $FormaPago_peticion = "01";
                            elseif ($FormaPago_peticion == 11) : //PLACETOPAY
                                $FormaPago_peticion = "22";
                            else :
                                $FormaPago_peticion = "";
                            endif;
                            ...
                            //VALIDAMOS QUE TENGA O NO LA PROPINA
                            if ($Propina_peticion == "S") :
                                $Propina_peticion = "S";
                            else :
                                $Propina_peticion = "N";
                            endif;
                            //ACA ESTAN LAS MESAS POR TIPO DE AMBIENTE
                            if ($datos_ambiente_peticion == "MR") :
                                $mesa_peticion = "002";
                            elseif ($datos_ambiente_peticion == "AT") :
                                $mesa_peticion = "021";
                            elseif ($datos_ambiente_peticion == "DP") :
                                $mesa_peticion = "022";
                            elseif ($datos_ambiente_peticion == "EV") :
                                $mesa_peticion = "023";
                            elseif ($datos_ambiente_peticion == "MC") :
                                $mesa_peticion = "034";
                            elseif ($datos_ambiente_peticion == "MV") :
                                $mesa_peticion = "035";
                            elseif ($datos_ambiente_peticion == "PT") :
                                $mesa_peticion = "120";
                            elseif ($datos_ambiente_peticion == "RS") :
                                $mesa_peticion = "121";
                            elseif ($datos_ambiente_peticion == "HY") :
                                $mesa_peticion = "HY001";
                            elseif ($datos_ambiente_peticion == "LG") :
                                $mesa_peticion = "LG01";
                            elseif ($datos_ambiente_peticion == "OL") :
                                $mesa_peticion = "OL01";
                            elseif ($datos_ambiente_peticion == "RT") :
                                $mesa_peticion = "RT01";
                            elseif ($datos_ambiente_peticion == "SB") :
                                $mesa_peticion = "SB01";
                            elseif ($datos_ambiente_peticion == "SG") :
                                $mesa_peticion = "SG01";
                            endif;

                            $partes = explode(" ", $Hora_entrega);
                            $fecha_pedido = $partes[0];
                            $hora_pedido = $partes[1];
                            $fecha_actual = date('Y-m-d');

                            if ($fecha_pedido == $fecha_actual) :
                                $hora_pedido = "07:30:00"; //Hora establecida
                                $hora_actual_en_segundos = time();
                                $hora_a_comparar_en_segundos = strtotime(date('Y-m-d') . ' ' . $hora_pedido);

                                if ($hora_actual_en_segundos > $hora_a_comparar_en_segundos) {


                                    if ($FormaPago_peticion == "01") :


                                        $POS_CrearPedido_peticion = SIMWebServiceZeus::POS_CrearPedido($urlendpoint_peticion, $Token_peticion, $datos_pedido_peticion, $Celular_peticion, $Direccion_peticion, $cedula_peticion, $nombre_completo_peticion, $mesa_peticion, $mesero_peticion, $FormaPago_peticion, $datos_ambiente_peticion, $Propina_peticion, $accion_peticion, $secuencia_peticion, $particular_peticion);

                                        $variable = "SIMWebServiceZeus::POS_CrearPedido($urlendpoint_peticion , $Token_peticion , $datos_pedido_peticion,$Celular_peticion, $Direccion_peticion, $cedula_peticion, $nombre_completo_peticion,$mesa_peticion, $mesero_peticion,$FormaPago_peticion, $datos_ambiente_peticion, $Propina_peticion, $accion_peticion,$secuencia_peticion, $particular_peticion)";



                                        $fact_peticion = "";
                                        $mensaje_peticion = "";
                                        foreach ($POS_CrearPedido_peticion->status as $info_peticion) {
                                            $mensaje_peticion = $info_peticion->message;
                                            $fact_peticion = $info_peticion->status;
                                        }
                                        //si no hubo respuesta entonces no se creo el pedido
                                        if ($fact_peticion == "SUCESS") :
                                            $SQLActualiza = "UPDATE  Domicilio" . $Version . " SET EstadoTransaccion = 'Enviada a zeus'  WHERE IDDomicilio = '$IDDomicilio_peticion'";
                                            $dbo->query($SQLActualiza);
                                        endif;

                                    else :

                                        //BUSCAMOS LOS ID DE DOMICILIOS EN LAS PETICIONES PARA VER SU ESTADO DE TRANSACCION
                                        $sql_placetopay = "SELECT * FROM `PeticionesPlacetoPay` WHERE  IDReserva = '$IDDomicilio_peticion' and IDClub = '" . $IDClub . "' and tipo = 'Domicilios' and estado_transaccion = 'APPROVED' ";
                                        $qry_sql_placetopay = $dbo->query($sql_placetopay);
                                        while ($r_sql_placetopay = $dbo->fetchArray($qry_sql_placetopay)) {
                                            $IDPeticionesPlacetoPay = $r_sql_placetopay["IDPeticionesPlacetoPay"];

                                            //0= SIN ENVIAR A ZEUS | 1=PEDIDO CREADO EN ZEUS
                                            if ($r_sql_placetopay["PedidoEnviado"] == 0) :
                                                ...
                                                //si no hubo respuesta entonces no se creo el pedido
                                                if ($fact_peticion == "SUCESS") :
                                                    $SQLActualiza = "UPDATE PeticionesPlacetoPay SET PedidoEnviado = '1' WHERE IDPeticionesPlacetoPay = '$IDPeticionesPlacetoPay'";
                                                    $dbo->query($SQLActualiza);

                                                    $SQLActualiza1 = "UPDATE  Domicilio" . $Version . " SET EstadoTransaccion = 'Enviada a zeus' WHERE IDDomicilio = '$IDDomicilio_peticion'";
                                                    $dbo->query($SQLActualiza1);
                                                endif;

...
class SIMWebServiceEcaddie
{
                ...
                if ($IDCaddiesEcaddie > 0 || $IDSocio > 0) :
                    ...
                    if ($dbo->rows($QRYConfiguracion) > 0) :
                        ...
                        while ($Datos = $dbo->fetchArray($QRYConfiguracion)) :
                            ...
                            $InfoConfiguracion["HoraMaximaPedirCaddie"] = $Datos["HoraMaximaPedirCaddie"];
    ...
    public function get_disponibilidad_elemento_servicio_caddie($IDClub, $IDSocio, $IDUsuario, $IDClubSeleccion, $IDServicio, $Fecha)
    {
        ...
        if (!empty($IDClub) && !empty($IDServicio) && !empty($Fecha)) :
            ...
            $SQLDisponibilidad = "SELECT *
                                  FROM  DisponibilidadServiciosCaddies DSC
                                  WHERE IDServiciosCaddie = $IDServicio AND DSC.Dias LIKE '%$Dia%' ";
    ...
    public function enviar_push_caddie($IDSolicitudCaddieRappi)
    {
        $dbo = SIMDB::get();
        $SQLSolicitudes = "SELECT * FROM SolicitudCaddieRappi WHERE IDSolicitudCaddieRappi = '" . $IDSolicitudCaddieRappi . "' ";
        $QRYSolicitudes = $dbo->query($SQLSolicitudes);
        while ($DatosSolicitud = $dbo->fetchArray($QRYSolicitudes)) {
            $Fecha = $DatosSolicitud["Fecha"];

            if (trim($DatosSolicitud["TipoCaddie"]) == 'Especial') :
            ...
            elseif (trim($TipoCaddie) == 'Turbo') :
                ...
                $CaddiesDisponibles = SIMWebServiceEcaddie::get_caddies_disponibles_caddies($DatosSolicitud["IDClub"], $DatosSolicitud["IDSocio"], "", $DatosSolicitud["IDClubSeleccion"], "", $DatosSolicitud["Fecha"], $DatosSolicitud["IDServiciosCaddie"], $DatosSolicitud["Hora"], $DatosSolicitud["IDElementoServiciosCaddies"], $DatosSolicitud["IDCaddiesEcaddie"]);
            ...
            foreach ($Caddies as $id => $Caddie) :
                ...
                SIMWebServiceEcaddie::EnviarNotificacion($DatosSolicitud["IDClub"], $IDSocioNotifica, $IDUsuario, $EnviaLink, $VerReservas, $Titular, $Mensaje, $FechaInicio, $FechaFin, $DiaReserva, $Cantidad, $IDSolicitud);
            /* endif;  */
            endforeach;
        }
    }
    ...
    public function set_solicitar_caddie_rappi($IDClub, $IDSocio, $IDUsuario, $IDClubSeleccion, $Fecha, $IDServicio, $Hora, $IDElemento, $TipoCaddie, $OpcionesCaddies, $IDCategoria)
    {
        ...
        $response = array();
        $Hoy = date("Y-m-d");
        $Manana = date('Y-m-d', strtotime($fechaInicioSemana . ' 1 day'));
        ...
        if (!empty($IDClub) && !empty($Fecha) && !empty($Hora) && !empty($TipoCaddie)) :
            ...
            if ($Fecha == $Hoy || ($Fecha == $Manana && $HoraActual >= $Configuracion["response"]["HoraMaximaPedirCaddie"] && $Configuracion["response"]["HoraMaximaPedirCaddie"] > "00:00:00")) {
                $respuesta[success] = false;
                $respuesta[message] = "Lo sentimos la hora máxima para solicitar un caddie para el siguiente dia es a las " . $Configuracion["response"]["HoraMaximaPedirCaddie"];
                $respuesta[response] = null;
                return $respuesta;
            }
            ...
            if ($PrimeroPagar == "S") {
                $sql_valor = "SELECT ValorCaddie FROM ClubesCaddies WHERE IDConfiguracionCaddies = '" . $Configuracion["response"]["IDConfiguracionCaddies"] . "' and  IDListaClubes = '" . $IDClubSeleccion . "' and ValorCaddie >0  LIMIT 1";
                $r_valor = $dbo->query($sql_valor);
                $row_valor = $dbo->fetchArray($r_valor);
                if ($row_valor["ValorCaddie"] > 0) {
                    $ValorPagar = $row_valor["ValorCaddie"];
                } else {
            ...
            $SETSolicitud = "INSERT INTO SolicitudCaddieRappi (IDSocio, IDClub, IDClubSeleccion,IDServiciosCaddie,IDElementoServiciosCaddies,IDCategoriaCaddie,EstadoSolicitud, Fecha, Hora, TipoCaddie, Opciones, Valor, FechaTrCr,UsuarioTrCr) 
                                    VALUES ('$IDSocio','$IDClub','$IDClubSeleccion','$IDServicio','$IDElemento','" . $IDCategoria . "','$EstadoSolicitud','$Fecha','$Hora','$TipoCaddie','$OpcionesCaddies','" . $ValorPagar . "',NOW(),'SOCIO-$IDSocio')";
            ...
            if ($dbo->rows($QRYSolicitudes) > 0) :
                ...
                while ($DatosSolicitud = $dbo->fetchArray($QRYSolicitudes)) :
                    ...
                    if ($Mas20Minutos < $FechaHoraActual) :
                        $update = "UPDATE SolicitudCaddieRappi SET EstadoSolicitud = 'Expirada' WHERE IDSolicitudCaddieRappi = $DatosSolicitud[IDSolicitudCaddieRappi] and EstadoSolicitud <> 'Confirmada' ";
                        $dbo->query($update);
                    endif;
            ...
            if ($IDCaddiesEcaddie > 0) :
                ...
                if ($dbo->rows($QRYSolicitudes) > 0) :
                    ...
                    while ($Datos = $dbo->fetchArray($QRYSolicitudes)) :
                        ...
                        //EVITAMOS QUE VEAN EL NOMBRE DEL SOCIO Y WHATSAPP ANTES DE ACEPTARLO

                        $datos_club = $dbo->fetchAll("SolicitudCaddieRappi", "IDSolicitudCaddieRappi = $IDSolicitud", "array");
                        $club_elegido = $dbo->fetchAll("ListaClubes", "IDListaClubes = $Datos[IDClubSeleccion]", "array");
                        $campo_elegido = $dbo->fetchAll("ElementoServiciosCaddies", "IDElementoServiciosCaddies = $Datos[IDElementoServiciosCaddies]", "array");

                        if ($datos_club["IDCaddiesEcaddie"] > 0) :

                            $Nombre = $DatosSocio[Nombre] . " " . $DatosSocio[Apellido];
                            $NumeroWhatsapp = $DatosSocio[Celular];
                        else :
                            $Nombre = "Privado";
                            $NumeroWhatsapp = "Numero Privado";
                        endif;
                        ...
                        $InfoResponse[NombreServicio] = $NombreServicio;
                        $InfoResponse[IDElemento] = $Datos[IDElementoServiciosCaddies];
                        $InfoResponse[NombreElemento] = $NombreElemento."\n"."Campo: ".$campo_elegido["Nombre"]."\n"."Club elegido: ".$club_elegido["Nombre"];
                        $InfoResponse[Fecha] = $Datos[Fecha];
                        $InfoResponse[Hora] = $Datos[Hora];
    ...
    public function set_tipo_pago_caddie_reserva($IDClub, $IDSocio, $IDUsuario, $IDSolicitud, $IDTipoPago, $CodigoPago)
    {
        ...
        if (!empty($IDClub) && (!empty($IDSocio) || !empty($IDUsuario)) && !empty($IDSolicitud) && !empty($IDTipoPago)) :
            ...
            if (count($DatosSolicitud) > 0) :
                ...
                if ($IDTipoPago == 3 || $IDTipoPago == 23 || ($IDTipoPago == 2 && $codigo_correcto == "S")) :
                    if ($PrimeroPagar == "S") {
                        $Actualiza = ", EstadoSolicitud = 'EsperandoCaddie'";
                        $Pagado = 'S';
                    } else {
    ...
    public function set_eliminar_caddie_reserva_empleado($IDClub, $IDUsuario, $IDSolicitud)
    {
        ...
        if (!empty($IDClub) && !empty($IDSolicitud) && !empty($IDUsuario)) :
            ...
            if (count($DatosSolicitud) > 0) :
                ...
                $IDCaddiesEcaddie = $DatosCaddie[IDCaddiesEcaddie];
                if ($IDCaddiesEcaddie > 0) :
                  $Cancelado = date("Y-m-d");

                    if ($DatosSolicitud[TipoCaddie] == 'Aleatorio') :
                        // SI ES ALEATORIO DEBE VOLVER A BUSCAR
                        $UPDATE = "UPDATE SolicitudCaddieRappi SET EstadoSolicitud = 'EsperandoCaddie' , UltimaCancelacion= '$Cancelado' WHERE IDSolicitudCaddieRappi = '$IDSolicitud'";
                        $dbo->query($UPDATE);
    ...
    public function set_respuesta_de_caddie_a_reserva($IDClub, $IDUsuario, $IDSolicitud, $AceptaSolicitud)
    {
        ...
        if (!empty($IDClub) && !empty($IDSolicitud) && !empty($AceptaSolicitud)) :
            ...
            $Configuracion = SIMWebServiceEcaddie::get_configuracion_caddies($IDClub, $IDUsuario, $IDUsuario);
            $PrimeroPagar = $Configuracion["response"]["PermitePagarPrimeroCaddie"];
                ...
                if ($IDCaddiesEcaddie > 0) :
                    ...
                    if ($DatosSolicitud[EstadoSolicitud] != 'Confirmada') :
                        ...
                        if ($AceptaSolicitud == 'S') :
                            ...
                            $total = "SELECT COUNT(*) as total FROM `SolicitudCaddieRappi` WHERE IDCaddiesEcaddie=$IDCaddiesEcaddie AND EstadoSolicitud ='Confirmada' AND FechaTrEd=CURDATE()";
                            $solicitudes =  $dbo->query($total);
                            $solicitudes_caddie = $dbo->fetchArray($solicitudes);

                            if ($solicitudes_caddie["total"] >= 0) :

                                $fecha_solicitud_caddie =  $DatosSolicitud["Fecha"];

                                $total = "SELECT * FROM `SolicitudCaddieRappi` WHERE IDCaddiesEcaddie=$IDCaddiesEcaddie AND EstadoSolicitud ='Confirmada' AND Fecha='$fecha_solicitud_caddie' ORDER by IDSolicitudCaddieRappi asc";
                                $solicitudes =  $dbo->query($total);
                                //datos de la solicitud ecaddie
                                $DatosSolicitud["Fecha"];
                                $DatosSolicitud["Hora"];

                                $fecha_solicitud = $DatosSolicitud["Fecha"] . "" . $DatosSolicitud["Hora"];

                                $contador_existencia_ocupada = 0;
                                if ($dbo->rows($solicitudes) > 0) {
                                    while ($r = $dbo->fetchArray($solicitudes)) {
                                        //datos de la fecha ya solicitudes aceptadas
                                        $fecha_sulictud_confirmada = $r["Fecha"] . "" . $r["Hora"];




                                        $datetime1 = date_create($fecha_sulictud_confirmada);
                                        $datetime2 = date_create($fecha_solicitud);
                                        ...
                                        if ($diferencia >= 6) :
                                            // echo "mas de 6 horas"; 
                                            $solicitudes_caddie["total"] = 0;
                                        else :
                                            $solicitudes_caddie["total"] = 2;
                                            //contamos si existe algun cadie ese dia para ese rango de fecha y hora (6 horas de diferencia)
                                            $contador_existencia_ocupada = $contador_existencia_ocupada++;

                                        endif;


                                        $fecha_sulictud_confirmada = "";
                                ...
                                //VERIFICAMOS QUE NO TENGA NINGUNA SOLICITUD CON DIFERENCIA DE 6 HORAS 
                                if ($contador_existencia_ocupada > 0) :
                                    $solicitudes_caddie["total"] = 2;
                                endif;
                               ...
                               $total_cancelaciones=0; 
                                //VALIDAMOS QUE NO HAYA CANCELADO UN SERVICIO EL DIA DE HOY, SI LO HIZO SE BLOQUEA POR 1 DIA.
                                 $hoy = date("Y-m-d");
                                 $cancelados = "SELECT COUNT(*) as total FROM `SolicitudCaddieRappi` WHERE IDCaddiesEcaddie=$IDCaddiesEcaddie AND EstadoSolicitud ='EsperandoCaddie' AND UltimaCancelacion='$hoy' LIMIT 1";
                                $cancelaciones =  $dbo->query($cancelados);
                                $row_cancelaciones = $dbo->fetchArray($cancelaciones);
                               $total_cancelaciones= $row_cancelaciones["total"];
                               
                            if($total_cancelaciones==1):
                                
                                 $message = "Lo sentimos, se bloqueo la opcion de aceptar servicios durante 24 horas por haber cancelado servicios ya tomados.";
                                
                                
                            elseif ($solicitudes_caddie["total"] == 0) :


                                if ($PrimeroPagar == "S") {
                                    $EstadoSolicitud = "Confirmada";

                                    $sql_valor = "SELECT ValorCaddie FROM ClubesCaddies WHERE IDConfiguracionCaddies = '" . $Configuracion["response"]["IDConfiguracionCaddies"] . "' and  IDListaClubes = '" . $DatosSolicitud["IDClubSeleccion"] . "' and ValorCaddie >0  LIMIT 1";
                                    $r_valor = $dbo->query($sql_valor);
                                    $row_valor = $dbo->fetchArray($r_valor);
                                    if ($row_valor["ValorCaddie"] > 0) {
                                        $ValorPagar = $row_valor["ValorCaddie"];
                                    ...
                                    $EstadoSolicitud = "PendientePago";
                                    $ValorPagar = $DatosCaddie[Valor];
                                }

                                // SI ACEPTA LA SOLICITUD ASIGNAMOS EL CADDIE Y PASAMOS DE ESTADO
                                $UPDATE = "UPDATE SolicitudCaddieRappi SET EstadoSolicitud = '" . $EstadoSolicitud . "', Valor = '" . $ValorPagar . "', IDCaddiesEcaddie = '$IDCaddiesEcaddie', FechaTrEd = NOW(), UsuarioTrEd = 'USUARIO-$IDUsuario-CADDIE-$IDCaddiesEcaddie' WHERE IDSolicitudCaddieRappi = '$IDSolicitud'";
                                $dbo->query($UPDATE);
                                ...
                                $message = "Lo sentimos, ya tiene otro servicio a menos de 6 horas en ese dia.";
                            endif;
                        else :
                            $InsertRechazadas = "INSERT INTO SolicitudRechazaCaddie (IDSolicitudCaddieRappi, IDCaddiesEcaddie) VALUES ('$IDSolicitud','$IDCaddiesEcaddie')";
                            $dbo->query($InsertRechazadas);

...
class SIMWebServiceReservas
{
    ...
    public function get_categorias_reservas($IDClub, $IDSocio)
    {
        $dbo = SIMDB::get();
        $response = array();
        ...
        if (!empty($IDClub)) :
            ...
            $TipoSocio_sql = "Select * From Socio Where IDSocio = '" . $IDSocio . "'  Limit 1";
            $TipoSocio_perfil = $dbo->query($TipoSocio_sql);
            $TipoSocio = $dbo->fetchArray($TipoSocio_perfil);

            $sql_servicio_perfil = "Select * From TipoSocioModulo Where IDClub = '" . $IDClub . "' and TipoSocio = '" . $TipoSocio[TipoSocio] . "' Limit 1";
            $r_servicio_perfil = $dbo->query($sql_servicio_perfil);
            $row_servicio_perfil = $dbo->fetchArray($r_servicio_perfil);
            $row_servicio_perfil["IDCategoriaServicio"];
            $array_id_servicio = explode("|", $row_servicio_perfil["IDCategoriaServicio"]);

            $id_servicio_ver = implode(",", $array_id_servicio);

            if ($row_servicio_perfil[Ocultar] == 1) :
                $condicion_servicio = " and IDCategoriasServicios not in ( $id_servicio_ver)";
            else :
                $condicion_servicio = " and IDCategoriasServicios in ( $id_servicio_ver )";
            endif;


            $SQLDatos = "SELECT * FROM CategoriasServicios WHERE IDClub = $IDClub AND Activo = 1   $condicion_servicio";
            $QRYDatos = $dbo->query($SQLDatos);
    ...
    public function get_servicios($IDClub, $TipoApp = "", $IDUsuario = "", $IDSocio = "", $IDCategoriasServicios = "", $IDClubAsociado = "")
    {
        ...
        if ($dbo->rows($qry) > 0) {
            ...
            while ($r = $dbo->fetchArray($qry)) {
                ...
                $servicio["LabelDocumentoInvitadoExterno"] = $r["LabelDocumentoInvitadoExterno"];
                ...
                $servicio["PermiteInvitadoExternoBuscador"] = $r["PermiteInvitadoExternoBuscador"];
                $servicio["InvitadoExternoBuscadorHeaderLabel"] = $r["InvitadoExternoBuscadorHeaderLabel"];
                ...
                $servicio["LinkExterno"] = $r["LinkExterno"];
                        ...
                        if (!$desactivado) {
                            ...
                            switch ($r_tipo_pago["IDTipoPago"]) {
                                ...
                                case "11":
                                    ...
                                    $imagen = "https://static.placetopay.com/placetopay-logo.png";
                                    break;
                                case "9":
            ...
            while ($row_reserva = $dbo->fetchArray($qry)) :
                ...
                if ($mostra_reserva == 1) {
                    ...
                    if ($dbo->rows($qry_auto) <= 0) {
                        ...
                        $reserva["LabelDocumentoInvitadoExterno"] = $datos_servicio["LabelDocumentoInvitadoExterno"];
                        $reserva["InvitadoExternoValor"] = $datos_servicio["InvitadoExternoValor"];
                        $reserva["PermiteInvitadoExternoBuscador"] = $datos_servicio["PermiteInvitadoExternoBuscador"];
                        $reserva["InvitadoExternoBuscadorHeaderLabel"] = $datos_servicio["InvitadoExternoBuscadorHeaderLabel"];
                        ...
                        if ($row_reserva["IDInvitadoBeneficiario"] > 0) :
                            ...
                            $Invitado = $dbo->getFields("SocioInvitado", "Nombre", "IDSocioInvitado = '" . $row_reserva["IDInvitadoBeneficiario"] . "' and IDClub = '" . $IDClub . "' ");
    ...
    public function get_historial_misreservas($IDClub, $IDSocio, $IDUsuario)
    {
        ...
        $Html = '<!doctype html>
                            ...
                            <body>
                            <table class="tabla">
                            <tr><th>Servicio</th><th>Fecha</th><th>Hora</th><th>Tipo</th></tr>';
    ...
    public function get_invitados_externos_reservas($IDClub, $IDSocio, $Tag, $IDServicio)
    {
        $dbo = &SIMDB::get();
        ...
        // Tag
        if (!empty($Tag)) :
            $condicion = " AND (Nombre  like '%" . $Tag . "%' or Cedula like '%" . $Tag . "%')";
        endif;


        $response = array();
        $sql = "SELECT * FROM ReservaGeneralInvitado WHERE IDSocioInvita='" . $IDSocio . "' AND Externo='S' AND IDSocio='0' " .  $condicion . " ORDER BY Favorito DESC";
        $qry = $dbo->query($sql);
        if ($dbo->rows($qry) > 0) {
            $message = $dbo->rows($qry) . " " . SIMUtil::get_traduccion('', '', 'Encontrados', LANG);
            while ($r = $dbo->fetchArray($qry)) {
                ...
                $InvitadoExterno["IDInvitadoExterno"] = $r["IDReservaGeneralInvitado"];
                $InvitadoExterno["Nombre"] = $r["Nombre"];
                $InvitadoExterno["Documento"] = $r["Cedula"];
                $InvitadoExterno["Correo"] = $r["Correo"];
                $InvitadoExterno["FechaNacimiento"] = $r["FechaNacimiento"];
                $InvitadoExterno["Favorito"] = $r["Favorito"];

                // CAMPOS ADICIONALES INIVTADOS

                $reponse_campos_invitados = array();
                $SQLCampos = "SELECT * FROM RespuestasCampoInvitadoExterno WHERE IDReservaGeneralInvitado = $r[IDReservaGeneralInvitado]";
                $QRYCampos = $dbo->query($SQLCampos);
                ...
                $InvitadoExterno["CamposDinamicos"] = $reponse_campos_invitados;


                array_push($response, $InvitadoExterno);
            } //ednw hile
            $respuesta["message"] = $message;
            $respuesta["success"] = true;
            $respuesta["response"] = $response;
    ...
    public function set_invitados_externos_reservas_favorito($IDClubAsociado, $IDSocio, $Favorito, $IDInvitadoExterno)

    {

        $dbo = &SIMDB::get();
        if (!empty($IDInvitadoExterno)) {

            //verifico que el invitado externo exista
            $id_invitado_externo = $dbo->getFields("ReservaGeneralInvitado", "IDReservaGeneralInvitado", "IDReservaGeneralInvitado = '" . $IDInvitadoExterno . "'");

            if (!empty($id_invitado_externo)) {


                $actualizar_invitado_externo_favorito = $dbo->query("UPDATE ReservaGeneralInvitado SET Favorito='" .  $Favorito . "' WHERE IDReservaGeneralInvitado='" . $IDInvitadoExterno . "'");
                ...
                $respuesta["message"] = "Error invitado Externo no existe";
                $respuesta["success"] = false;
                $respuesta["response"] = null;
            }
        } else {

...
class SIMWebServiceTaloneras
{
    ...
    public function get_estado_cuenta_taloneras($IDClub, $IDSocio, $IDUsuario)
    {
        ...
        if (!empty($IDClub) && (!empty($IDSocio) || !empty($IDUsuario))) :
            ...
            while ($Servicio = $dbo->fetchArray($QRYServicio)) :

                $EstadoTalonera[IDClub] = $IDClub;
                $EstadoTalonera[IDServicio] = $Servicio[IDServicio];

                $NombreServicio = $Servicio[TituloServicio];
                // SI NO TIENE TITULO EL SERVICIO SACAMOS EL TITULO DEL SERVICIO MAESTRO
                if (empty($NombreServicio)) :
                    $NombreServicio = $dbo->getFields("ServicioMaestro", "Nombre", "IDServicioMaestro = $Servicio[IDServicioMaestro]");
                endif;

                $EstadoTalonera[NombreServicio] = $NombreServicio;

                $SQLSocioTalonera = "SELECT * FROM SocioTalonera WHERE IDClub = '$IDClub' AND (IDServicio = $Servicio[IDServicio] OR (TodosLosServicios = 1 AND IDServicio = 0)) AND $Campo = '$Valor' AND Activo='1'";
    ...
    public function set_taloneras($IDClub, $IDSocio, $IDUsuario, $IDServicio, $IDTalonera, $Asignados)
    {
        ...
        if (!empty($IDClub) && (!empty($IDSocio) || !empty($IDUsuario)) && !empty($IDTalonera)) :
           ...
           $saldomonedero = $datos_talonera[SaldoTaloneraMonedero] + $DatosSocioTalonera["SaldoMonedero"];
            
                        $SQLUpdate = "UPDATE SocioTalonera SET IDTalonera = '$IDTalonera', ValorPagado = '$ValorPagado', FechaRecarga = '$FechaRecarga', FechaVencimiento = '$FechaVencimiento', SaldoMonedero = '$saldomonedero', FechaTrEd = NOW(), UsuarioTrEd = '$Campo-$Valor' WHERE IDSocioTalonera = $IDSocioTalonera";
                        $dbo->query($SQLUpdate);
                    endif;

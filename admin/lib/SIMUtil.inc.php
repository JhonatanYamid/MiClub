...
class SIMUtil
{
    ...
    public function generar_codigo_barras_correspondencia($parametros_codigo_barras, $IDCliente)
    {
        ...
        // Including the barcode technology
        require_once LIBDIR . 'barcodegen/class/BCGcode39_correspondencia.barcode.php';

        // Loading Font
        $font = new BCGFontFile(LIBDIR . 'barcodegen/font/Arial.ttf', 22);
    ...
    public function notifica_recibo_domicilio($IDDomicilio, $Version = "")
    {
        ...
        if (!empty($correo)) :
            ...
            $accion_socio = $datos_socio["Accion"];
            $celular_socio = $datos_socio["Celular"];
            $direccion_socio =   $datos_domicilio["Direccion"];
            // $direccion_socio = $datos_domicilio["Direccion"] . " Dirección Actual:" . $datos_socio["Direccion"];
            ...
            $qry = $dbo->query($sql);
            while ($r = $dbo->fetchArray($qry)) {
                $pedido .= "<li>" . $r["Producto"] = $dbo->getFields("Producto" . $Version, "Nombre", "IDProducto = '" . $r["IDProducto"] . "'");
                $pedido .= ":" . $r["Cantidad"] . " " . SIMUtil::get_traduccion('', '', 'Comentarios', LANG) . ":" . $r["Comentario"] . "<br>";

                //caracteristicas
                $caracteristicas = "";
                $sql_carac = "SELECT DC.*,PP.Nombre as Categoria, CP.Nombre as Caracteristica,CP.Valor
                ...
                AND (DC.IDDomicilioDetalle =$r[IDDomicilioDetalle] OR (DC.IDDomicilioDetalle = 0 AND DC.IDDomicilioCaracteristica < 15772))
                ORDER BY PP.Nombre";
                $r_carac = $dbo->query($sql_carac);
                while ($row_carac = $dbo->FetchArray($r_carac)) {


                    $caracteristicas .=   $row_carac["Categoria"] . " : " . $row_carac["Caracteristica"] .  "<br>";
                }
                $pedido .= $caracteristicas . "</li>";
            } //edn while
            ...
            $msg = "<br>" . SIMUtil::get_traduccion('', '', 'CordialSaludo', LANG) . "<br><br>" .
                ...
                "Dirección Actual:" . $direccion_socio . "<br>" .
                ...
                SIMUtil::get_traduccion('', '', 'DescripcionPedido', LANG) . ":" .  "<ul>" . "<br>" . $pedido . "<br>" . "</ul>" .
...
https://itunes.apple.com/app/bijao-beach-club/id1325705162?ls=1&mt=8 <br><br>
    ...
    public function notificar_solicitud_objeto_perdido($IDObjetoPerdido, $IDSocio)
    {
        ...
        if (!empty($correo)) :
            ...
            $msg = "<br>Cordial Saludo,<br><br>
                        ...
                        Correo: " . $datos_socio["CorreoElectronico"] . "<br>
						Celular: " . $datos_socio["Celular"] . "<br>
						Telefono: " . $datos_socio["Telefono"] . "<br>
						Direccion: " . $datos_socio["Direccion"] . "<br><br>
    ...
    public function notificar_solicitud_canje($id_solicitud, $destino = "")
    {
        ...
        if (($config_canje["PermiteEnviarCorreoSocio"] == "S" || empty($config_canje["PermiteEnviarCorreoSocio"])) && empty($destino)) {
        ...
        if (!empty($correo)) {
            ...
            $msg = str_replace("[NombreClub]", strtoupper(($dbo->getFields("ListaClubes", "Nombre", "IDListaClubes = '" . $datos_solicitud["IDListaClubes"] . "'"))), $msg);
            ...
            $msg = str_replace("[CorreoDestino]", $row_detalle_club["CorreoNotificacion"], $msg);
            ...
            if (count($array_benef) > 0) {
                foreach ($array_benef as $IDSocioBenef) {
                    ...
                    $datos_socio_benef .= $datos_benef["Nombre"] . " " . $datos_benef["Apellido"] . "<br>";
                }
            }
            /*  if (count($array_benef) > 0) {
                foreach ($array_benef as $IDSocioBenef) {
        ...
        if (!empty($id_reserva)) :
            ...
            if (!empty($correo) && $solo_socio == "") :
                $msg = "<br>Cordial Saludo,<br><br>
									...
									<b>Valor pagado:</b>" . $datos_reserva["ValorPagado"] . "<br>
    ...
    /*  public function generar_qr_restaurante_domicilio($IDRestaurante, $parametros_codigo_qr)
    {

        //set it to writable location, a place for temp generated PNG files
        //$PNG_TEMP_DIR = dirname(__FILE__).DIRECTORY_SEPARATOR.'temp'.DIRECTORY_SEPARATOR;

        $PNG_TEMP_DIR = RESTAURANTEQR_DIR;
        ...
        $filename = $PNG_TEMP_DIR . 'test' . md5($parametros_codigo_qr . $IDRestaurante . '|' . $errorCorrectionLevel . '|' . $matrixPointSize) . '.png';
        QRcode::png($parametros_codigo_qr, $filename, $errorCorrectionLevel, $matrixPointSize, 2);
 ...
 */
        ...
        if (!empty($IDClub) && !empty($IDReservaGeneral)) {
            ...
            while ($row_elemento_usuario = $dbo->fetchArray($result_elemento_usuario)) :
                ...
                if (!empty($datos_empleado["IDUsuario"]) && $datos_empleado["Token"] != "2byte" && $datos_empleado["Token"] != "" && $datos_empleado["PushReserva"] != "N") :
                    ...
                    $otro_dato .= "\nServicio: " . utf8_decode($NombreServicio) . "-" . $datos_elemento["Nombre"];
                    ...
                    if ((int) $id_auxiliar > 0) {
                        ...
                        while ($row_aux = $dbo->fetchArray($result_aux)) :
                            ...
                            if (!empty($datos_empleado["IDUsuario"]) && $datos_empleado["Token"] != "2byte" && $datos_empleado["Token"] != "" && $datos_empleado["PushReserva"] != "N") :
                                ...
                                $otro_dato .= "\nServicio: " . $NombreServicio . "-" . $datos_elemento["Nombre"];
    ...
    public function calcular_tarifa($IDClub, $IDSocio, $IDServicio, $Fecha, $Hora, $IDElemento, $IDReservaGeneral = "", $IDTipoReserva)
    {
        ...
        elseif ($IDServicio == 25981) :
            $datoReserva = $dbo->fetchAll("ReservaGeneral", "IDReservaGeneral = '" . $IDReservaGeneral . "'");
            $valor = $datoReserva["ValorPagado"];
    ...
    {
        $ValorProfesor = 10000;
        $ValorHoras11_14 = 12000;
        $ValorHoras18_20 = 22500;
        $ValorOtrasHoras = 17500;
    ...
    public function valor_reserva_liga_risaralda_clases_grupales($Hora, $ListaAuxiliar, $IDElemento, $Fecha)
    {
        ...
        $ValorOtrasHoras = 17500;
        $ValorIniciantes_M_J_8_10 = 17500;
    ...
    public function obtener_accion_pasarela($IDTipoPago, $IDClub)
    {
        ...
        switch ($IDTipoPago) {
            ...
            case "20":
                ...
                break;
            case "21":
                $accion = $datos_clubAdicionales["URLPayWay"];
                break;
            case "26":
                $accion = $datos_clubAdicionales["UrlLocalLukaPay"];
                // $accion = "https://www.miclubapp.com/lukapay.php?a=1";
                break;
            case "28":
                $accion = $datos_clubAdicionales["URLMercadoPago"];
                break;
            case "29":
                $accion = $datos_clubAdicionales["URLYappy"];
                break;
    ...
    public function noticar_nueva_LaboralVacaciones($id_LaboralVacaciones, $TipoNotif = "")
    {
        ...
        if ($validaPersonaACargo  == 1) {
            ...
            // WHERE (s.NumeroDocumento = '" . $datos_user['DocumentoJefe'] . "' OR u.NumeroDocumento = '" . $datos_user['DocumentoJefe'] . "') AND (s.IDClub=" . $datos_club['IDClub'] . " OR u.IDClub=" . $datos_club['IDClub'] . " ) LIMIT 1";

            $sql_jefe = "SELECT IDUsuario as IDUser, Email as CorreoElectronico, Nombre, Cargo FROM Usuario WHERE NumeroDocumento = '" . $datos_user['DocumentoJefe'] . "' AND IDClub=" . $datos_club['IDClub'] . " UNION SELECT IDSocio as IDUser, CorreoElectronico as CorreoElectronico, CONCAT(Nombre,' ',Apellido) as Nombre, CargoSocio as Cargo FROM Socio WHERE NumeroDocumento = '" . $datos_user['DocumentoJefe'] . "' AND IDClub=" . $datos_club['IDClub'] . " LIMIT 1";
            ...
            // IF(IDSocio > 0,s.CargoSocio,u.Cargo) as Cargo
            // FROM Socio s LEFT JOIN Usuario u ON s.NumeroDocumento=u.NumeroDocumento 
            // WHERE (s.NumeroDocumento = '" . $datos_user['DocumentoEspecialista'] . "' OR u.NumeroDocumento = '" . $datos_user['DocumentoEspecialista'] . "') AND (s.IDClub=" . $datos_club['IDClub'] . " OR u.IDClub=" . $datos_club['IDClub'] . " ) LIMIT 1";

            $sql_aprobador = "SELECT IDUsuario as IDUser, Email as CorreoElectronico, Nombre, Cargo FROM Usuario WHERE NumeroDocumento = '" . $datos_user['DocumentoEspecialista'] . "' AND IDClub=" . $datos_club['IDClub'] . " UNION SELECT IDSocio as IDUser, CorreoElectronico as CorreoElectronico, CONCAT(Nombre,' ',Apellido) as Nombre, CargoSocio as Cargo FROM Socio WHERE NumeroDocumento = '" . $datos_user['DocumentoEspecialista'] . "' AND IDClub=" . $datos_club['IDClub'] . " LIMIT 1";
    ...
    public function noticar_nueva_AuxiliosSolicitud_infinito($id_AuxiliosSolicitud, $TipoNotif = "")
    {
        ...
        $ConfiguracionAuxiliosInfinito = $dbo->fetchAll("ConfiguracionAuxiliosInfinito", " IDClub = '" . $datos_AuxiliosSolicitud['IDClub'] . "' and IDModulo = {$datos_AuxiliosSolicitud['IDModulo']}  ", "array");

        $ConfiguracionAuxiliosInfinito['NombreModulo'] = (empty($ConfiguracionAuxiliosInfinito['NombreModulo'])) ? "Auxilio" : $ConfiguracionAuxiliosInfinito['NombreModulo'];
        ...
        // Valida si notifica a los jefes o a un solo aprobador
        if ($ConfiguracionAuxiliosInfinito['GestionDeUnAprobador'] == 'S' && !empty($ConfiguracionAuxiliosInfinito['NumeroDocumentoUnicoAprobador'])) {
            $sql_especialista = "SELECT IDUsuario,Email FROM Usuario WHERE NumeroDocumento = '" . $ConfiguracionAuxiliosInfinito['NumeroDocumentoUnicoAprobador'] . "' and IDClub = " . $datos_club['IDClub'] . " LIMIT 1";
            ...
            $sql_especialista = "SELECT IDUsuario,Email FROM Usuario WHERE NumeroDocumento = '" . $datos_user['DocumentoEspecialista'] . "' and IDClub = " . $datos_club['IDClub'] . " LIMIT 1";
        ...
        if (!empty($correo)) :
            ...
            $msg = "<br>Cordial Saludo,<br><br>
				Se ha generado una nueva solicitud de {$ConfiguracionAuxiliosInfinito['NombreModulo']}.
				Recuerde ingresar al sistema para conocer mas detalles .<br><br>
                Ingreso por la plataforma web: Auxilios > Auxilios Solicitud. <br>
                Ingreso por la aplicaci&oacute;n m&oacute;vil: {$ConfiguracionAuxiliosInfinito['NombreModulo']} > Revisar solicitudes. <br><br>

				Numero: " . $datos_AuxiliosSolicitud["IDAuxiliosInfinitoSolicitud"] . "<br>
				Socio: " . ($datos_user["Nombre"] . " " . $datos_user["Apellido"]) . " <br>
                Accion: " . $datos_user["Accion"] . "" . "<br>
				Correo: " . ($datos_user["CorreoElectronico"]) . "<br>
				Estado: " . (SIMResources::$EstadoAuxilio[$datos_AuxiliosSolicitud['IDEstado']]) . "<br>
				Fecha: " . $datos_AuxiliosSolicitud['FechaTrCr'] . "<br><br>
            ...
            if (count($array_correo) > 0) {
                ...
                if ($TipoNotif == "") {
                    $TipoNotif = "Nueva Solicitud {$ConfiguracionAuxiliosInfinito['NombreModulo']}";
                }
                ...
                //Envio tambien el push con la notificacion del nuevo Solicitud Auxilio
                $Mensaje = "Se ha recibido una nueva solicitud de {$ConfiguracionAuxiliosInfinito['NombreModulo']} por parte de un trabajador. \nIngrese a Auxilios > Revisar Solicitudes";
                $mensajeConfirmacion = "Cordial saludo: su Solicitud de {$ConfiguracionAuxiliosInfinito['NombreModulo']} se ha radicado con éxito al área encargada para su revisión.";
                if ($user == 'socio') {
                    SIMUtil::enviar_notificacion_push_general_funcionario($datos_club["IDClub"], $idEspecialista, $Mensaje);
    ...
    public function notificar_nueva_AuxiliosSolicitud_infinito($id_AuxiliosInfinitoSolicitud, $TipoNotif = "")
    {
        ...
        $ConfiguracionAuxiliosInfinito = $dbo->fetchAll("ConfiguracionAuxiliosInfinito", " IDClub = '" . $datos_AuxiliosInfinitoSolicitud['IDClub'] . "' and IDModulo = {$datos_AuxiliosInfinitoSolicitud['IDModulo']}  ", "array");

        $ConfiguracionAuxiliosInfinito['NombreModulo'] = (empty($ConfiguracionAuxiliosInfinito['NombreModulo'])) ? "Tiempo para mi" : $ConfiguracionAuxiliosInfinito['NombreModulo'];
        ...
        // Validar si nofifica a los jefes o al unico aprobador
        if ($ConfiguracionAuxiliosInfinito['GestionDeUnAprobador'] == 'S' && !empty($ConfiguracionAuxiliosInfinito['NumeroDocumentoUnicoAprobador'])) {
            $correoJefe = $dbo->getFields("Usuario", "Email", "IDClub={$datos_club['IDClub']} and NumeroDocumento = '{$ConfiguracionAuxiliosInfinito['NumeroDocumentoUnicoAprobador']}'");
            $NombreJefe = $dbo->getFields("Usuario", "Nombre", "IDClub={$datos_club['IDClub']} and NumeroDocumento = '{$ConfiguracionAuxiliosInfinito['NumeroDocumentoUnicoAprobador']}'");
            $CargoJefe = $dbo->getFields("Usuario", "Cargo", "IDClub={$datos_club['IDClub']} and NumeroDocumento = '{$ConfiguracionAuxiliosInfinito['NumeroDocumentoUnicoAprobador']}'");
            $idEspecialista = $dbo->getFields("Usuario", "IDUsuario", "IDClub={$datos_club['IDClub']} and NumeroDocumento = '{$ConfiguracionAuxiliosInfinito['NumeroDocumentoUnicoAprobador']}'");
        } else {
            $sql_jefe = "SELECT 
                    IF(IDSocio > 0,IDSocio,IDUsuario) as IDUser,
                    IF(IDSocio > 0,s.CorreoElectronico,u.Email) as CorreoElectronico,
            ...
            $query_aprobador = $dbo->query($sql_aprobador);
            $row_aprobador = $dbo->assoc($query_aprobador);
            $idEspecialista = $row_aprobador['IDUser'];
        ...
        if (in_array($datos_club['IDClub'], $arrIDClubLuker)) {
            ...
            $asunto = "Nueva Solicitud de {$ConfiguracionAuxiliosInfinito['NombreModulo']}";
            $correo = $correoJefe;
            $msg = "<br>Cordial Saludo,<br><br>
				Se ha generado una nueva solicitud de {$ConfiguracionAuxiliosInfinito['NombreModulo']}.<br>
				Recuerde ingresar al sistema para conocer mas detalles .<br><br>
                ...
                Auxilio: " . ($datos_AuxiliosInfinito['Nombre']) . "<br>
				Socio: " . ($datos_user["Nombre"] . " " . $datos_user["Apellido"]) . " <br>
                Accion: " . $datos_user["Accion"] . "" . "<br>
				Correo: " . ($datos_user["CorreoElectronico"]) . "<br>
				Estado: " . (SIMResources::$EstadoAuxilio[$datos_AuxiliosInfinitoSolicitud['IDEstado']]) . "<br>
        ...
        if (!empty($correo)) :
            self::send_mail($datos_club['IDClub'], $correo, $asunto, $msg);
            //Envio tambien el push con la notificacion del nuevo Solicitud Auxilio
            $Mensaje = "Se ha recibido una nueva solicitud de {$ConfiguracionAuxiliosInfinito['NombreModulo']} por parte de un trabajador. \nIngrese a {$ConfiguracionAuxiliosInfinito['NombreModulo']} > Revisar Solicitudes";
            $mensajeConfirmacion = "Cordial saludo: su Solicitud de {$ConfiguracionAuxiliosInfinito['NombreModulo']} se ha radicado con éxito al área encargada para su revisión.";
            if ($user == 'socio') {
                SIMUtil::enviar_notificacion_push_general_funcionario($datos_club["IDClub"], $idEspecialista, $Mensaje);
        ...
        $CorreoNotificacionSolicitudAuxilio = $dbo->getFields('ConfiguracionAuxiliosInfinito', 'CorreoNotificacionSolicitudAuxilio', "IDClub=" . $datos_club['IDClub'] . " AND IDModulo="  . $datos_AuxiliosInfinitoSolicitud['IDModulo'] . " limit 1");

        //SI ES CENTRAL CERVECERA SE ENVIA AL CORREO DE CADA TIPO DE AUXILIOS INFITOS
        if ($datos_club['IDClub'] == 00) :
            if ($datos_AuxiliosInfinito["Correo"] != "") :
                $CorreoNotificacionSolicitudAuxilio = $datos_AuxiliosInfinito["Correo"];
            endif;
        endif;

        if (!empty($CorreoNotificacionSolicitudAuxilio)) {
            self::send_mail($datos_club['IDClub'], $CorreoNotificacionSolicitudAuxilio, $asunto, $msg);
        }
    }
    ...
    public function notificar_respuesta_AuxilioSolicitud_infinito($id_AuxiliosInfinitoSolicitud, $Comentario)
    {
        ...
        if (in_array($datos_club['IDClub'], $arrIDClubLuker)) {
            ...
            if ($Estado == 3) {
                ...
                $msg = "Sr(a). $Nombre<br>
                $Cargo<br><br>
                Su solicitud de " . ($datos_AuxiliosInfinito['Nombre']) . " ha sido aprobada desde $PeriodoDeFechas
                Atentamente, <br>
                $NombreEncargado";
            } else {
                ...
                if ($Perfil == 1) {
                    ...
                    Su solicitud de " . ($datos_AuxiliosInfinito['Nombre']) . " no ha sido aprobada por su jefe inmediato, por favor comunicarse con él.<br><br>
                    Atentamente,<br>
                    $NombreEncargado";
                } else {
                    $msg = "Sr(a). $Nombre <br><br>
                    Su solicitud de " . ($datos_AuxiliosInfinito['Nombre']) . " no ha sido aprobada por n&oacute;mina, por favor comunicarse con &eacute;sta &aacute;rea.<br><br>
                    Atentamente,<br>
                    $NombreEncargado";
                }
            }
            ...
            $Tipo = 'Auxilio';
            $asunto = "Respuesta Solicitud de {$ConfiguracionAuxiliosInfinito['NombreModulo']}";
            $msg = "<br>Cordial Saludo,<br><br>
            Se ha generado un nueva respuesta a una solicitud de {$ConfiguracionAuxiliosInfinito['NombreModulo']}.
            Recuerde ingresar al sistema para conocer mas detalles .<br><br>
            Ingreso por la aplicaci&oacute;n m&oacute;vil: {$ConfiguracionAuxiliosInfinito['NombreModulo']} > Mis Solicitudes. <br><br>
            Numero: " . $datos_AuxiliosInfinitoSolicitud["IDAuxiliosInfinitoSolicitud"] . "<br>
            Socio: " . ($dbo->getFields("Socio", "Nombre", "IDSocio = '" . $datos_AuxiliosInfinitoSolicitud['IDSocio'] . "'") . " " . $dbo->getFields("Socio", "Apellido", "IDSocio = '" . $datos_AuxiliosInfinitoSolicitud[IDSocio] . "'")) . "<br>
            $Tipo: " . ($datos_AuxiliosInfinito['Nombre']) . "<br>
        ...
        //Envio tambien el push al Socio remitente
        $mensajeRespuesta = "Cordial saludo: su solicitud de {$ConfiguracionAuxiliosInfinito['NombreModulo']} ha sido resuelta. \nIngresa para conocer su resolución:\n \n {$ConfiguracionAuxiliosInfinito['NombreModulo']} > Mis Solicitudes.";
    ...
    function Calcular_diferencia_horas($FechaInicio, $FechaFin, $Movimiento = "", $ExtraDespuesDeTurno = "")
    {
        ...
        if ($Dia == '6' || $Dia == '0' || !empty($DiaFestivo["Fecha"])) {
            ...
            //si marca despues de las 09:00 pm se debe de colocar el calculo como horas nocturnas
            if ($r_HoraInicio > $HoraFinDia) {
                $TiempoDiferencia1 = $HoraInicio->diff($Date_FinDiaLaboral);
                $TiempoDiferencia1 = $TiempoDiferencia1->format('%H:%I:%S');


                $arrSumarHoras = array($TiempoDiferencia, $TiempoDiferencia1);
                $TotalHoras = self::sumar_hora($arrSumarHoras, "S");


                // echo "Total" . $TotalNocturnas;
                $TiempoDiferencia = $TotalHoras;
            }
        }
    ...
    function Tipo_tiempo_laboral($HoraInicioJornadaLaboral, $HoraFinJornadaLaboral, $FechaMovimientoEntrada, $FechaMovimientoSalida)
    {
        ...
        if (($Dia == "Lunes" || $Dia == "Martes" || $Dia == "Miércoles" || $Dia == "Jueves" || $Dia == "Viernes") && empty($DiaFestivo["Fecha"])) {
            ...
            //si tiene extras  antes o despues de salir o  la hora salida establecida es mayor de las 9pm
            if ($HoraMovimientoEntrada < $HoraInicioJornadaLaboral || $HoraMovimientoSalida > $HoraFinJornadaLaboral || $HoraFinJornadaLaboral > $HoraFinDia) {
                ...
                if ($HoraMovimientoSalida > $HoraFinDia && $HoraMovimientoSalida > $HoraFinJornadaLaboral && $HoraFinJornadaLaboral > $HoraFinDia) {
                    ...
                    //Hora Nocturna sin extras pero se debe sumar 
                    $HoraNocturna = $Date_FinJornadaLaboral->diff($Date_FinDiaLaboral);
                    $HoraNocturna = $HoraNocturna->format('%H:%I');
                }


                //aqui sale despues de las 9pm pero se deben sumar las horas nocturnas y que no tengan extras
                if ($HoraMovimientoSalida > $HoraFinDia && $HoraMovimientoSalida <= $Date_FinJornadaLaboral &&  $extraNocturnaSola == NULL && $extraNocturnaSalida == NULL) {
                    $HoraNocturna2 = $Date_MovimientoSalida->diff($Date_FinDiaLaboral);
                    $HoraNocturna2 = $HoraNocturna2->format('%H:%I');
                }
                ...
                //nocturnas
                $extraNocturnaEntrada = !empty($extraNocturnaEntrada) ? $extraNocturnaEntrada : "00:00";
                $extraNocturnaSalida = !empty($extraNocturnaSalida) ? $extraNocturnaSalida : "00:00";
                $extraNocturnaSola = !empty($extraNocturnaSola) ? $extraNocturnaSola : "00:00";
                $HoraNocturna = !empty($HoraNocturna) ? $HoraNocturna : "00:00";
                $HoraNocturna2 = !empty($HoraNocturna2) ? $HoraNocturna2 : "00:00";

                //sumar horas nocturnas
                $arrSumarHorasNocturnas = array($extraNocturnaEntrada, $extraNocturnaSalida, $extraNocturnaSola, $HoraNocturna, $HoraNocturna2);
                $TotalNocturnas = self::sumar_hora($arrSumarHorasNocturnas);
            ...
            if ($HoraMovimientoEntrada < $HoraFinNoche && $HoraMovimientoSalida < $HoraFinNoche) {
    ...
    function sumar_hora($arrHoras, $calcularFormatoEnHoras = "")
    {
        ...
        //retornar en formato de horas
        if (!empty($calcularFormatoEnHoras)) {
    ...
    function notificar_nueva_solicitud_ingreso($IDClub, $IDIngresosSolicitud)
    {
        ...
        $correo = $dbo->getFields("ConfiguracionClub", "CorreoJefeIngreso", " IDClub = $IDClub");
    ...
    function validaDiaFestivo($IDClub, $Fecha)
    {
        $dbo = &SIMDB::get();

        $Pais = $dbo->getFields("ConfiguracionClub", "IDPais", "IDClub=$IDClub");
        $Festivo = $dbo->getFields("Festivos", "Fecha", "IDPais = '" . $Pais . "' and Fecha='" . $Fecha . "'");
    ...
    function Notificar_solicitud_laboral($IDClub, $IDSocio = "", $IDUsuario = "", $Id, $Modulo)
    {
        ...
        if (!empty($IDSocio)) {
            $Campo = "IDSocio";
            $Valor = $IDSocio;
            $NumeroDocumentoJefe = $dbo->getFields('Socio', "DocumentoJefe", "IDSocio=$IDSocio");
            $datos_solicitante = $dbo->fetchAll("Socio", " IDSocio = '" . $IDSocio . "' ", "array");
        } else {
            $Campo = "IDUsuario";
            $Valor = $IDUsuario;
            $NumeroDocumentoJefe = $dbo->getFields('Usuario', "DocumentoJefe", "IDUsuario=$IDUsuario");
            $datos_solicitante = $dbo->fetchAll("Usuario", " IDUsuario = '" . $IDUsuario . "' ", "array");
        }
        $NombreClub = $dbo->getFields('Club', 'Nombre', "IDClub=$IDClub");

        if (empty($NumeroDocumentoJefe) || $NumeroDocumentoJefe <= 0) {
            return;
        }
        $sql_jefe = "SELECT IDUsuario as IDUser, Email as CorreoElectronico, Nombre, Cargo FROM Usuario WHERE NumeroDocumento = '$NumeroDocumentoJefe' AND IDClub=" . $IDClub . " UNION SELECT IDSocio as IDUser, CorreoElectronico as CorreoElectronico, CONCAT(Nombre,' ',Apellido) as Nombre, CargoSocio as Cargo FROM Socio WHERE NumeroDocumento = '" . $datos_user['DocumentoJefe'] . "' AND IDClub=" . $IDClub . " LIMIT 1";

        $q_jefe = $dbo->query($sql_jefe);
        $r_jefe = $dbo->rows($q_jefe);
        $Jefe = $dbo->assoc($q_jefe);

        if ($r_jefe > 0) {
            $correo = $Jefe['CorreoElectronico'];
            $idJefe = $Jefe['IDUser'];


            $idUser = $Valor;
            $NombreSolicitante = $datos_solicitante['Nombre'] . ' ' . $datos_solicitante['Apellido'];
            $NumeroDocumentoSolicitante = $datos_solicitante['NumeroDocumento'];

            switch ($Modulo) {
                case 'Permiso':
                    $datos_Solicitud = $dbo->fetchAll("LaboralPermiso", " IDLaboralPermiso = '" . $Id . "' ", "array");
                    $asunto = "Solicitud de Permiso";
                    $msg = "Tiene una nueva solicitud de Permiso por parte de $NombreSolicitante, Cc $NumeroDocumentoSolicitante, para el día " . $datos_Solicitud['FechaInicio'] . " hasta " . $datos_Solicitud['FechaFin'] . ".
                <br>
                Motivo: " . SIMResources::$motivo_laboral[$datos_Solicitud['IDMotivo']] . "
                <br>
                Estado: " . SIMResources::$estado_laboral[$datos_Solicitud['IDEstado']] . "
                <br>
                Comentario solicitud: " . $datos_Solicitud['Comentario'] .
                        "
                <br><br><br>";
                    break;
                case 'Certificado':
                    $datos_Solicitud = $dbo->fetchAll("LaboralCertificado", " IDLaboralCertificado = '" . $Id . "' ", "array");
                    $asunto = "Solicitud de Certificado";
                    $msg = "Tiene una nueva solicitud de Certificado por parte de $NombreSolicitante, Cc $NumeroDocumentoSolicitante.
                    <br>
                    Tipo de certificado: " . SIMResources::$tipo_certificado_laboral[$datos_Solicitud['IDTipoCertificado']] . "
                    <br>
                    A nombre de: " . $datos_Solicitud['AnombreDe'] . "
                    <br>
                    Fechas: " . $datos_Solicitud['Fechas'] . "
                    <br>
                    Estado: " . SIMResources::$estado_laboral[$datos_Solicitud['IDEstado']] . "
                    <br>
                    Comentario solicitud: " . $datos_Solicitud['Comentario'] .
                    ...
                    break;
                case 'Compensatorio':
                    $datos_Solicitud = $dbo->fetchAll("LaboralCompensatorio", " IDLaboralCompensatorio = '" . $Id . "' ", "array");
                    $asunto = "Solicitud de Compensatorio";
                    $msg = "Tiene una nueva solicitud de Compensatorio por parte de $NombreSolicitante, Cc $NumeroDocumentoSolicitante, para el día " . $datos_Solicitud['FechaInicio'] . " hasta " . $datos_Solicitud['FechaFin'] . ".
                        <br>
                        Días: " . $datos_Solicitud['DiasTomar'] . "
                        <br>
                        Estado: " . SIMResources::$estado_laboral[$datos_Solicitud['IDEstado']] . "
                        <br>
                        Comentario solicitud: " . $datos_Solicitud['Comentario'] .
            ...
            $cuerpoMensaje = "Sr(a). " . $Jefe['Nombre'] .
                ", Reciba un cordial saludo.<br><br>
             $msg   
            Por favor no responda este correo, si desea dar una respuesta ingrese al administrador.<br>
            Cordialmente,<br><br>
            <b>Notificaciones " . utf8_decode($NombreClub);
            if (!empty($correo)) :
                self::send_mail($IDClub, $correo, $asunto, $cuerpoMensaje);
            endif;
            $mensaje = "Se ha recibido una nueva solicitud de $Modulo por parte de " . $NombreSolicitante;
            $mensajeConfirmacion = "Cordial saludo: su solicitud de $Modulo se ha radicado con éxito al área encargada.";
            if (!empty($IDUsuario)) {
                //Envio tambien el push con la notificacion
                if ($mensaje != '') :
                    SIMUtil::enviar_notificacion_push_general_funcionario($IDClub, $idJefe, $mensaje);
                endif;
                //Envio tambien el push al remitente
                if ($mensajeConfirmacion != '') :
                    SIMUtil::enviar_notificacion_push_general_funcionario($IDClub, $idUser, $mensajeConfirmacion);
                    SIMUtil::enviar_notificacion_push_general($IDClub, $idUser, $mensajeConfirmacion, 118, $Id);
                endif;
            } else {
                //Envio tambien el push con la notificacion
                if ($mensaje != '') :
                    SIMUtil::enviar_notificacion_push_general($IDClub, $idJefe, $mensaje, 118);
                endif;
                //Envio tambien el push al remitente
                if ($mensajeConfirmacion != '') :
                    SIMUtil::enviar_notificacion_push_general($IDClub, $idUser, $mensajeConfirmacion, 118, $Id);

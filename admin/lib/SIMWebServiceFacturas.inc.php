...
class SIMWebServiceFacturas
{
    ...
    public function get_detalle_facturas_hebraica($IDClub, $IDSocio, $IDFactura, $NumeroFactura)
    {
        ...
        $credito = abs($TotalCredito);
        $Valor = ($TotalDebito - $credito);
        $divisa = ($Valor + ($Valor * 0.03));
        //Redondeamos el valor en usd
        $divisa = round($divisa * 100) / 100;
        ...
        $cuerpo_factura .= ' 
Si quiere hacer el pago en divisas el monto a cancelar es de <b> ' . $divisa . ' USD  </b> <br><br>
Si quiere hacer el pago en Bs, el monto a cancelar es el equivalete a <b> ' . $Valor . ' USD </b>  a la tasa de BCV del día de pago.<br><br>
<img src="https://www.miclubapp.com/file/factura/hebraica/hebraica.png" alt="" width="100%" height="100%" /></body></html>';
    ...
    public function detalle_factura_valle_arriba($IDClub, $IDSocio, $IDFactura, $NumeroFactura)
    {
        ...
        if ($TotalSaldo > 0) {
            ...
            $Totales .= '
                <tr class="modo2">
                    <td class="content-pago">
                    <form target="_self" name="frmpago" id="frmpago" action="' . URLROOT . 'lukapay.php" method="post" enctype="multipart/form-data">
                            <input type="hidden" name="valor" id="valor" value="' . $TotalSaldo . '">
                            <input type="hidden" name="extra1" id="extra1" value="0">
                            <input type="hidden" name="extra2" id="extra2" value="' . $IDClub . '">
                            <input type="hidden" name="IDSocio" id="IDSocio" value="' .  $IDSocio . '">
                            <input type="hidden" name="Accion" id="Accion" value="' . $Accion . '">                    
                            <input type="hidden" name="Modulo" id="Modulo" value="Facturacion">                        
                            <input type="submit" class="btn-pago" value="Pagar">
                        </form>
                    </td>
                </tr>';
        }
            ...
            foreach ($DATOS->socio[$id]->cuentas->cuenta as $ids => $consumos) :
                ...
                if (trim((string)$cuenta) == trim($NumeroFactura)) :
                    foreach ($consumos->detalleconsumos->consumos as $id => $detalle) :
                        ...
                        $total = $detalle->valor; //$detalle->cantidad * $detalle->valor;
...
<style>*{margin:0;padding:0;box-content:border-box}td{text-align:center}.text-factura{font-size:80%}td{border-bottom:2px solid #e7e7e7;border-right:1px solid #e7e7e7;}</style>
    ...
    /* . PLAYA GRANDE FACTURAS ... */
    public function get_factura_playa_grande($IDClub, $IDSocio, $FechaInicio, $FechaFin, $Dispositivo)
    {
        $dbo = &SIMDB::get();
        $IDSocio = SIMNet::req("IDSocio");
       // require(LIBDIR . "SIMWebServiceCampestrePereira.inc.php");
        ...
        //Traigo las facturas del socio desde la tabla CarteraPlayaGrande 

        $dbo = &SIMDB::get();
        $fecha_actual= date("Y-m-d");
        $condicion= "FechaEmision>= '$FechaInicio' and FechaEmision <= '$FechaFin' and ";
        if($FechaInicio != $fecha_actual and $FechaFin != $fecha_actual ):
        $condicion= "FechaEmision>= '$FechaInicio' and FechaEmision <= '$FechaFin' and ";
        endif;
        
        $AccionSocio = $dbo->getFields("Socio", "NumeroDocumento", "IDSocio = '" . $IDSocio . "'");
        $sql_PagosPendientes = "SELECT IDCarteraPlayaGrande, IDClub, Nota1, Saldo , FechaEmision FROM `CarteraPlayaGrande` WHERE $condicion IDSocio = '" . $IDSocio . "' ";
        $qry_PagosPendientes = $dbo->query($sql_PagosPendientes);
        ...
        foreach ($rows as $row) {
            ...
            $id = $row["IDCarteraPlayaGrande"];
            $factura["NumeroFactura"] = $id;
            $factura["Color"] = "";
            $factura["Fecha"] =  $row["Nota1"];
            $factura["ColorFecha"] = "";
            $factura["ValorFactura"] = "$" . number_format($row["Saldo"], 0, ',', '.') . " USD";
            $factura["ValorNumeroFactura"] = $row["Saldo"];
    ...
    public function get_detalle_factura_playa_grande($IDClub, $IDFactura, $NumeroFactura, $Dispositivo, $IDSocio)
    {
        ...
        $sql_valor = "SELECT * FROM CarteraPlayaGrande Where IDCarteraPlayaGrande IN  ( $NumeroFactura ) ";
        
        $r_valor = $dbo->query($sql_valor);
        
        $sql_PagosPendientes = "SELECT * FROM `CarteraPlayaGrande` WHERE  IDCarteraPlayaGrande = '" . $NumeroFactura . "' limit 1";
        $qry_PagosPendientes = $dbo->query($sql_PagosPendientes); 
        $datos = mysqli_fetch_array($qry_PagosPendientes);
        ...
        foreach ($rows as $row) {
            ...
            $valor = "$" . number_format($datos["Saldo"], 0, ',', '.');
            $Pagar = $datos["Saldo"];
            $detalle_producto .= "
                            <tr class=modo1> 
                                <td>". $row["Nota1"] . "</td> 
                                <td>$" . number_format($row["Saldo"], 0, ',', '.') . " USD</td>
                            </tr>";
        }
        ...
        $cabeza_factura =
            '<table border="0" cellpadding="0" cellspacing="0" class="tabla">
                <tbody>
                    <tr class="modo2">
                        <td>Cliente : <font color=#ffffff>' . $row["Nombre"] . '</font></td>
                    </tr>
                    <tr class="modo2">
                        <td>Fecha: <font color=#ffffff>' . $row["FechaEmision"] . '</font></td>
                    </tr>
                    <tr class="modo2">
                        <td>Total: <font color=#ffffff>' . $valor . ' USD</font></td>
                    </tr>
                </tbody>
            </table>';
        ...
        $factura["CuerpoFactura"] = $cuerpo_factura;
        $factura["IDFactura"] = $IDFactura;
        $factura["TotalPagar"] = (int) $datos_extracto["Saldo"];
        $factura["BotonPago"] = "N";
        $factura["WebViewInterno"] = $WebInterno;
        $extracto["Action"] = "https://www.e-collect.com/customers/Plus/ClubCampestrePlus.html";
    ...
    /* . PISPESCA FACTURAS ... */
    public function get_factura_pispesca($IDClub, $IDSocio, $FechaInicio, $FechaFin, $Dispositivo)
    {
        $dbo = &SIMDB::get();
        $IDSocio = SIMNet::req("IDSocio");
        require(LIBDIR . "SIMWebServiceCampestrePereira.inc.php");
        ...
        //Traigo las facturas del socio desde la tabla CarteraPispesca  

        $dbo = &SIMDB::get();

        $AccionSocio = $dbo->getFields("Socio", "NumeroDocumento", "IDSocio = '" . $IDSocio . "'");
        $sql_PagosPendientes = "SELECT IDCarteraPispesca, Nombre, Total , Fecha FROM `CarteraPispesca` WHERE  IDSocio = '" . $IDSocio . "' ";
        $qry_PagosPendientes = $dbo->query($sql_PagosPendientes);
        ...
        foreach ($rows as $row) {
            ...
            $factura["IDClub"] = $row["IDClub"];
            //NECESITO PASAR ESTE A LA FUNCION VARIAS FACTURAS
            $id = $row["IDCarteraPispesca"];
            $factura["NumeroFactura"] = $id;
            $factura["Color"] = "";
            $factura["Fecha"] =  $row["Fecha"];
            $factura["ColorFecha"] = "";
            $factura["ValorFactura"] = "$" . number_format($row["Total"], 0, ',', '.');
            $factura["ValorNumeroFactura"] = $row["Total"];
    ...
    public function get_detalle_factura_pispesca($IDClub, $IDFactura, $NumeroFactura, $Dispositivo, $IDSocio)
    {
        ...
        $sql_valor = "SELECT * FROM CarteraPispesca Where IDCarteraPispesca IN  ( $NumeroFactura ) ";
        
        $r_valor = $dbo->query($sql_valor);
        
        $sql_PagosPendientes = "SELECT * FROM `CarteraPispesca` WHERE  IDCarteraPispesca = '" . $NumeroFactura . "' limit 1";
        $qry_PagosPendientes = $dbo->query($sql_PagosPendientes); 
        $datos = mysqli_fetch_array($qry_PagosPendientes);
        ...
        foreach ($rows as $row) {
            ...
            $valor = "$" . number_format($datos["Total"], 0, ',', '.');
            $Pagar = $datos["Total"];
            $detalle_producto .= "
                            <tr class=modo1> 
                                <td>CUOTA SOCIO</td>
    ...
    /* . BACATA FACTURAS ... */
    public function get_factura_bacata($IDClub, $IDSocio, $FechaInicio, $FechaFin, $Dispositivo)
    {
        $dbo = &SIMDB::get();
        $IDSocio = SIMNet::req("IDSocio");
        require(LIBDIR . "SIMWebServiceCampestrePereira.inc.php");
        ...
        $nombre_categoria = "Cartera";
        $fecha_actual= date("Y-m-d");
        $condicion= "Fecha>= '$FechaInicio' and Fecha <= '$FechaFin' and ";
        if($FechaInicio != $fecha_actual and $FechaFin != $fecha_actual ):
        $condicion= "Fecha>= '$FechaInicio' and Fecha <= '$FechaFin' and ";
        endif;
        ...
        $AccionSocio = $dbo->getFields("Socio", "NumeroDocumento", "IDSocio = '" . $IDSocio . "'");
        $sql_PagosPendientes = "SELECT Factura, Nombre, sum(Total_Neto+Iva) as total , Fecha FROM `CarteraBacata` WHERE $condicion IDSocio = '" . $IDSocio . "' GROUP by Factura ";
        $qry_PagosPendientes = $dbo->query($sql_PagosPendientes);
    ...
    public function get_detalle_factura_bacata($IDClub, $IDFactura, $NumeroFactura, $Dispositivo, $IDSocio)
    {
        ...
        $sql_valor = "SELECT * FROM CarteraBacata Where Factura IN  ( $NumeroFactura ) ";
        
        $r_valor = $dbo->query($sql_valor);
        
        $sql_PagosPendientes = "SELECT Factura, Nombre, sum(Total_Neto+Iva) as total , Fecha FROM `CarteraBacata` WHERE  Factura = '" . $NumeroFactura . "' GROUP by Factura limit 1";
        $qry_PagosPendientes = $dbo->query($sql_PagosPendientes); 
        $datos = mysqli_fetch_array($qry_PagosPendientes);
        ...
        $cabeza_factura =
            '<table border="0" cellpadding="0" cellspacing="0" class="tabla">
                <tbody>
                    <tr class="modo2">
                        <td>Factura : <font color=#ffffff>' . $row["Nombre"] . '</font></td>
                    </tr>
                    <tr class="modo2">
                        <td>Fecha: <font color=#ffffff>' . $fecha . '</font></td>
                    </tr>
 ...
 */
        if ($IDSocio == 344649 || $IDSocio == 10684 || $IDSocio == 574110) :
            $IDSocio = 52995;
        endif;
        ...
        */
        ...
        $arr_SociosConsumos = array(51228, 52089, 49836, 49957, 52970, 52836, 50883, 52995, 49739, 348832);
        ...
        if (!isset($consumos['message']) && in_array($IDSocio, $arr_SociosConsumos)) {
            ...
            foreach ($consumos as $id_consumo => $datos_consumo) {
                ...
                $factura["NumeroFactura"] = "Consumos|$Consecutivo";
                $factura["Fecha"] = $fecha_extracto;
                ...
                $factura["Color"] = "";
                $factura["ColorFecha"] = "";
                $factura["ValorNumeroFactura"] = $ValorTotal;
                $factura["Campos"] = array();
        ...
        //if ($DATOS[error] != 3 && $DATOS[error] != 2) :
        ...
        $nombre_categoria = "Cartera suspendida";
        // $nombre_categoria = "Cartera vencida";
        ...
        foreach ($rows as $row) {
            ...
            $factura["NumeroFactura"] = "CarteraVencida|" . $id;
            $factura["IDFactura"] = "CarteraVencida|" . $id;
            $factura["Color"] = "";
        ...
        $nombre_categoria = "Cartera";
        // Valida si trae datos de estado de cuenta
        if (!isset($DATOS['error'])) {
            foreach ($DATOS as $id => $campos) :
                ...
                $factura["NumeroFactura"] = "Cartera|" . $campos[factura];
        ...
        $datos_facturas["ImagenLateral"] = $urlIMagen;
        $datos_facturas["TextoLateral"] = "";
        $datos_facturas["MostrarDecimal"] = "S";
        $datos_facturas["PermiteSeleccionarVarias"] = "S";
        $datos_facturas["TextoSeleccionarDeseleccionar"] = "Selecionar todas las facturas";
        $datos_facturas["TextoIntroSeleccionarVariasPago"] = "Seleccione las facturas que desea pagar";
        array_push($response, $datos_facturas);
    ...
    public function get_detalle_factura_pereira($IDClub, $IDFactura, $NumerosFacturas, $Dispositivo, $IDSocio)
    {
        ...
        $datos_consulta = explode("|", $NumerosFacturas);
        $ConsecutivoFactura = $datos_consulta[1];
        $NumeroDocumento = $dbo->getFields('Socio', 'NumeroDocumento', "IDSocio = $IDSocio");
        // if ($IDSocio == 5533 || $IDSocio == 574110) {
        //     $NumeroDocumento = 10131111;
        // }
        if ($datos_consulta[0] == "Consumos") {
            $WebInterno = "S";
            ...
            if (!isset($consumos['message'])) {
                foreach ($consumos as $datos_detalle) {
                    if ($datos_detalle['consecutivo'] == $ConsecutivoFactura) {
                        ...
                        $Factura .= $datos_consulta[1] . "|" . $ValorTotal . "|0/";
        ...
        if ($datos_consulta[0] == "Cartera") :
            ...
            $DATOS = SIMWebServiceCampestrePereira::EstadoCuenta($NumeroDocumento);
            foreach ($DATOS as $id => $campos) :
                if ($campos[factura] == $datos_consulta[1]) :
                    $fecha = substr($campos[fecha], 0, 10);
                    $vencimiento = substr($campos[vencimiento], 0, 10);
                    $valor = "$" . number_format($campos[valor], 0, ',', '.');
                    $Pagar = $campos[valor];
                    $Factura .= $datos_consulta[1] . "|" . $campos[valor] . "|" . $campos[cuota] . "/";
            ...
            $cabeza_factura =
                ...
                <tbody>
                    <tr class="modo2">
                        <td>Numero Factura: <font color=#ffffff>' . $NumerosFacturas . '</font></td>
                    </tr>
                    <tr class="modo2">
                        <td>Fecha: <font color=#ffffff>' . $fecha . '</font></td>
                    </tr>
            ...
            $boton =
                '   <form target="_self" name="frmpago" id="frmpago" action="' . URLROOT . 'CredibancoVApi.php" method="post">
                            ...
                            <input type="hidden" name="NumeroDocumento" id="NumeroDocumento" value="' . $NumeroDocumento . '">
                            <input type="hidden" name="Accion" id="Accion" value="' . $AccionSocio . '">                    
                            <input type="hidden" name="ConsumoId" id="ConsumoId" value="">
                            <input type="hidden" name="ref" id="ref" value="' . $SiguienteNumeroFac . '">
                            <input type="hidden" name="Factura" id="Factura" value="' . $Factura . '">
                            <input type="hidden" name="Modulo" id="Modulo" value="CarteraPereira">
                        </form>';
        ...
        if ($datos_consulta[0] == "CarteraVencida") :
            $detalle_producto .=
                "<tbody>
            <tr>
                <th>Concepto</th>
            ...
            $sql_valor = "SELECT * FROM CarteraVencida Where IDCarteraVencida =" . $datos_consulta[1];
            $r_valor = $dbo->query($sql_valor);
            ...
            foreach ($rows as $row) {
                ...
                $Pagar = $row["Valor"];
                $Factura .= $datos_consulta[1] . "|" . $Pagar . "|0/";
            ...
            $cabeza_factura =
                ...
                <tbody>
                    ...
                    <tr class="modo2">
                        <td>Total: <font color=#ffffff>' . $Pagar . '</font></td>
                    </tr>
                </tbody>
            </table>';
        ...
        $factura["IDClub"] = $IDClub;
        $factura["CuerpoFactura"] = $cuerpo_factura;
        $factura["IDFactura"] = $NumerosFacturas;
        $factura["TotalPagar"] = (int) $datos_extracto["Valor"];
        $factura["BotonPago"] = "N";
        $factura["WebViewInterno"] = $WebInterno;
        $extracto["Action"] = "https://www.e-collect.com/customers/Plus/ClubCampestrePlus.html";
    ...
    public function get_detalle_varias_factura_pereira($IDClub, $IDFactura, $NumerosFacturas, $Dispositivo, $IDSocio)
    {
        require(LIBDIR . "SIMWebServiceCampestrePereira.inc.php");
        $dbo = &SIMDB::get();
        $WebInterno = "S";
        ...
        $arr_validaFactura = array();
        // $comas = strpos($IDFactura, ",");
        // if ($comas === false) :
        //     $ArregloIDFactura[] = $IDFactura;
        // else :
        //     $ArregloIDFactura = explode(",", $IDFactura);

        //     foreach ($ArregloIDFactura as $arrFactura) {
        //         $data_factura = explode('|', $arrFactura);
        //         array_push($arr_validaFactura, $data_factura[0]);
        //     }

        // endif;


        $comas = strpos($NumerosFacturas, ",");
        if ($comas === false) :
            $ArregloNumeroFactura[] = $NumerosFacturas;
        else :
            $ArregloNumeroFactura = explode(",", $NumerosFacturas);
            foreach ($ArregloNumeroFactura as $arrFactura) {
                $data_factura = explode('|', $arrFactura);
                array_push($arr_validaFactura, $data_factura[0]);
            }
        endif;
        ...
        // Valida si van a pagar Consumos y Cartera a la vez
        if (in_array('Cartera', $arr_validaFactura) && in_array('Consumos', $arr_validaFactura)) {
            $respuesta["message"] = "Lo sentimos, no se pueden pagar Consumos y Cartera al mismo tiempo.";
            $respuesta["success"] = false;
            $respuesta["response"] = null;
            return $respuesta;
        }
        // Fin Valida si van a pagar Consumos y Cartera a la vez
        ...
        foreach ($ArregloNumeroFactura as $id1 => $NumeroFactura) :
            $datos_consulta = explode("|", $NumeroFactura);
            // Factura vencida
            if ($datos_consulta[0] == "CarteraVencida") {
                ...
                foreach ($rows as $row) {
                    ...
                    $Pagar += $row["Valor"];
                    $total += $row["Valor"];
                    $Factura .= $datos_consulta[1] . "|" . $row['Valor'] . "|0/";
            ...
            if ($datos_consulta[0] == "Consumos") {
                ...
                if (!isset($consumos['message'])) {
                    ...
                    foreach ($consumos as $id_consumo => $datos_detalle) {
                        if (in_array($datos_detalle['consecutivo'], $datos_consulta)) {
                            $NumeroConsumo = $datos_detalle["consecutivo"];
                            $FechaConsumo = $datos_detalle["horaConsumo"];
                            $Accion = $datos_detalle["accionId"];
                            $Documento = $datos_detalle["terceroId"];
                            $Producto = $datos_detalle["nombreProducto"];
                            $Cantidad = $datos_detalle["cantidad"];
                            $Valor = $datos_detalle["precioPublico"];
                            $Descuento = $datos_detalle["descuento"];
                            $Iva = $datos_detalle["valorImpuesto"];
                            $SumaIva += $Iva;
                            $Subtotal = $Valor;
                            $Subtotalitem = $Cantidad * $Valor;
                            ...
                            $fecha = $FechaConsumo;
                            $vencimiento = "";
                            $valor = "$" . number_format($Subtotalitem, 0, ',', '.');
                            $Pagar += $Subtotalitem;
                            $total += $Subtotalitem;
                            $Factura .= $datos_consulta[1] . "|" . $SubTotal . "|0/";
        ...
        $cabeza_factura =
            ...
            <tbody>
                <tr class="modo2">
                    <td>Facturas: <font color=#ffffff>' . $NumerosFacturas . '</font></td>
                </tr>              
                <tr class="modo2">
        ...
        $boton =
            '   <form target="_self" name="frmpago" id="frmpago" action="' . URLROOT . 'CredibancoVApi.php" method="post">
                            ...
                            <input type="hidden" name="extra2" id="extra2" value="' . $IDClub . '">
                            <input type="hidden" name="IDSocio" id="IDSocio" value="' .  $IDSocio . '">
                            <input type="hidden" name="NumeroDocumento" id="NumeroDocumento" value="' . $Cedula . '">
                            <input type="hidden" name="Accion" id="Accion" value="' . $AccionSocio . '">                    
                            <input type="hidden" name="ConsumoId" id="ConsumoId" value="' . time() . '">
                            <input type="hidden" name="ref" id="ref" value="' . $SiguienteNumeroFac . '">
                            <input type="hidden" name="Factura" id="Factura" value="' . $Factura . '">
                ...
                while ($r = $qry_factura->fetch()) {
                    ...
                    while ($row_detalle = $r_detalle->fetch()) {
                        $detalle_factura .= '
                                            ...
                                            ';
                        $TotalDetalleVenta += $row_detalle["vlor_ttal"];
                    }
                    ...
                    if ($r[cdgo_estdo] == 'A') :
                        ...
                        $TotalPagar = $TotalPagar + $TotalDetalleVenta;
